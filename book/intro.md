...
Reth is a new Ethereum full node that allows users to sync and interact with the entire blockchain, including its historical state if in archive node.
...
As a data engineer/analyst, or as a data indexer, you'll want to use Archive node. For all other use cases where historical access is not needed, you can use Full node.
