...
//!
//! Each cached engine holds a `MockEthProvider` + `EngineApiTreeHandler`. Access
//! is serialized per engine via mutex to prevent concurrent test interference.
//! After each test, an FCU resets the chain head back to genesis.
...
/// The engine tree type used by engine-x tests.
...
/// Execute a single engine-x test against a cached engine.
fn run_single_test(
    name: &str,
    def: &ExTestDef,
    ...
    for (payload_idx, entry) in def.engine_new_payloads.iter().enumerate() {
        ...
        if let Err(err) = engine.tree.on_engine_message(msg) {
            if expects_error {
                continue;
            }
            return Err(Error::Assertion(format!(
        ...
        if let Err(err) = engine.tree.on_engine_message(fcu_msg) {
            return Err(Error::Assertion(format!(
                "Test case: {name}\nPayload {payload_idx}: \
...
fn reset_to_genesis(engine: &mut CachedEngine) {
    ...
    // Best-effort reset: ignore errors (the engine may be in a broken state
    // after a failed test, but the next test group will get a fresh engine).
