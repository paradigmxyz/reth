...
Once the `execute` function returns, the `execute_and_verify_receipt` continues with verifying the receipt by calculating the `receipts_root` and comparing it to the `header.receipts_root` from the header originally passed into the `execute_and_verify_receipt` function. In addition to the `receipts_root`, the `logs_bloom` is also evaluated against the `header.logs_bloom`.
[File: ]()
```rust ignore
/// Verify receipts
