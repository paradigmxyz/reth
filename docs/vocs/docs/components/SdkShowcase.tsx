...
export function SdkShowcase() {
  return (
    <div className="reth-sdk-grid">
      {projects.map((project) => (
        <article
          key={project.name}
          className="reth-sdk-card"
        >
          <div className="reth-sdk-badge">
            {project.loc} LoC
          </div>

          <h3>{project.name}</h3>
          <p className="reth-sdk-company">{project.company}</p>
          <p className="reth-sdk-description">{project.description}</p>

          <a
            href={project.githubUrl}
            target="_blank"
            rel="noopener noreferrer"
            className="reth-sdk-link"
          >
            <svg aria-hidden="true" className="reth-sdk-link-icon" fill="currentColor" viewBox="0 0 24 24">
