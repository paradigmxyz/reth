# reth db static-files

Static file operations (split, etc.)

```bash
$ reth db static-files --help
```
```txt
Usage: reth db static-files [OPTIONS] <COMMAND>

Commands:
  split  Split static files into new files with different blocks-per-file setting
