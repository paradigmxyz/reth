# reth db debug-report

Generates a debug report for state root validation errors. Use this when encountering invalid state root errors to collect all necessary information for bug reports

```bash
$ reth db debug-report --help
```
```txt
Usage: reth db debug-report [OPTIONS] --block <BLOCK>

Options:
      --block <BLOCK>
          The block number to generate a debug report for. This is typically the block where state root validation failed
  ...
  -o, --output <OUTPUT>
          Output file path for the debug report (JSON format). If not specified, outputs to stdout
