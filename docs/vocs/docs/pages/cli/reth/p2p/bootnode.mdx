...
Options:
      ...
      --nat <NAT>
          ...
          Can be repeated with one IPv4 and one IPv6 `extip:<IP>` to advertise a dual-stack node. The enode advertises only the `--addr` family; the discv5 and EIP-868 ENRs carry both families, and with `--v5` discv4 also answers the opposite family on the shared port.
      ...
      --v5
          ...
          The opposite-family sibling socket is only bound when an `extip:<IP>` of that family is advertised via `--nat`; the default `--nat any` serves the `--addr` family only.

      --bootnodes <BOOTNODES>
          Comma-separated nodes to seed discovery: `enode://` URLs with an IP host and/or signed `enr:` records. Each entry seeds discv4 and (with `--v5`) discv5
