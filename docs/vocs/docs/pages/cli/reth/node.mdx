...
Engine:
      ...
      --engine.sparse-trie-retain-blocks <SPARSE_TRIE_RETAIN_BLOCKS>
          Keep trie nodes touched within this many recent blocks in the sparse trie cache even after those blocks are persisted. The cache always keeps what the unpersisted in-memory chain touched; a larger window needs fewer proofs per block at the cost of memory
