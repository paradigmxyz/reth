...
RPC:
      ...
      --rpc.eth-proof-window <RPC_ETH_PROOF_WINDOW>
          The maximum proof window for historical proof generation. This value allows for generating historical proofs up to a configured number of blocks from current tip (up to `tip - window`)
