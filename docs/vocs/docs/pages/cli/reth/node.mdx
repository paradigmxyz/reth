...
Engine:
      ...
      --engine.trie-prewarm-threads <TRIE_PREWARM_THREADS>
          Number of threads that warm the trie table pages a block's proofs will read, walking the paths named by the block access list while the block executes. 0 disables it
