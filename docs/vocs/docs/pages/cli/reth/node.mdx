...
Debug:
      ...
      --debug.snap-trust-unverified-bals
          Accept BALs from peers without integrity checks when the corresponding header carries no `block_access_list_hash` (i.e. pre-Amsterdam blocks).

          This is intended for testing snap/2 against networks that do not propagate BALs (e.g. mainnet today). Do not enable on production-facing nodes: a malicious peer can serve any BAL it likes for the affected blocks.
