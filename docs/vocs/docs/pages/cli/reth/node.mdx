...
Engine:
      ...
      --engine.skip-state-root-validation-for-bench
          UNSAFE benchmark-only mode for block production and `engine_newPayload` validation. Trusts header state roots, skips state-root/trie computation, may write invalid non-reusable chain data, and must not be used for production validation
