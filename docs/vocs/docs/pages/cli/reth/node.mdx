...
Engine:
      ...
      --engine.share-sparse-trie-with-payload-builder
          ...
          When enabled, the engine spawns a sparse trie computation pipeline at FCU time and passes it to the payload builder, enabling incremental state root computation during payload building instead of a blocking `state_root_with_updates()` call.
