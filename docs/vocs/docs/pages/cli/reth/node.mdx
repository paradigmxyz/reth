...
RPC State Cache:
      ...
      --rpc-cache.idle-timeout <DURATION>
          Evict blocks, receipts, and block access lists after this duration without a cache hit (e.g. 5m, 30s). Zero disables expiration. Cleanup runs at most once per second and at least once per minute, so idle entries may remain until the next sweep
