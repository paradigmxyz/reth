...
Engine:
      ...
      --engine.storage-worker-count <STORAGE_WORKER_COUNT>
          Configure the number of storage proof workers spawned for each block. If not specified, the count is derived from the block: 2x available parallelism, halved for blocks with few transactions and doubled for blocks at or above 60M gas
