...
RPC State Cache:
      ...
      --rpc-cache.prewarm-bals
          Replay new canonical blocks to generate and cache block access lists before native BAL support.

          Also cache block access lists generated by RPC requests. Prewarming stops when the first canonical block with a block access list hash is received.
