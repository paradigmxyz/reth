...
RPC:
      ...
      --http.compression <HTTP_COMPRESSION>
          Configure HTTP response compression algorithms.
          
          To disable compression entirely, use --http.disable-compression.
          
          Algorithm preference order is enforced: zstd > gzip > deflate > br

          [default: zstd,gzip,deflate]
          
          [possible values: zstd, gzip, deflate, br]

      --http.decompression <HTTP_DECOMPRESSION>
          Enable HTTP request decompression with a comma-separated list of algorithms.
          
          Decompression is disabled by default and only enabled when algorithms are specified.
      ...
      --rpc.max-request-size <RPC_MAX_REQUEST_SIZE>
          ...
          When HTTP decompression is enabled (via --http.decompression), this limit applies to the decompressed payload size.
