...
Engine:
      --engine.persistence-threshold <PERSISTENCE_THRESHOLD>
          ...
          To persist blocks as fast as the node receives them, set this value and `--engine.num-state-masking-blocks` to zero. This will cause more frequent DB writes.
