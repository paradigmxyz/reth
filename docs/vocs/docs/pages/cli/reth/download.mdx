...
Storage:
  ...
  -y, --non-interactive
      ...
      --list
          ...
          Queries the snapshots API and prints all available snapshots for the selected chain, including profile (minimal/full/archive), channel (stable/edge), block number, and size.

      --channel <CHANNEL>
          Preferred release channel for automatic snapshot discovery.

          Controls which channel is selected when discovering the latest snapshot. Use with --list to filter the listing by channel.

          [default: stable]
          [possible values: stable, edge]
