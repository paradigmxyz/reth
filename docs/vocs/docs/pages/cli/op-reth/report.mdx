...
Static Files:
      ...
      --block <BLOCK>
          The block number to include in the report. If not specified, only general database information will be collected

      --checksum
          Include checksum of all database tables. WARNING: This will take a very long time to run!

  -o, --output <OUTPUT>
          Output file path for the debug report archive. Defaults to `reth_report_<timestamp>.tar.gz` in the current directory
