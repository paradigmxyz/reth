# op-reth db settings set account_history_rocksdb

Store account history in RocksDB instead of the database

```bash
$ op-reth db settings set account_history_rocksdb --help
```
```txt
Usage: op-reth db settings set account_history_rocksdb [OPTIONS] <VALUE>
