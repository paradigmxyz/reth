...
RPC:
      ...
      --http.vhosts <HTTP_VHOSTS>
          Comma separated list of virtual hostnames from which to accept HTTP requests (server enforced). Accepts '*' wildcard
      ...
      --ws.vhosts <WS_VHOSTS>
          Comma separated list of virtual hostnames from which to accept `WebSocket` requests (server enforced). Accepts '*' wildcard
