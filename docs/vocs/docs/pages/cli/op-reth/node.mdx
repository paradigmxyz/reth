...
Metrics:
      ...
      --metrics.prometheus.push.username <USERNAME>
          Username for HTTP Basic Authentication to the push gateway.

          Must be used together with `--metrics.prometheus.push.password`. Only applies when a push gateway URL is configured.

          [env: RETH_METRICS_PUSH_USERNAME=]

      --metrics.prometheus.push.password <PASSWORD>
          Password for HTTP Basic Authentication to the push gateway.

          Must be used together with `--metrics.prometheus.push.username`. Only applies when a push gateway URL is configured.
