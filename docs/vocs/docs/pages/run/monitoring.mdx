...
```bash
reth node --metrics.prometheus 127.0.0.1:9001
```

Now, as the node is running, you can `curl` the endpoint you provided to the `--metrics.prometheus` flag to get a text dump of the metrics at that time:
...
This will start a Prometheus service which [by default scrapes itself about the current instance](https://prometheus.io/docs/introduction/first_steps/#:~:text=The%20job%20contains%20a%20single,%3A%2F%2Flocalhost%3A9090%2Fmetrics.). So you'll need to change its config to hit your Reth node’s metrics endpoint at `localhost:9001` which you set using the `--metrics.prometheus` flag.
...
As this might be a point of confusion, `localhost:9001`, which we supplied to `--metrics.prometheus`, is the endpoint that Reth exposes, from which Prometheus collects metrics. Prometheus then exposes `localhost:9090` (by default) for other services (such as Grafana) to consume Prometheus metrics.
