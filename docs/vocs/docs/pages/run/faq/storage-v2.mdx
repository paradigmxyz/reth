...
The pipeline's `move_to_static_files` step is a legacy `storage.v1` backfill path. `storage.v2`
nodes write directly to their final storage backends, so that migration step is skipped.
