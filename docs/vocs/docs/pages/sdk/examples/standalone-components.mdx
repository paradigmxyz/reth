...
let factory = EthereumNode::provider_factory_builder()
    .open_read_only(MAINNET.clone(), "path/to/datadir", runtime)?;
...
```rust
let factory = EthereumNode::provider_factory_builder()
    .open_read_only(MAINNET.clone(), ReadOnlyConfig::from_datadir("datadir").disable_long_read_transaction_safety(), runtime)?;
```
...
- `--engine.persistence-threshold 0 --engine.num-state-masking-blocks 0` - Persists new canonical blocks to disk immediately

Using these flags ensures external processes can read new blocks without delay.
