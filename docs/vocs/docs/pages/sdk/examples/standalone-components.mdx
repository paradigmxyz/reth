...
By default, Reth buffers 2 blocks in memory before persisting to disk. External readers will see data lagging by ~2 blocks.

For immediate persistence:

```bash
reth node --engine.persistence-threshold 0
```

This increases disk I/O but makes blocks visible immediately. See [`example-engine-persistence-threshold`](https://github.com/paradigmxyz/reth/tree/main/examples/engine-persistence-threshold) for a demonstration.
