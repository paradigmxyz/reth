...
| [`ots`](/jsonrpc/ots)       | The `ots` API provides partial Otterscan support; address history search is unimplemented.              | No        |
...
You can pass the `all` option, which enables the JSON-RPC namespaces `admin,debug,eth,net,trace,txpool,web3,rpc,reth,ots,flashbots,miner,mev` on the HTTP server. The `testing` namespace is excluded and must be selected explicitly:
...
To enable the testing namespace on a trusted endpoint, include `testing` in an explicit list, for example `--http.api eth,testing`.
...
The IPC transport is enabled by default and has access to all namespaces except `testing`, unless explicitly disabled with `--ipcdisable`.
...
```bash
reth node --http --http.api "admin,debug,eth,net,trace,txpool,web3,rpc,reth,ots,flashbots,miner,mev"
```
