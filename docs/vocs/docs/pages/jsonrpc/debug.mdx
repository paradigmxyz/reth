...
The optional second argument accepts the same tracer options as [`debug_traceBlock`](#debug_traceblock).

| Client | Method invocation                                                              |
| ------ | ------------------------------------------------------------------------------ |
| RPC    | `{"method": "debug_intermediateRoots", "params": [block_hash, opts]}` |
...
Returns account information, without the storage root, after replaying a block through the transaction at the given index.
