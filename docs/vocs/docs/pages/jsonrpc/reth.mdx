...
If `count` is provided, `count` consecutive blocks starting at the given block are re-executed and the merged execution outcome is returned. `count` must be between 1 and 128.
...
The action is one of `enable`, `disable`, `pause` or `unpause`.
...
Subscribe to finalized chain notifications. Committed chain notifications are buffered and emitted once a new finalized block is received; each notification contains all committed chain segments up to (and including) the finalized block, in the same format as `reth_subscribeChainNotifications`.
...
// Example notification once a new finalized block is known: a list of the committed chain segments
{"jsonrpc":"2.0","method":"reth_subscription","params":{"subscription":"0xcd2e3f4a5b6c7d8e9f0a1b2c3d4e5f60","result":[{"Commit":{"new":{"blocks":[...],"receipts":[...],"first_block":1000,"last_block":1002}}}]}}
