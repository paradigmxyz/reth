...
impl<T: PayloadTypes> PayloadBuilderHandle<T> {
    ...
    /// Returns a stream of the best payloads built for a given ID.
    /// Payloads are emitted as soon as they are generated by the inner job.
    /// Returns `None` if no jobs are found for the given ID.
    pub async fn subscribe_best_payload(
        &self,
        id: PayloadId,
    ) -> Option<
        Result<Pin<Box<dyn Stream<Item = T::BuiltPayload> + Send + 'static>>, PayloadBuilderError>,
    > {
        let (tx, rx) = oneshot::channel();
        self.to_service.send(PayloadServiceCommand::SubscribeBestPayloads(id, tx)).ok()?;
        rx.await.map_err(PayloadBuilderError::from).transpose()
    }
...
where
    ...
    Gen: PayloadJobGenerator,
    Gen::Job: PayloadJob<PayloadAttributes = T::PayloadBuilderAttributes>,
    <Gen::Job as PayloadJob>::BuiltPayload: Into<T::BuiltPayload> + 'static,
{
    ...
    fn subscribe_best_payloads(
        &self,
        id: PayloadId,
    ) -> Option<Pin<Box<dyn Stream<Item = T::BuiltPayload> + Send>>> {
        let job = self.payload_jobs.iter().find(|(_, job_id)| *job_id == id)?;
        let stream = job.0.subscribe_best_payloads();
        Some(Box::pin(stream.map(Into::into)))
    }
}
        ...
        loop {
            ...
            while let Poll::Ready(Some(cmd)) = this.command_rx.poll_next_unpin(cx) {
                match cmd {
                    ...
                    PayloadServiceCommand::SubscribeBestPayloads(id, tx) => {
                        let stream = this.subscribe_best_payloads(id);
    ...
    T: PayloadTypes,
{
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            Self::SubscribeBestPayloads(f0, _) => {
                f.debug_tuple("SubscribeBestPayloads").field(&f0).finish()
            }
        }
    }
}
