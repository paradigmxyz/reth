...
impl<N: NodePrimitives> StateTrieOverlayManager<N> {
    ...
    /// Returns the sparse trie clone for `parent_hash`.
    pub fn sparse_trie_for_parent(
        ...
        (anchored.block_hash == parent_hash).then(|| Arc::clone(&anchored.trie))
    }
...
mod tests {
    ...
    #[test]
    fn insert_block_does_not_prepare_child_overlay_from_cached_parent() {
        let manager = StateTrieOverlayManager::default();
        let blocks = test_blocks();
    ...
    #[test]
    fn sparse_trie_clone_is_replaced_and_retained_after_block_removal() {
        let manager = StateTrieOverlayManager::default();
        let blocks = test_blocks();
        ...
        manager.replace_sparse_trie(first_hash, StateTrieOverlaySparseTrie::default());
        assert!(manager.sparse_trie_for_parent(first_hash).is_some());
        assert!(manager.sparse_trie_for_parent(B256::with_last_byte(0xaa)).is_none());
