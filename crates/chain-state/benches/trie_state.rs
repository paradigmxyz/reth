...
fn generate_erc20_blocks<N: NodePrimitives>(len: usize) -> Vec<ExecutedBlock<N>> {
...
fn generate_raw_transfer_blocks<N: NodePrimitives>(len: usize) -> Vec<ExecutedBlock<N>> {
    let mut rng = rand::thread_rng();
...
fn generate_uniswap_blocks<N: NodePrimitives>(len: usize) -> Vec<ExecutedBlock<N>> {
    let mut rng = rand::thread_rng();
