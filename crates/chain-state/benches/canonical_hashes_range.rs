...
use reth_chain_state::{
    test_utils::TestBlockBuilder, MemoryOverlayStateProviderRef, ExecutedBlockWithTrieUpdates,
};
...
fn bench_canonical_hashes_range(c: &mut Criterion) {
    ...
    for (name, num_blocks) in scenarios {
        group.bench_function(format!("{}_blocks_{}", name, num_blocks), |b| {
            ...
            b.iter(|| {
                black_box(provider.canonical_hashes_range(
                    black_box(start_block),
                    black_box(end_block)
                ).unwrap())
            })
        });
    ...
    group.bench_function("all_in_memory_matches", |b| {
        ...
        b.iter(|| {
            black_box(provider.canonical_hashes_range(
                black_box(first_block),
                black_box(last_block + 1)
            ).unwrap())
        })
    });
...
fn setup_provider_with_blocks(
    num_blocks: usize,
) -> (MemoryOverlayStateProviderRef<'static, EthPrimitives>, Vec<ExecutedBlockWithTrieUpdates<EthPrimitives>>) {
