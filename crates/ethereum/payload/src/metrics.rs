...
pub(crate) fn record_inclusion_list_transaction_included() {
    metrics::counter!("ef_execution_inclusion_list_block_building_transactions_included_total")
        .increment(1);
}
...
pub(crate) fn record_inclusion_list_transaction_excluded(reason: &'static str) {
    metrics::counter!(
        "ef_execution_inclusion_list_block_building_transactions_excluded_total",
        "reason" => reason
    )
    .increment(1);
}
