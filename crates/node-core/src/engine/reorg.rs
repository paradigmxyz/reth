...
use reth_primitives::{
    ...
    proofs,
    revm::{
        config::revm_spec_by_timestamp_after_merge,
        env::{fill_block_env, tx_env_with_recovered},
    },
    Block, Header, Receipt, Receipts,
};
use reth_provider::{BlockReader, BundleStateWithReceipts, ProviderError, StateProviderFactory};
use reth_revm::{
...
use reth_rpc_types_compat::engine::payload::try_block_to_payload;
use revm::{
    db::states::bundle_state::BundleRetention,
    primitives::{BlockEnv, CfgEnv, CfgEnvWithHandlerCfg, EVMError, EnvWithHandlerCfg},
...
pub struct EngineReorg<S, Engine: EngineTypes, Provider> {
    /// Underlying stream
    #[pin]
    stream: S,
    /// Database provider.
...
impl<S, Engine: EngineTypes, Provider> EngineReorg<S, Engine, Provider> {
...
impl<S, Engine, Provider> Stream for EngineReorg<S, Engine, Provider>
where
    ...
    Provider: BlockReader + StateProviderFactory,
{
    ...
    let next_block = payload_validator
        ...
        .map_err(|error| RethError::Custom(error.to_string()))?;
    ...
    let cfg_env = CfgEnvWithHandlerCfg::new_with_spec_id(
        cfg,
        revm_spec_by_timestamp_after_merge(chain_spec, reorg_target.timestamp),
    );
    let mut block_env = BlockEnv::default();
    fill_block_env(&mut block_env, chain_spec, &reorg_target.header, true);
    ...
    for tx in next_block.body {
        ...
        let result = revm::Evm::builder()
            .with_db(&mut state)
            .with_env_with_handler_cfg(EnvWithHandlerCfg::new_with_cfg_env(
                cfg_env.clone(),
                block_env.clone(),
                tx_env_with_recovered(&tx_recovered),
            ))
            .build()
            .transact();
        ...
        state.commit(exec_result.state);
    ...
    let reorg_block = Block {
        header: Header {
            ...
            receipts_root: bundle_state.receipts_root_slow(reorg_target.header.number).unwrap(),
            logs_bloom: bundle_state.block_logs_bloom(reorg_target.header.number).unwrap(),
            gas_used: cumulative_gas_used,
            blob_gas_used,
            excess_blob_gas,
            state_root: state_provider.state_root(bundle_state.state())?,
        },
        body: transactions,
    ...
    Ok((
        try_block_to_payload(reorg_block),
        reorg_target
            .header
