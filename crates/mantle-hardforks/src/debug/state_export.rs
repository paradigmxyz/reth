...
fn extract_storage_key_mapping_from_bundle(
    bundle_state: &BundleState,
) -> BTreeMap<B256, BTreeMap<B256, B256>> {
    ...
    for (address, account) in &bundle_state.state {
        ...
        for slot in account.storage.keys() {
            ...
            mapping.entry(hashed_address).or_default().insert(hashed_key, original_key);
        }
    }
...
/// * `export_bundle_storage_only` - If true, only export storage details for accounts in
///   `bundle_state`
///
/// # Format
    ...
    Provider: DBProvider,
{
    ...
    let mut plain_storage_cursor =
        provider.tx_ref().cursor_dup_read::<tables::PlainStorageState>()?;
    let mut current_address: Option<Address> = None;

    while let Some((address, storage_entry)) = if current_address.is_none() {
    ...
    for (hashed_addr, keys) in bundle_storage_mapping {
        storage_key_mapping.entry(hashed_addr).or_default().extend(keys);
    }
        ...
        if let Some(account_info) = account_opt {
            ...
            match hashed_storage_cursor.seek_exact(hashed_address)? {
                Some((found_addr, storage_entry)) if found_addr == hashed_address => {
                    ...
                    let original_key = storage_key_mapping
                        .get(&hashed_address)
            ...
            let (code, code_hash_str) = if let Some(code_hash) = account_info.bytecode_hash {
                ...
                (
                    "0x".to_string(),
                    format!("0x{}", hex::encode(alloy_primitives::KECCAK256_EMPTY.as_slice())),
                )
            };
            ...
            writeln!(
                writer,
                "      \"hashed_address\": \"0x{}\",",
                hex::encode(hashed_address.as_slice())
            )?;
            ...
            if should_export_storage {
                ...
                for (original_key, value) in storage_by_original_key {
                    ...
                    writeln!(
                        writer,
                        "\n        \"0x{}\": {{",
                        hex::encode(original_key.as_slice())
                    )?;
                    writeln!(
                        writer,
                        "          \"original_key\": \"0x{}\",",
                    ...
                    writeln!(
                        writer,
                        "          \"hashed_key\": \"0x{}\",",
                        hex::encode(hashed_key.as_slice())
                    )?;
                    writeln!(
                        writer,
                        "          \"value\": \"0x{}\"",
