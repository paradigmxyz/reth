    ...
    Chainspec: EthereumHardforks,
{
        ...
        DB::Error: Display,
    {
        ...
        if let Some(ref mut hook) = self.hook {
            hook.on_state(&result_and_state.state, false);
        }
    ...
    pub fn on_state(&mut self, state: &EvmState, is_final: bool) {
        if let Some(ref mut hook) = &mut self.hook {
            hook.on_state(state, is_final);
        }
    }
}
