...
pub struct MockExecutorProvider {
    exec_results: Arc<Mutex<Vec<BlockExecutionOutcome>>>,
}

impl MockExecutorProvider {
    /// Extend the mocked execution results
    pub fn extend(&self, results: impl IntoIterator<Item = impl Into<BlockExecutionOutcome>>) {
...
impl<DB> Executor<DB> for MockExecutorProvider {
    ...
    fn execute(self, _: Self::Input<'_>) -> Result<Self::Output, Self::Error> {
        let BlockExecutionOutcome { bundle, receipts, requests, first_block: _ } =
