...
impl CachedReads {
    /// Gets a database wrapper for compatibility with cache-aware call sites.
    pub const fn as_db<DB>(&mut self, db: DB) -> CachedReadsDb<'_, DB> {
        CachedReadsDb { cached: self, db }
    }

    /// Gets a mutable database wrapper for compatibility with cache-aware call sites.
...
/// Cache wrapper retained for compatibility with cache-aware call sites.
#[derive(Debug)]
...
/// Mutable cache wrapper retained for compatibility with cache-aware call sites.
#[derive(Debug)]
