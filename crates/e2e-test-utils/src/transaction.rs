...
/// `TransactionTestContext` is a structure used for testing transactions.
///
/// This structure contains the necessary context for creating and manipulating
/// transactions in a test environment. It includes a `chain_id` to identify the
/// blockchain network and a `wallet` to sign and send transactions.
///
/// # Fields
///
/// * `chain_id` - The ID of the blockchain network. This is used to prevent replay attacks.
/// * `wallet` - A `LocalWallet` instance used to sign and send transactions.
#[derive(Clone)]
...
impl TransactionTestContext {
    ...
    /// Creates a new EIP-1559 transaction with the current `chain_id` and signs it.
    ...
    /// Creates a new EIP-4844 transaction with the current `chain_id` and signs it.
    ...
    /// Creates a new Optimism block info transaction with the current `chain_id` and signs it.
    ...
    /// Signs and encodes a given transaction request.
    ...
    /// Creates a dummy EIP-4844 transaction with the current `chain_id`.
    ...
    /// Validates the sidecar of a given transaction envelope.
