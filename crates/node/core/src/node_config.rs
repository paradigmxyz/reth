...
mod tests {
    ...
    fn test_load_path_works() {
        with_config_path(|path| {
            let config: ExampleConfig = NodeConfig::load_path(path).expect("load_path failed");
    ...
    fn test_load_path_reads_existing_config() {
        with_config_path(|path| {
            let config: ExampleConfig = ExampleConfig { name: "Test".to_string(), count: 42 };
            ...
            let loaded = NodeConfig::load_path(path).expect("load_path failed");
    ...
    fn test_load_path_fails_on_invalid_toml() {
        with_config_path(|path| {
            ...
            let result: eyre::Result<ExampleConfig> = NodeConfig::load_path(path);
