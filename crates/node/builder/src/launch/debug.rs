    ...
    B: BlockProvider<Block = BlockTy<N::Types>> + Clone,
{
    ...
    async fn launch_node(self) -> eyre::Result<NodeHandle<N, AddOns>> {
        ...
        if let Some(provider) = debug_block_provider {
            ...
            if all_ws && urls.len() > 1 {
                info!(
                    target: "reth::cli",
                    "Using WS fallback RPC consensus client with {} URL(s)",
                    urls.len(),
                );

                let block_provider =
                    RpcBlockProvider::<AnyNetwork, _>::new_with_ws_fallback(
                        &urls,
                        |block_response| {
                ...
                handle.node.task_executor.spawn_critical_task(
                    "ws fallback rpc consensus client",
                    async move { rpc_consensus_client.run().await },
                );
            } else {
                info!(
                    target: "reth::cli",
                    "Using RPC consensus client with {} URL(s)",
                    urls.len(),
                );
