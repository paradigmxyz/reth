...
use reth_node_core::{
    ...
    version::{CARGO_PKG_VERSION, CLIENT_CODE, NAME_CLI//! Builder support for rpc components.
        ...
        ) -> eyre::Result<()>,
    {
        ...
        let launch_rpc = match mode {
            RpcServerMode::TransportOnly => {
                let handle = RethRpcServerHandles {
                    rpc: Some(server_config.start(&cloned_modules).await?),
                    auth: None, // no auth server
                };
                Ok(handle).boxed()
            }            
        };
        ...
        let launch_rpc = match mode {
            RpcServerMode::AuthOnly => futures::future::ok(Default::default()).boxed(), // ❌ This line
            _ => {
                ...
            }
        };
