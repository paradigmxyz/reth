...
/// ## Internals
///
/// The node builder is fully type safe, it uses the [`NodeTypesWithDBAdapter`] trait to enforce
/// that all components are configured with the correct types. However the database types and with
/// that the provider trait implementations are currently created by the builder itself during the
/// launch process, hence the database type is not part of the [`NodeTypesWithDBAdapter`] trait and
/// the node's components, that depend on the database, are configured separately. In order to have
/// a nice trait that encapsulates the entire node the
