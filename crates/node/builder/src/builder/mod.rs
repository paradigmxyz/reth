...
/// [`NodeTypesWithDBAdapter`], these include the node's primitive types and the node's engine types.
///
...
/// ## Internals
///
/// The node builder is fully type safe, it uses the [`NodeTypesWithDBAdapter`] trait to enforce that
...
/// process, hence the database type is not part of the [`NodeTypesWithDBAdapter`] trait and the node's
