...
impl<Http: HttpClient + Clone> EraClient<Http> {
    ...
    async fn verify_checksum(&self, number: u64, actual_checksum: Vec<u8>) -> eyre::Result<bool> {
        Ok(actual_checksum == self.expected_checksum(number).await?)
    }

    async fn assert_checksum(&self, number: u64, actual_checksum: Vec<u8>) -> eyre::Result<()> {
    ...
    async fn expected_checksum(&self, number: u64) -> eyre::Result<Vec<u8>> {
