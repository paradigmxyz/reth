...
use crate::{
    ...
    interop::{is_stale, is_valid, MaybeInteropTransaction},
            ...
            if !to_revalidate.is_empty() {
                ...
                while let Some((tx, check)) = checks_stream.next().await {
                    if let Some(Err(err)) = check {
                        ...
                        tx.transaction.set_interop(timestamp + TRANSACTION_VALIDITY_WINDOW)
                    }
                }
            }
            if !to_remove.is_empty() {
