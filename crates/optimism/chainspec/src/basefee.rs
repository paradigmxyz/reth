...
/// If the next block base fee is less than the minimum base fee, the minimum base fee is returned.
///
    ...
    H: BlockHeader,
{
    ...
    let mut next_base_fee = parent.next_block_base_fee(base_fee_params).unwrap_or_default();

    if next_base_fee < min_base_fee {
        next_base_fee = min_base_fee;
    }
