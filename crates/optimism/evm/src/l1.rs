...
pub fn extract_l1_info<B: BlockBody>(body: &B) -> Result<L1BlockInfo, OpBlockExecutionError> {
    let l1_info_tx =
            body.transactions().first().ok_or_else(|| L1BlockInfoError::MissingInfoTransaction.into())?;
...
pub fn extract_l1_info_from_tx<T: Transaction>(
    ...
    if l1_info_tx_data.len() < 4 {
        return Err(L1BlockInfoError::InvalidCalldata.into())
    }
...
pub fn parse_l1_info_tx_bedrock(data: &[u8]) -> Result<L1BlockInfo, OpBlockExecutionError> {
    ...
    if data.len() != 256 {
        return Err(L1BlockInfoError::UnexpectedCalldataLength { 
            expected: 256,
            actual: data.len(),
        }.into())
    }
    ...
    let l1_base_fee = U256::try_from_be_slice(&data[64..96]).ok_or_else(|| {
        OpBlockExecutionError::L1BlockInfoError(L1BlockInfoError::ConversionFailed("l1 base fee").into())
    })?;

    let l1_fee_overhead = U256::try_from_be_slice(&data[192..224]).ok_or_else(|| {
        OpBlockExecutionError::L1BlockInfoError(L1BlockInfoError::ConversionFailed("l1 fee overhead").into())
    })?;
    let l1_fee_scalar = U256::try_from_be_slice(&data[224..256]).ok_or_else(|| {
        OpBlockExecutionError::L1BlockInfoError(L1BlockInfoError::ConversionFailed("l1 fee scalar").into())
    })?;
...
pub fn parse_l1_info_tx_ecotone(data: &[u8]) -> Result<L1BlockInfo, OpBlockExecutionError> {
    ...
    let l1_base_fee_scalar = U256::try_from_be_slice(&data[..4]).ok_or_else(|| {
        OpBlockExecutionError::L1BlockInfoError(L1BlockInfoError::ConversionFailed("l1 base fee scalar").into())
    })?;
    let l1_blob_base_fee_scalar = U256::try_from_be_slice(&data[4..8]).ok_or_else(|| {
        OpBlockExecutionError::L1BlockInfoError(L1BlockInfoError::ConversionFailed("l1 blob base fee scalar").into())
    })?;
    ...
    let l1_blob_base_fee = U256::try_from_be_slice(&data[64..96]).ok_or_else(|| {
        OpBlockExecutionError::L1BlockInfoError(L1BlockInfoError::ConversionFailed("l1 blob base fee").into())
    })?;
...
pub fn parse_l1_info_tx_isthmus(data: &[u8]) -> Result<L1BlockInfo, OpBlockExecutionError> {
    ...
    let l1_base_fee_scalar = U256::try_from_be_slice(&data[..4]).ok_or_else(|| {
        L1BlockInfoError::ConversionFailed("l1 base fee scalar").into()

     })?;
    let l1_blob_base_fee_scalar = U256::try_from_be_slice(&data[4..8]).ok_or_else(|| {
         L1BlockInfoError::ConversionFailed("l1 blob base fee scalar").into()

     })?;
    let l1_base_fee = U256::try_from_be_slice(&data[32..64]).ok_or_else(|| {
        L1BlockInfoError::ConversionFailed("l1 base fee").into()

    })?;

    let l1_blob_base_fee = U256::try_from_be_slice(&data[64..96]).ok_or_else(|| {
        L1BlockInfoError::ConversionFailed("l1 blob base fee").into()

     })?;
    let operator_fee_scalar = U256::try_from_be_slice(&data[160..164]).ok_or_else(|| {
        L1BlockInfoError::ConversionFailed("operator fee scalar").into()

     })?;
    let operator_fee_constant = U256::try_from_be_slice(&data[164..172]).ok_or_else(|| {
        L1BlockInfoError::ConversionFailed("operator fee constant").into()
...
impl RethL1BlockInfo for L1BlockInfo {
    fn l1_tx_data_fee(
        ...
        let spec_id = if chain_spec.is_fjord_active_at_timestamp(timestamp) {
            ...
            return Err(L1BlockInfoError::HardforksNotActive.into())
        };
