...
use reth_primitives::revm_primitives::{InvalidTransaction, OpInvalidTransaction};
    ...
    fn try_from(err: InvalidTransaction) -> Result<Self, Self::Error> {
        match err {
            InvalidTransaction::OptimismError(err) => match err {
                OpInvalidTransaction::DepositSystemTxPostRegolith => {
                    Ok(Self::DepositSystemTxPostRegolith)
                }
                OpInvalidTransaction::HaltedDepositPostRegolith => {
