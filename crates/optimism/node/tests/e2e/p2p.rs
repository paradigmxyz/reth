...
async fn can_sync() -> eyre::Result<()> {
    ...
    let canonical_chain = canonical_payload_chain
        .iter()
        .map(|p| p.block().hash())
        .collect::<Vec<_>>();
    ...
    third_node
        .update_forkchoice(
            canonical_chain[tip_index - 10],
            canonical_chain[tip_index - 10],
        )
        .await?;
    third_node
        .wait_block((tip - 10) as u64, canonical_chain[tip_index - 10], true)
        .await?;
    // Trigger live sync to block 90
    third_node
        .update_optimistic_forkchoice(canonical_chain[tip_index])
        .await?;
    third_node
        .wait_block(tip as u64, canonical_chain[tip_index], false)
        .await?;
    ...
    let side_chain = side_payload_chain
        .iter()
        .map(|p| p.block().hash())
        .collect::<Vec<_>>();
    ...
    let _ = third_node
        .submit_payload(side_payload_chain[0].clone())
        .await;

    // It will issue a pipeline reorg to 88a, and then make 89b canonical AND finalized.
    third_node
        .update_forkchoice(side_chain[0], side_chain[0])
        .await?;
