...
//         &self,
//     ) -> impl Iterator<Item = (&Address, &<B::Body as
// reth_primitives_traits::BlockBody>::Transaction)>
//            + '_ {
...
//         self,
//     ) -> impl Iterator<Item = RecoveredTx<<B::Body as
// reth_primitives_traits::BlockBody>::Transaction>>     where
...
//     ///
//     /// If the number of senders does not match the number of transactions in the block, this
// falls     /// back to manually recovery, but _without ensuring that the signature has a low `s`
// value_.     /// See also [`TransactionSigned::recover_signer_unchecked`]
//     ///
...
//             generators::rng().fill(bytes.as_mut_slice());
//             let data = Data {
//                 block: SealedBlock::arbitrary(&mut
// arbitrary::Unstructured::new(&bytes)).unwrap(),             };
//
//             let encoded = bincode::serialize(&data).unwrap();
//             let decoded: Data = bincode::deserialize(&encoded).unwrap();
//             assert_eq!(decoded, data);
...
//             generators::rng().fill(bytes.as_mut_slice());
//             let data = Data {
//                 block: SealedBlockWithSenders::arbitrary(&mut
// arbitrary::Unstructured::new(&bytes))                     .unwrap(),
//             };
//
//             let encoded = bincode::serialize(&data).unwrap();
