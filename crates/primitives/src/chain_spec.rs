...
pub trait ChainSpec: 'static + Debug + Sync + Send + Unpin {
    ...
    fn all_forks(&self) -> Vec<Hardfork>;
    ...
    fn fork_filter(&self, fork: Hardfork) -> ForkFilter {
        let all_forks = self.all_forks();
        let future_forks: Vec<BlockNumber> = all_forks
            .iter()
            .filter(|f| f.fork_block() > fork.fork_block())
            .map(|f| f.fork_block())
            .collect();

        // this data structure is not chain-agnostic, so we can pass in the constant mainnet
        // genesis
        ForkFilter::new(fork.fork_block(), self.genesis_hash(), future_forks)
    }
}
...
impl MainnetSpec {
    pub const Frontier: Hardfork = Hardfork::new(0, 1150000, [0xfc, 0x64, 0xec, 0x04]);
    // TODO: Add all other forks
}
...
impl ChainSpec for MainnetSpec {
    ...
    fn all_forks(&self) -> Vec<Hardfork> {
        vec![MainnetSpec::Frontier] // TODO: Add all forks
    }
}
