...
//     /// proof.
//     ///
//     /// Returns [BlobTransactionValidationError::InvalidProof] if any blob KZG proof in the
// response     /// fails to verify, or if the versioned hashes in the transaction do not match the
// actual     /// commitment versioned hashes.
//     #[cfg(feature = "c-kzg")]
...
//     pub(crate) fn payload_len_with_signature_without_header(&self, signature: &Signature) ->
// usize {         let payload_length = self.fields_len() + signature.payload_len();
...
//     /// `tx_type || rlp(chain_id, nonce, max_priority_fee_per_gas, max_fee_per_gas, gas_limit,
// to,     /// value, input, access_list, max_fee_per_blob_gas, blob_versioned_hashes)`
//     ///
