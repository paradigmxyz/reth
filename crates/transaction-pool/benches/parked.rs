...
fn create_pool_with_txs(
    ...
    for sender_idx in 0..sender_count {
        let sender_addr = format!("0x{sender_idx:040x}").parse().unwrap();
                ...
                b.iter_batched(
                    || {
                        ...
                        let new_tx = f.validated_arc(
                            MockTransaction::eip1559().inc_price().rng_hash().with_nonce(999999),
                        );
                        (pool, new_tx)
                    },
                    |(mut pool, tx)| {
...
fn bench_truncate_pool(c: &mut Criterion) {
    ...
    for &pool_size in &[1000, 5000, 10000] {
        for &sender_count in &[10, 50, 100] {
            ...
            group.bench_with_input(
                BenchmarkId::new(format!("{pool_size}txs_{sender_count}senders"), pool_size),
                ...
                |b, &(pool_size, sender_count)| {
                    b.iter_batched(
                        || {
                            ...
                            let limit =
                                SubPoolLimit { max_txs: pool_size / 2, max_size: usize::MAX };
                            (pool, limit)
                        },
                        |(mut pool, limit)| black_box(pool.truncate_pool(limit)),
                        criterion::BatchSize::SmallInput,
                    );
                },
            );
...
fn bench_scalability(c: &mut Criterion) {
    ...
    for &ratio in &[1, 10, 50, 100, 500] {
        ...
        group.throughput(Throughput::Elements(pool_size as u64));
        group.bench_with_input(
            BenchmarkId::new(format!("{ratio}txs_per_sender"), ratio),
            &(pool_size, sender_count),
            |b, &(pool_size, sender_count)| {
                b.iter(|| {
                    let (pool, _) = create_pool_with_txs(pool_size, sender_count);
