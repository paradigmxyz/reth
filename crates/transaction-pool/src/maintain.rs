...
use reth_primitives::{
    Address, BlockHash, BlockNumber, BlockNumberOrTag, PooledTransactionsElementEcRecovered,
    ...
    P: TransactionPool,
{
    ...
    let local_transactions = local_transactions
        .into_iter()
        .map(|tx| Into::<TransactionSignedEcRecovered>::into(tx.as_ref()).into_signed())
        .collect::<Vec<_>>();
