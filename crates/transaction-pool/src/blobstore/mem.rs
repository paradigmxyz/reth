...
impl BlobStore for InMemoryBlobStore {
    ...
    fn get_by_versioned_hashes_v1(
        ...
        for (_tx_hash, blob_sidecar) in self.inner.store.read().iter() {
            if let BlobTransactionSidecarVariant::Eip4844(sidecar) = blob_sidecar.as_ref() {
                for (hash_idx, match_result) in sidecar.match_versioned_hashes(versioned_hashes) {
    ...
    fn get_by_versioned_hashes_v2(
        ...
        let mut result = vec![None; versioned_hashes.len()];
        for (_tx_hash, blob_sidecar) in self.inner.store.read().iter() {
            if let BlobTransactionSidecarVariant::Eip7594(sidecar) = blob_sidecar.as_ref() {
                for (hash_idx, match_result) in sidecar.match_versioned_hashes(versioned_hashes) {
                    result[hash_idx] = Some(match_result);
                }
