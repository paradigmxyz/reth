...
use crate::{
    pool::events::{TransactionEvent, TransactionEventType},
...
/// A Stream that receives [TransactionEventType] for _all_ transaction.
#[derive(Debug)]
#[must_use = "streams do nothing unless polled"]
pub struct AllTransactionsEvents {
    pub(crate) events: UnboundedReceiver<TransactionEvent>,
}
...
pub(crate) struct PoolEventBroadcast {
    ...
    all_events_broadcaster: PoolEventBroadcaster,
...
impl PoolEventBroadcast {
    ...
    fn broadcast_event(&mut self, hash: &TxHash, event: TransactionEventType) {
    ...
    pub(crate) fn pending(&mut self, tx: &TxHash, replaced: Option<&TxHash>) {
        self.broadcast_event(tx, TransactionEventType::Pending);

        if let Some(replaced) = replaced {
            // notify listeners that this transaction was replaced
            self.broadcast_event(replaced, TransactionEventType::Replaced(*tx));
        }
    }

    /// Notify listeners about a transaction that was added to the queued pool.
    pub(crate) fn queued(&mut self, tx: &TxHash) {
        self.broadcast_event(tx, TransactionEventType::Queued);
    }
...
impl PoolEventBroadcaster {
    ...
    fn broadcast(&mut self, tx_hash: TxHash, event: TransactionEventType) {
        self.senders
            .retain(|sender| sender.send(TransactionEvent::new(tx_hash, event.clone())).is_ok())
    }
}
