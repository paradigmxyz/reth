...
use crate::{BlockBody, FullBlock, FullReceipt, FullSignedTx, FullTxType, MaybeSerialize};

/// Configures all the primitive types of the node.
pub trait NodePrimitives: Send + Sync + Unpin + Clone + Default + fmt::Debug + 'static {
    /// Block primitive.
    type Block: Send + Sync + Unpin + Clone + Default + fmt::Debug + MaybeSerialize + 'static;
