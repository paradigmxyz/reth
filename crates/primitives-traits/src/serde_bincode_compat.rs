...
//
// /// A type that can be used to satisfy bincode compatibility requirements.
// ///
// /// Caution: This panics when trying to convert the target type and should only be used to
// satisfy trait requirements #[derive(Debug, Clone, Default, Copy, PartialEq, Eq, serde::Serialize,
// serde::Deserialize)] pub struct NoBincodeCompat<T>(pub PhantomData<T>);
//
// impl<T> From<&T> for NoBincodeCompat<T> {
//     fn from(_: &T) -> Self {
//         NoBincodeCompat(PhantomData::default())
//     }
// }
//
// impl<T> From<NoBincodeCompat<T>> for T {
//     fn from(_: NoBincodeCompat<T>) -> Self {
//         panic!("NoBincodeCompat should not be converted into the target type")
//     }
// }
//
// mod block_bincode {
