...
#[derive(Debug)]
pub struct SnapBootstrap<'a, C, F, X> {
...
impl<'a, C, F, X> SnapBootstrap<'a, C, F, X> {
    /// Creates a session without starting network or database work.
    pub fn new(client: &'a C, factory: &'a F, context: X, runtime: Runtime) -> Self {
        Self {
            client,
            factory,
        ...
        F::ProviderRW: SnapSyncProvider,
    {
        let pivot = {
            let provider = self.factory.database_provider_ro().map_err(db_error)?;
            self.policy.select(&provider, head, None)?
        };
        let Some(pivot) = pivot else { return Ok(None) };
        let pivot = SnapDownloadProgress::new(
            pivot.target().number,
            pivot.target().hash,
            pivot.state_root(),
        );
    ...
    async fn rebuild_trie(&self, generation: SnapDownloadProgress) -> Result<(), SnapSyncError>
    where
    ...
    fn advance_target(&self, generation: SnapDownloadProgress, head: u64) -> Option<u64> {
        self.policy.pivot_block(head, None).filter(|target| *target > generation.target_block)
    }

    // Downloading ranges are re-anchored only once the pivot has fallen far enough behind.
    fn pending_advance(&self, generation: SnapDownloadProgress, head: u64) -> Option<u64> {
        self.policy
            .needs_advance(generation.generation(), head)
...
pub enum SnapSyncOutcome {
    ...
    Stalled {
        ...
        generation: Option<SnapDownloadProgress>,
    },
}
...
mod tests {
    ...
    async fn assembles_downloads_catch_up_and_trie_in_one_session() {
        ...
        let error = SnapBootstrap::new(&client, &factory, context, Runtime::test())
            .with_policy(policy())
            .run()
            .await
            .unwrap_err();
    ...
    async fn generation_outside_the_bal_window_restarts_on_a_fresh_pivot() {
        ...
        let outcome = SnapBootstrap::new(&client, &factory, context.clone(), Runtime::test())
            .with_policy(policy().with_history(1))
            .run()
            .await
            .unwrap();
    ...
    async fn restarts_on_the_canonical_pivot(phase: SnapPhase) {
        ...
        let mut orphaned =
            SnapDownloadProgress::new(2, B256::repeat_byte(0xee), B256::repeat_byte(0xdd));
        ...
        assert_eq!(
            generation,
            SnapDownloadProgress::new(2, blocks[2].hash_slow(), blocks[2].state_root)
        );
        assert_eq!(
