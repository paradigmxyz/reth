...
mod tests {
    ...
    async fn test_prune() {
        ...
        provider_rw.append_block(genesis, None, None).unwrap();
        provider_rw.append_block(block.clone(), None, None).unwrap();
        ...
        for block_number in 2..=tip {
            ...
            head = nblock.hash;
            provider_rw.append_block(nblock, None, None).unwrap();
        }
