...
impl<DB: Database, D: BodyDownloader, C: Consensus> Stage<DB> for BodyStage<D, C> {
    ...
    async fn execute(
        ...
        // Get base tx id as last cumulative tx count
        let key = db.get_block_numhash(previous_block)?;
        let mut base_tx_id = db.get_tx_count(key)?;
            ...
            bodies_stream.try_next().await.map_err(|err| StageError::Internal(err.into()))?
        {
            ...
            cum_tx_cursor.append(key, base_tx_id + block.body.len() as u64)?;
...
mod tests {
    ...
    mod test_utils {
        ...
        use reth_interfaces::{
            db::{
                models::{NumTransactions, StoredBlockOmmers},
                ...
                if let Some(progress) = blocks.first() {
                    self.db.commit(|tx| {
                        ...
                        tx.put::<tables::CumulativeTxCount>(
                            (progress.number, progress.hash()).into(),
                            last_count + progress.body.len() as u64,
                        )
                    })?;
                }
        ...
        impl BodyTestRunner {
            ...
            pub(crate) fn validate_db_blocks(
                ...
                self.db.query(|tx| {
                    let mut block_body_cursor = tx.cursor::<tables::BlockOmmers>()?;
                    ...
                    //     "We wrote a block body outside of our synced range. Found block with
                    // number {}, highest block according to stage is {}",
                    //     key.number(), highest_block
                    // );
