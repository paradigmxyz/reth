...
impl<DB: Database> Stage<DB> for SendersStage {
    ...
    async fn execute(
        ...
        let start_hash = tx
            ...
            .ok_or(DatabaseIntegrityError::CanonicalHash { number: start_block })?;
        ...
        while let Some((key, body)) = body_walker.next().transpose()? {
            ...
            let transactions = tx_cursor
                .walk(cum_tx_count - (body.tx_amount as u64) - 1)?
    ...
    async fn unwind(
        ...
        let unwind_hash = tx
            ...
            .ok_or(DatabaseIntegrityError::CanonicalHeader { number: input.unwind_to })?;
