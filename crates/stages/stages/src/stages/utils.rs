...
use reth_provider::{
    providers::StaticFileProvider, BlockReader, DBProvider, ProviderError, StaticFileProviderFactory,
};
    ...
    for (index, element) in collector.iter()?.enumerate() {
        ...
        if current_partial != partial_key {
            ...
            if !append_only {
                ...
                if let Some(existing_list) = rocksdb.get::<reth_db_api::tables::StoragesHistory>(key)? {
    ...
    for (index, element) in collector.iter()?.enumerate() {
        ...
        if current_partial != partial_key {
            ...
            if !append_only {
                ...
                if let Some(existing_list) = rocksdb.get::<reth_db_api::tables::AccountsHistory>(key)? {
                    current_list.extend(existing_list.iter());
                }
            }
        }
