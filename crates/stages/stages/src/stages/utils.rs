    ...
    for (idx, entry) in changeset_cursor.walk_range(range)?.enumerate() {
        ...
        if idx > 0 && idx.is_multiple_of(interval) && total_changesets > 1000 {
            info!(
                ...
                progress = %PercentProgress::<4>::new(idx, total_changesets),
                "Collecting indices"
            );
        }
...
/// Displays an index progress percentage without allocating a temporary string.
struct PercentProgress<const PRECISION: usize> {
    index: usize,
    total: usize,
}

impl<const PRECISION: usize> PercentProgress<PRECISION> {
    const fn new(index: usize, total: usize) -> Self {
        Self { index, total }
    }
}

impl<const PRECISION: usize> Display for PercentProgress<PRECISION> {
        ...
        + DbCursorRO<reth_db_api::tables::AccountsHistory>,
{
    ...
    for (index, element) in collector.iter()?.enumerate() {
        ...
        if index > 0 && index.is_multiple_of(interval) && total_entries > 10 {
            info!(
                target: "sync::stages::index_history",
                progress = %PercentProgress::<2>::new(index, total_entries),
                "Writing indices"
            );
        }
