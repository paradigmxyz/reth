...
pub async fn fetch(
    ...
    while !metadata.is_done() {
        ...
        while let Some(worker_msg) = orchestrator_rx.recv().await {
            ...
            if let Some(worker) = workers.get(&available_worker) {
...
mod tests {
    ...
    async fn test_download() {
        ...
        let err = fetch(filename, target_dir, url, 4, Some(b3sum))
    .await
    .expect_err("expected EmptyContentLength error");

    assert!(matches!(err, DownloaderError::EmptyContentLength { .. }));
