...
impl Metadata {
    ...
    /// Returns a list of all chunks with their remaining number of bytes to be downloaded.
    pub(crate) fn needed_ranges(&self) -> Vec<(usize, Option<(usize, usize)>)> {
        self.chunks
            .iter()
            .enumerate()
    ...
    pub(crate) fn commit(&self) {
        dbg!(&self.path);
        reth_fs_util::atomic_write_file(&self.path, |file| bincode::serialize_into(file, &self))
            .unwrap()
    }
