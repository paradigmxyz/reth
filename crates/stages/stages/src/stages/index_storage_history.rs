        ...
        + reth_provider::NodePrimitivesProvider,
{
    ...
    /// Unwind the stage.
    ///
    /// TODO: Add `RocksDB` support for unwind. Currently only unwinds MDBX data.
    /// When `storages_history_in_rocksdb` is enabled, this should also remove
    /// the corresponding entries from `RocksDB`.
    fn unwind(
        &mut self,
        provider: &Provider,
        input: UnwindInput,
