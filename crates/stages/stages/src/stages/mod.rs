...
mod tests {
    ...
    async fn test_prune() {
        ...
        provider_rw.insert_historical_block(genesis.try_with_senders().unwrap()).unwrap();
        provider_rw.insert_historical_block(block.clone().try_with_senders().unwrap()).unwrap();
        ...
        for block_number in 2..=tip {
            ...
            head = nblock.hash();
            provider_rw.insert_historical_block(nblock.try_with_senders().unwrap()).unwrap();
        }
        provider_rw
