...
impl CachedStateMetrics {
    ...
    /// Increments account cache hits counter.
    pub fn increment_account_hits(&self, value: f64) {
        self.account_cache_hits.increment(value);
    }

    /// Increments account cache misses counter.
    pub fn increment_account_misses(&self, value: f64) {
        self.account_cache_misses.increment(value);
    }

    /// Increments storage cache hits counter.
    pub fn increment_storage_hits(&self, value: f64) {
        self.storage_cache_hits.increment(value);
    }

    /// Increments storage cache misses counter.
    pub fn increment_storage_misses(&self, value: f64) {
        self.storage_cache_misses.increment(value);
    }

    /// Increments code cache hits counter.
    pub fn increment_code_hits(&self, value: f64) {
        self.code_cache_hits.increment(value);
    }

    /// Increments code cache misses counter.
    pub fn increment_code_misses(&self, value: f64) {
        self.code_cache_misses.increment(value);
    }
...
impl ExecutionCache {
    ...
    /// Gets an account from cache without inserting on miss.
    pub fn get_account(&self, address: &Address) -> Option<Option<Account>> {
        self.0.account_cache.get(address)
    }

    /// Gets a storage value from cache without inserting on miss.
    pub fn get_storage(&self, address: &Address, key: &StorageKey) -> Option<StorageValue> {
        self.0.storage_cache.get(&(*address, *key))
    }

    /// Gets code from cache without inserting on miss.
    pub fn get_code(&self, hash: &B256) -> Option<Option<Bytecode>> {
        self.0.code_cache.get(hash)
    }
