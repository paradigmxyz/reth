...
#[tokio::test]
async fn test_forkchoice_updated_refactor_sanity_check() {
    reth_tracing::init_test_tracing();
    let chain_spec = MAINNET.clone();
    let mut test_harness = TestHarness::new(chain_spec.clone());
    ...
    let head_block = blocks.last().unwrap().recovered_block();

    // Test 1: FCU with head already canonical
    let (tx, rx) = oneshot::channel();
    test_harness
        .tree
        .on_engine_message(FromEngine::Request(
            BeaconEngineMessage::ForkchoiceUpdated {
                state: ForkchoiceState {
                    ...
                    safe_block_hash: head_block.hash(),
                    finalized_block_hash: head_block.hash(),
                },
                payload_attrs: None,
                tx,
                version: EngineApiMessageVersion::default(),
    ...
    // Test 2: FCU with new head that requires chain update
    let new_block = test_block_builder.generate_random_block(5, head_block.hash());
    test_harness.tree.state.tree_state.insert_executed(ExecutedBlockWithTrieUpdates {
        block: ExecutedBlock {
            recovered_block: Arc::new(new_block.clone()),
            execution_output: Arc::new(ExecutionOutcome::default()),
            hashed_state: Arc::new(HashedPostState::default()),
        },
        trie: ExecutedTrieUpdates::empty(),
    ...
    test_harness
        ...
        .on_engine_message(FromEngine::Request(
            BeaconEngineMessage::ForkchoiceUpdated {
                state: ForkchoiceState {
                    head_block_hash: new_block.hash(),
                    safe_block_hash: new_block.hash(),
                    finalized_block_hash: new_block.hash(),
                },
                payload_attrs: None,
                tx,
                version: EngineApiMessageVersion::default(),
    ...
    let response = rx.await.unwrap().unwrap().await.unwrap();
    assert!(response.payload_status.is_valid(), "FCU with new head should be valid");

    // Test 3: FCU with missing block
        ...
        .on_engine_message(FromEngine::Request(
            BeaconEngineMessage::ForkchoiceUpdated {
                state: ForkchoiceState {
                    ...
                    safe_block_hash: missing_block_hash,
                    finalized_block_hash: missing_block_hash,
                },
                payload_attrs: None,
                tx,
                version: EngineApiMessageVersion::default(),
    ...
    let response = rx.await.unwrap().unwrap().await.unwrap();
    assert!(response.payload_status.is_syncing(), "FCU with missing block should be syncing");

    // Test 4: FCU with payload attributes
    let (tx, rx) = oneshot::channel();
    test_harness
        .tree
        .on_engine_message(FromEngine::Request(
    ...
    let response = rx.await.unwrap().unwrap().await.unwrap();
    // Should be valid but may have payload ID if attributes are valid
    assert!(response.payload_status.is_valid(), "FCU with payload attributes should be valid");
}
