...
async fn test_tree_persist_blocks() {
    ...
    if let PersistenceAction::SaveBlocks(input, _) = received_action {
        let expected_new_db_tip = blocks
            .last()
            .expect("at least one block")
            .recovered_block()
            .number
            .saturating_sub(tree_config.memory_block_buffer_target());
        let expected_persist_len = expected_new_db_tip as usize;
        assert_eq!(input.blocks().len(), expected_persist_len);
        assert_eq!(input.blocks(), &blocks[..expected_persist_len]);
        assert_eq!(input.prev_db_tip(), 0);
        assert_eq!(input.prev_partial_state_trie(), 0);
        assert_eq!(input.new_db_tip(), expected_new_db_tip);
        assert_eq!(input.new_partial_state_trie(), expected_new_db_tip);
    } else {
...
async fn test_tree_state_on_new_head_reorg() {
    ...
    //
    // The tree's canonical head is still block 3, so retaining one block targets block 2.
    ...
    assert_eq!(saved_blocks.blocks(), &blocks[..2]);
    ...
    // The same action remains in progress until completion is polled.
    let current_action = test_harness.tree.persistence_state.current_action().cloned();
    assert_eq!(
        current_action,
        Some(CurrentPersistenceAction::SavingBlocks {
...
async fn test_get_save_blocks_input() {
    ...
    assert_eq!(input.blocks().len(), expected_blocks_to_persist_length);
    for (i, item) in input.blocks().iter().enumerate().take(expected_blocks_to_persist_length) {
    ...
    assert!(!input.blocks().iter().any(|b| b.recovered_block().hash() == fork_block_hash));

    // check that the original block 4 is still included
    assert!(input.blocks().iter().any(|b| b.recovered_block().number == 4 &&
    ...
    assert_eq!(
        ...
        Some(CurrentPersistenceAction::SavingBlocks { highest: input.last_block().unwrap() })
    );
}
...
fn test_get_save_blocks_input_with_state_masking_blocks() {
    ...
    assert_eq!(
        input.blocks().iter().map(|block| block.recovered_block().number()).collect::<Vec<_>>(),
        vec![2, 3, 4, 5]
    );
    assert_eq!(
        input
...
#[test]
fn test_get_save_blocks_input_preserves_mainline_target() {
    let chain_spec = MAINNET.clone();
    let mut test_harness = TestHarness::new(chain_spec);
    let mut test_block_builder = TestBlockBuilder::eth();
...
#[test]
fn test_get_save_blocks_input_steady_state_and_head_flush() {
    let chain_spec = MAINNET.clone();
    let mut test_harness = TestHarness::new(chain_spec);
    let mut test_block_builder = TestBlockBuilder::eth();
    ...
    assert_eq!(input.new_partial_state_trie(), 23);
    assert_eq!(input.new_db_tip(), 29);
    assert_eq!(
        input
            .state_trie_blocks()
            .iter()
    ...
    let head_input = test_harness.tree.get_save_blocks_input(PersistTarget::Head).unwrap();
    assert_eq!(head_input.new_db_tip(), 29);
    assert_eq!(head_input.new_partial_state_trie(), 29);
    assert_eq!(
        head_input
            .state_trie_blocks()
            .iter()
    ...
    assert!(head_input.state_trie_masking_blocks().is_empty());
}
...
fn test_on_persistence_complete_retains_blocks_above_partial_state_trie() {
    ...
    // The retained suffix is the input to the next cycle: block 4 is state/trie catchup, block 6
    // is fresh block data, and blocks 5-6 form the new mask.
    test_harness.tree.config =
        TreeConfig::default().with_persistence_threshold(4).with_num_state_masking_blocks(2);
    let next = test_harness.tree.get_save_blocks_input(PersistTarget::Threshold).unwrap();
    assert_eq!(next.prev_partial_state_trie(), 3);
    assert_eq!(next.prev_db_tip(), 5);
    assert_eq!(next.new_partial_state_trie(), 4);
    assert_eq!(next.new_db_tip(), 6);
    assert_eq!(
        next.blocks().iter().map(|block| block.recovered_block().number()).collect::<Vec<_>>(),
        vec![4, 5, 6]
    );
    assert_eq!(
        next.state_trie_blocks()
            .iter()
            .map(|block| block.recovered_block().number())
            .collect::<Vec<_>>(),
        vec![4]
    ...
    assert_eq!(
        next.state_trie_masking_blocks()
            .iter()
            .map(|block| block.recovered_block().number())
            .collect::<Vec<_>>(),
        vec![5, 6]
