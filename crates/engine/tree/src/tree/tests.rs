...
fn on_new_persisted_block_queues_sparse_trie_prune_request() {
    ...
    let prune = test_harness.tree.pending_sparse_trie_prune.take().unwrap();
    assert_eq!(prune.anchor_hash, blocks[0].recovered_block().hash());
    let (_, prune_blocks) = test_harness
        .tree
        .state
        .tree_state
        .blocks_by_hash(blocks[2].recovered_block().hash())
        .unwrap();
    let retained_paths = prune.with_in_memory_blocks(prune_blocks).unwrap().retained_paths.freeze();
...
fn on_new_persisted_block_queues_sparse_trie_prune_when_changed_paths_unknown() {
    ...
    let (_, prune_blocks) = test_harness
        ...
        .blocks_by_hash(blocks[2].recovered_block().hash())
        .unwrap();
    assert!(prune.with_in_memory_blocks(prune_blocks).is_none());
}
