    ...
    V: EngineValidator<T> + WaitForCaches,
{
        ...
        P: BlockReader + StateProviderFactory + StateReader + Clone,
    {
        if let Some((historical, blocks)) = self.state.tree_state.blocks_by_hash(hash) {
            trace!(target: "engine::tree", %hash, %historical, "found canonical state for block in memory, creating provider builder");
        ...
        if let Some(header) = self.provider.header(hash)? {
            trace!(target: "engine::tree", %hash, number = %header.number(), "found canonical state for block in database, creating provider builder");
