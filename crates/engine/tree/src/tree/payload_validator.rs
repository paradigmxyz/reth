    ...
    Evm: ConfigureEvm<Primitives = N> + 'static,
{
    ...
    fn state_provider_builder(
        ...
        if let Some((historical, blocks)) = state.tree_state.blocks_by_hash(hash) {
            trace!(target: "engine::tree::payload_validator", %hash, %historical, "found canonical state for block in memory, creating provider builder");
        ...
        if let Some(header) = self.provider.header(hash)? {
            trace!(target: "engine::tree::payload_validator", %hash, number = %header.number(), "found canonical state for block in database, creating provider builder");
