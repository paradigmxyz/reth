...
//! 3. Prepare the per-block state-root task. The default strategy picks skipped, synchronous, or
//!    sparse-trie execution from [`TreeConfig`].
//! 4. Spawn the payload processor. This always prepares transaction conversion and prewarming; a
//!    streaming state-root task can provide a sink for prewarm and execution updates.
...
//! | `sparse-trie` | sparse-trie task setup | apply execution or BAL updates and compute the state root | awaited by the state-root task |
...
//! | `serial-root` | sparse trie timeout fallback | race serial state-root computation against the sparse trie task | polled by the state-root task |
    ...
    Evm: ConfigureEvm<Primitives = N> + 'static,
{
        ...
        Evm: ConfigureEngineEvm<T::ExecutionData, Primitives = N>,
    {
        ...
        let processor_options = PayloadProcessorSpawnOptions::new(parallel_bal_execution, None);
    ...
    #[instrument(
        ...
        target = "engine::tree::payload_validator",
        skip_all,
        fields(has_state_root_streams = !state_root_streams.is_empty(), parallel_bal_execution = options.parallel_bal_execution)
    )]
    fn spawn_payload_processor<T: ExecutableTxIterator<Evm>>(
        ...
        InsertBlockErrorKind,
    > {
        let PayloadProcessorSpawnOptions { parallel_bal_execution, .. } = options;
        let start = Instant::now();
