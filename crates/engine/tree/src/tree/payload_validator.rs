    ...
    Evm: ConfigureEvm<Primitives = N> + 'static,
{
    ...
    ///
    /// For small blocks (below the configured gas threshold), the sparse trie task's coordination
    /// overhead (~5-6ms for channel setup, proof workers, multiproof gathering) is disproportionate
    /// to the actual execution cost. These blocks fall back to parallel state root computation.
    ///
