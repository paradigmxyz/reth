...
//!
//! Serial execution can be overridden via [`CustomBlockExecutor`] installed on
//! [`BasicEngineValidator::with_custom_block_executor`]. Returning `None` from the hook falls back
        ...
        Evm: ConfigureEngineEvm<T::ExecutionData, Primitives = N>,
    {
        ...
        let execution_result = if parallel_bal_execution {
            ...
            match state_provider {
                Ok(state_provider) => {
                    self.execute_serial_block(state_provider, env, &input, &mut handle)
                }
        ...
        Evm: ConfigureEngineEvm<T::ExecutionData, Primitives = N>,
    {
        if let Some(custom) = &self.custom_block_executor {
            ...
            if let Some(result) = custom(custom_input)? {
                ...
                self.metrics
                    .record_block_execution_gas_bucket(result.output.result.gas_used, execution_duration);
                return Ok((
                    result.output,
                    result.senders,
        ...
        self.execute_block(state_provider, env, input, handle)
    }
...
pub type CustomBlockExecutor<N, Evm> = Arc<
    dyn Fn(CustomBlockExecutorInput<'_, Evm, N>) -> Result<Option<CustomBlockExecutionOutput<N>>, InsertBlockErrorKind>
        + Send
        + Sync
        + 'static,
>;
...
mod custom_block_executor_tests {
    ...
    fn custom_block_executor_none_falls_back() {
        ...
        let executor: CustomBlockExecutor<EthPrimitives, MockEvmConfig> =
            Arc::new(move |input| {
