...
// Simple 2-way comparison structures and functions (no database involvement)
...
#[derive(Debug, Default)]
struct SimpleTrieUpdatesDiff {
    account_nodes: HashMap<Nibbles, SimpleDiff<Option<BranchNodeCompact>>>,
    removed_nodes: HashMap<Nibbles, SimpleDiff<bool>>,
    storage_tries: HashMap<B256, SimpleStorageTrieUpdatesDiff>,
}

impl SimpleTrieUpdatesDiff {
    fn has_differences(&self) -> bool {
        !self.account_nodes.is_empty() ||
            !self.removed_nodes.is_empty() ||
            !self.storage_tries.is_empty()
    ...
    fn log_differences(self) {
        ...
        warn!(target: "engine::tree", "Found differences between task and synchronous trie updates");

        for (path, SimpleDiff { task, regular }) in &self.account_nodes {
            warn!(target: "engine::tree", ?path, ?task, ?regular, "Account trie update mismatch");
        }

        for (path, SimpleDiff { task: task_removed, regular: regular_removed }) in
            &self.removed_nodes
        {
            warn!(target: "engine::tree", ?path, ?task_removed, ?regular_removed, "Removed account trie node mismatch");
        }
...
#[derive(Debug, Default)]
struct SimpleStorageTrieUpdatesDiff {
    is_deleted: Option<SimpleDiff<bool>>,
    storage_nodes: HashMap<Nibbles, SimpleDiff<Option<BranchNodeCompact>>>,
    removed_nodes: HashMap<Nibbles, SimpleDiff<bool>>,
}

impl SimpleStorageTrieUpdatesDiff {
    fn has_differences(&self) -> bool {
        self.is_deleted.is_some() ||
            !self.storage_nodes.is_empty() ||
            !self.removed_nodes.is_empty()
    ...
    fn log_differences(&self, address: B256) {
        if let Some(SimpleDiff { task: task_deleted, regular: regular_deleted }) = self.is_deleted {
            warn!(target: "engine::tree", ?address, ?task_deleted, ?regular_deleted, "Storage trie deletion mismatch");
        }

        for (path, SimpleDiff { task, regular }) in &self.storage_nodes {
            warn!(target: "engine::tree", ?address, ?path, ?task, ?regular, "Storage trie update mismatch");
        }

        for (path, SimpleDiff { task: task_removed, regular: regular_removed }) in
            &self.removed_nodes
        {
            warn!(target: "engine::tree", ?address, ?path, ?task_removed, ?regular_removed, "Removed storage trie node mismatch");
        }
    }
}

/// Compares two branch nodes directly (no database).
fn branch_nodes_equal_simple(
    task: Option<&BranchNodeCompact>,
    regular: Option<&BranchNodeCompact>,
) -> bool {
    match (task, regular) {
...
/// Compares trie updates from state root task and synchronous state root calculation.
/// This is a simpler version that doesn't involve database lookups.
pub(super) fn compare_trie_updates_simple(task: TrieUpdates, regular: TrieUpdates) {
    let mut task = adjust_trie_updates(task);
    let mut regular = adjust_trie_updates(regular);

    let mut diff = SimpleTrieUpdatesDiff::default();
    ...
    for key in task
        ...
        .collect::<BTreeSet<_>>()
    {
        let (task_node, regular_node) =
            (task.account_nodes.remove(&key), regular.account_nodes.remove(&key));

        if !branch_nodes_equal_simple(task_node.as_ref(), regular_node.as_ref()) {
            diff.account_nodes.insert(key, SimpleDiff { task: task_node, regular: regular_node });
        }
    }
        ...
        .collect::<BTreeSet<_>>()
    {
        ...
        if task_removed != regular_removed {
            diff.removed_nodes
                .insert(key, SimpleDiff { task: task_removed, regular: regular_removed });
        }
    }
    ...
    for key in task
        ...
        .collect::<BTreeSet<_>>()
    {
        let (mut task_storage, mut regular_storage) =
            (task.storage_tries.remove(&key), regular.storage_tries.remove(&key));

        if task_storage != regular_storage {
            #[expect(clippy::or_fun_call)]
            let storage_diff = compare_storage_trie_updates_simple(
                task_storage.as_mut().unwrap_or(&mut Default::default()),
                regular_storage.as_mut().unwrap_or(&mut Default::default()),
            );
            if storage_diff.has_differences() {
                diff.storage_tries.insert(key, storage_diff);
            }
...
fn compare_storage_trie_updates_simple(
    task: &mut StorageTrieUpdates,
    regular: &mut StorageTrieUpdates,
) -> SimpleStorageTrieUpdatesDiff {
    let mut diff = SimpleStorageTrieUpdatesDiff {
        is_deleted: (task.is_deleted != regular.is_deleted)
            .then_some(SimpleDiff { task: task.is_deleted, regular: regular.is_deleted }),
        ..Default::default()
    };
    ...
    {
        let (task_node, regular_node) =
            (task.storage_nodes.remove(&key), regular.storage_nodes.remove(&key));

        if !branch_nodes_equal_simple(task_node.as_ref(), regular_node.as_ref()) {
            diff.storage_nodes.insert(key, SimpleDiff { task: task_node, regular: regular_node });
        }
    }
