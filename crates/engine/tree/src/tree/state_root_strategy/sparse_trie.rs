    ...
    S: SparseTrie + Default + Clone + 'static,
{
    ...
    fn promote_account(&mut self, addr: B256) -> bool {
        ...
        let updated_storage_root = match self.storage.get_mut(&addr) {
            ...
            // Leaves that are queued but not applied hold the account back even when none of
            // them changes a value: until they are applied the trie may still be blind, so its
            // root is not readable either.
            Some(StorageSlot::Idle(work)) if work.updated || !work.is_drained() => {
                if !work.is_drained() {
