    ...
    N: NodePrimitives,
{
    ...
    ///
    /// `parent_hash` and `parent_state_root` must identify the parent state used for execution.
    ///
        ...
        Some(
            self.spawn_state_root(
                ...
                StateRootTaskOptions {
                    parent_hash,
                    parent_state_root,
                    // Direct producers do not own `EngineApiTreeState`, which retains the
                    // reusable trie between jobs. They start from the supplied parent state.
