...
/// Converts [`EvmState`] into [`HashedPostState`].
...
/// Collects addresses and storage slots from the provided [`HashedPostState`] into
/// [`MultiProofTargets`].
fn hashed_post_state_to_proof_targets(hashed_post_state: &HashedPostState) -> MultiProofTargets {
    // No need to iterate over `hashed_post_state.storages` separately, because its keys are
    // a subset of `hashed_post_state.accounts` keys. See `evm_state_to_hashed_post_state`
    // implementation.
    hashed_post_state
        .accounts
        .keys()
        .map(|address| {
            (
                *address,
                hashed_post_state.storages.get(address).map_or_else(B256Set::default, |storage| {
                    storage.storage.keys().copied().collect::<B256Set>()
                }),
            )
        })
        .collect()
        ...
        DatabaseProviderFactory<Provider: BlockReader> + StateCommitmentProvider + Clone + 'static,
{
    ...
    fn on_state_update(&mut self, source: StateChangeSource, update: EvmState) -> u64 {
        ...
        .flatten()
        {
            ...
            let mut chunked_state_update = HashedPostState {
                ...
                storages: B256Map::with_capacity_and_hasher(chunk.len(), Default::default()),
            };
            ...
            for (address, storage_slots) in chunk {
                ...
                let storage = state_update.storages.entry(address);
                let mut chuncked_hashed_storage = HashedStorage::default();
            ...
            self.multiproof_manager.spawn_or_queue(MultiproofInput {
                config: self.config.clone(),
                source: Some(source),
                hashed_state_update: chunked_state_update,
                proof_targets: chunked_proof_targets,
                proof_sequence_number: self.proof_sequencer.next_sequence(),
                state_root_message_sender: self.tx.clone(),
            });
        }
    ...
    ///      with [`hashed_post_state_to_proof_targets`], and chunked via [`ChunkedProofTargets`].
