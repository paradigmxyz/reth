        ...
        DatabaseProviderFactory<Provider: BlockReader> + StateCommitmentProvider + Clone + 'static,
{
    ...
    fn on_state_update(&mut self, source: StateChangeSource, update: EvmState) -> u64 {
        ...
        // Chunked state updates. Each `HashedPostState` will have at most
        // `MULTIPROOF_ACCOUNTS_CHUNK_SIZE` accounts and `MULTIPROOF_STORAGES_CHUNK_SIZE`
            ...
            &hashed_state_update.accounts.into_iter().chunks(MULTIPROOF_ACCOUNTS_CHUNK_SIZE)
        {
            ...
            for (address, account) in accounts_chunk {
                ...
                // Process the rest of the storage slots and add them to the list of chunks.
                for (i, storages_chunk) in storages_iter.enumerate() {
                    state_update_chunks[i + 1].push((
                        address,
                        account,
