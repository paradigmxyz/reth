    ...
    Evm: ConfigureEvm<Primitives = N> + 'static,
{
    ...
    fn prepare_payload_builder(
        ...
        let PayloadStateRootJobContext {
            payload_processor,
            parent_state_root,
            overlay_factory,
            config,
        ...
        // Sharing the engine state-root task with the payload builder is opt-in.
        if !config.share_sparse_trie_with_payload_builder() || config.skip_state_root() {
            return Ok(None)
        }
        ...
        Ok(Some(
            payload_processor
                .spawn_state_root(
                    overlay_factory,
                    parent_state_root,
                    // Full proof workers — tx count unknown at FCU time because the
                    // block is built incrementally.
                    false,
                    config,
                    None,
                )
