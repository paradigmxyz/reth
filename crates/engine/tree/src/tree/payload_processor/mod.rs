    ...
    pub fn state_hook(&self) -> impl OnStateHook {
        ...
        move |_source: StateChangeSource, state: &EvmState| {
            if let Some(sender) = &to_multi_proof {
                // Hash by reference: only copy touched accounts and changed storage,
                // avoiding a full EvmState clone that includes untouched storage,
                // original values, and bytecode.
