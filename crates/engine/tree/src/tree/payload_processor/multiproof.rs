...
/// Converts an [`EvmState`] **by reference** into a [`HashedPostState`], avoiding a full clone
/// of the `EvmState`.
///
/// This is functionally identical to [`evm_state_to_hashed_post_state`] but copies only the
/// touched/changed data (account info + changed storage values) instead of taking ownership of
/// the entire `EvmState` (which includes untouched storage, original values, bytecode, etc.).
