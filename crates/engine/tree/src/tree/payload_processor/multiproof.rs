...
pub enum MultiProofMessage {
    ...
    ///
    /// The state is pre-hashed to [`HashedPostState`] at the sender (`state_hook`) to avoid
    /// cloning the full [`EvmState`] which includes bytecodes and `original_info`.
...
/// Converts an [`EvmState`] by reference into a [`HashedPostState`], avoiding cloning.
///
/// Only copies nonce (`u64`), balance (`U256`), and `code_hash` (`B256`) from `AccountInfo` —
/// all Copy types. Bytecode and `original_info` are not touched.
