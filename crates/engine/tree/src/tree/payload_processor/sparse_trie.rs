...
pub(super) struct SparseTrieTask<BPF, A>
where
    ...
    A: SparseTrieInterface, // Account trie implementation
{
    ...
    /// Sparse trie state which is re-used across tasks to preserve allocations.
    pub(super) trie: SparseStateTrie<A>,
    pub(super) metrics: MultiProofTaskMetrics,
    /// Blinded node provider factory.
    blinded_provider_factory: BPF,
}

impl<BPF, A> SparseTrieTask<BPF, A>
where
    BPF: BlindedProviderFactory + Send + Sync,
    BPF::AccountNodeProvider: BlindedProvider + Send + Sync,
    BPF::StorageNodeProvider: BlindedProvider + Send + Sync,
...
/// the trie updates.
#[derive(Debug)]
pub struct StateRootComputeOutcome<A> {
    /// The state root.
    pub state_root: B256,
    /// The trie updates.
...
pub(crate) fn update_sparse_trie<BPF, A>(
    trie: &mut SparseStateTrie<A>,
