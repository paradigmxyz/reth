    ...
    S: SparseTrie + Default + Clone,
{
    ...
    /// for accounts trie.
    fn promote_pending_account_update(
        &mut self,
        addr: B256,
        ...
        let trie_account = match self.account_updates.get(&addr) {
            ...
            Some(LeafUpdate::Touched) => return Ok(false),
        ...
        let (account, storage_root) = if let Some(account) = account.take() {
            // If account is Some(_) here it means it didn't have any storage updates and we can
            // fetch the storage root directly from the account trie.
    ...
    fn promote_pending_account_updates(&mut self) -> SparseTrieResult<()> {
        ...
        loop {
            ...
            if self.pending_account_updates.len() == 1 {
                let (addr, mut account) = self
                    .pending_account_updates
                    .drain()
                    .next()
                    .expect("pending account updates has one entry");
                if self.promote_pending_account_update(addr, &mut account)? {
                    num_promoted = 1;
                } else {
