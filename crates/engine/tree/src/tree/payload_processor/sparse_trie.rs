    ...
    S: SparseTrieExt + Default + Clone,
{
    ...
    fn process_updates(&mut self) -> Result<(), ProviderError> {
        ...
        for (addr, updates) in &mut self.storage_updates {
            ...
            if let Some(inner) = trie.as_revealed_mut() {
                inner
                    .update_leaves(updates, |path, min_len| {
                        let key = B256::from_slice(&path.pack());
                        let target = proof_v2::Target::new(key).with_min_len(min_len);
                        targets.storage_targets.entry(*addr).or_default().push(target);
                    })
                    .expect("update_leaves failed");
            }
        ...
        if let Some(account_trie) = self.trie.state_trie_mut() {
            account_trie
                .update_leaves(&mut self.account_updates, |path, min_len| {
                    let key = B256::from_slice(&path.pack());
                    let target = proof_v2::Target::new(key).with_min_len(min_len);
                    targets.account_targets.push(target);
                })
                .expect("update_leaves failed");
        }
