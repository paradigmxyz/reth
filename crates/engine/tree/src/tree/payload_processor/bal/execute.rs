...
use revm_primitives::AddressIndexMap;
use revm_state::bal::{AccountBal as RevmAccountBal, Bal as RevmBal};
use std::sync::Arc;
...
fn convert_alloy_to_revm_bal(bal: &AlloyBal) -> Result<Arc<RevmBal>, BalExecutionError> {
    ...
    let accounts: AddressIndexMap<RevmAccountBal> = bal
        .iter()
        .map(RevmAccountBal::clone_from_alloy)
        .collect::<Result<_, _>>()
        .map_err(|e| {
