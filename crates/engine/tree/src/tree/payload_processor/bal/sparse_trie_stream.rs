...
/// Streams a single BAL account's post-block state. Sends one combined account/storage update if
/// any of balance, nonce, code, or storage changed. Sends nothing for pure-read accounts (only
/// `storage_reads` declared).
...
mod tests {
    ...
    #[test]
    fn storage_change_sends_combined_storage_and_account_update() {
        // BAL has storage change. One update carries both storage and account, with unchanged
        // account fields falling back to snapshot values.
        let (tx, rx) = unbounded();
        ...
        let msgs = drain(&rx);
        assert_eq!(msgs.len(), 1, "expected exactly one combined update");
