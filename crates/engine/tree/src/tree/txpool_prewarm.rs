...
pub struct TxPoolPrewarmHints {
    inner: Arc<B256Map<Arc<MultiProofTargetsV2>>>,
}

impl TxPoolPrewarmHints {
    pub(crate) fn new(hints: B256Map<Arc<MultiProofTargetsV2>>) -> Self {
        Self { inner: Arc::new(hints) }
    }

    fn snapshot(hints: &B256Map<Arc<MultiProofTargetsV2>>) -> Self {
        Self::new(hints.clone())
    }

    /// Returns the predicted state-root access hint for `transaction_hash`.
    pub fn get(&self, transaction_hash: &B256) -> Option<&MultiProofTargetsV2> {
    ...
    Evm: ConfigureEvm<Primitives = N> + 'static,
{
    ...
    let mut hints = B256Map::<Arc<MultiProofTargetsV2>>::default();
...
fn prewarm_transactions<N, P, Evm>(
    ...
    generation: u64,
    publication: &TxPoolSnapshotPublication,
) -> Option<B256Map<Arc<MultiProofTargetsV2>>>
where
    N: NodePrimitives,
        ...
        for transaction in sender_transactions {
            ...
            match evm.transact(transaction.transaction.clone()) {
                Ok(result) => {
                    ...
                    if !targets.is_empty() {
                        hints.insert(transaction.hash, Arc::new(targets));
                    }
                }
                Err(err) => {
                    trace!(
...
mod tests {
    ...
    fn snapshot(
        parent_hash: B256,
        hints: B256Map<Arc<MultiProofTargetsV2>>,
    ...
    fn hint_snapshot_is_immutable() {
        ...
        let mut hints = B256Map::from_iter([(
            ...
            Arc::new(MultiProofTargetsV2 {
                account_targets: vec![B256::with_last_byte(3).into()],
                storage_targets: B256Map::default(),
            }),
        )]);

        let snapshot = TxPoolPrewarmHints::snapshot(&hints);
        hints.insert(second_hash, Arc::new(MultiProofTargetsV2::default()));
