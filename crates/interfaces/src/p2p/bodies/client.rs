...
pub type BodiesFut = Pin<
    Box<dyn Future<Output = Result<PeerRequestResult<Vec<BlockBody>>, RecvError>> + Send + Sync>,
>;
...
pub trait BodiesClient: DownloadClient {
    /// The bodies type
    type Output: Future<Output = Result<PeerRequestResult<Vec<BlockBody>>, RecvError>> + Sync + Send;
