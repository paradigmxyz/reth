...
use reth_node_core::{
    args::DatadirArgs, cli::config::{PayloadBuilderConfig, RethRpcConfig, RethTransactionPoolConfig}, dirs::{ChainPath, DataDirPath, MaybePlatformPath}, engine_api_store::EngineApiStore, events::cl::ConsensusLayerHealthEvents, exit::NodeExitFuture, init::init_genesis, node_config::NodeConfig, primitives::{kzg::KzgSettings, Head}, utils::write_peers_to_file
};
use reth_primitives::{
...
impl<DB> NodeBuilder<DB, InitState> {
    ...
    pub fn testing_node(
        ...
        let data_dir = path.unwrap_or_chain_default(self.config.chain.chain, DatadirArgs::default());
