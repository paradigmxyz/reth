...
impl<DB: Database, C: Consensus, EF: ExecutorFactory> BlockchainTree<DB, C, EF> {
    ...
    /// `N` is the `max_reorg_depth` plus the number of block hashes needed to satisfy the
    /// `BLOCKHASH` opcode in the EVM.
    pub fn connect_buffered_blocks_to_canonical_hashes(&mut self) -> Result<(), RethError> {
        let num_of_canonical_hashes =
            self.config.max_reorg_depth() + self.config.num_of_additional_canonical_block_hashes();
    ...
    pub fn find_canonical_header(&self, hash: &BlockHash) -> Result<Option<SealedHeader>, RethError> {
    ...
    pub fn is_block_hash_canonical(&self, hash: &BlockHash) -> Result<bool, RethError> {
    ...
    pub fn make_canonical(&mut self, block_hash: &BlockHash) -> Result<CanonicalOutcome, RethError> {
    ...
    fn commit_canonical(&self, chain: Chain) -> Result<(), RethError> {
    ...
    pub fn unwind(&mut self, unwind_to: BlockNumber) -> Result<(), RethError> {
    ...
    fn revert_canonical(&mut self, revert_until: BlockNumber) -> Result<Option<Chain>, RethError> {
