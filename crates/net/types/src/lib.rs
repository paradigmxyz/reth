...
//!
//! A more complete representation of a peer is the [NodeRecord] type, which includes the peer's IP
//! address, the ports where it is reachable (TCP and UDP), and the peer's public key. This is what
//! is returned from discovery v4 queries.
//!
//! The most comprehensive node record type is the Ethereum Node Record ([Enr]), which is a signed,
//! versioned record that includes the information from a [NodeRecord] along with additional
//! metadata. This is the data structure returned from discovery v5 queries.
//!
//! When we need to deserialize an identifier that could be any of these three types ([PeerId],
//! [NodeRecord], and [Enr]), we use the [AnyNode] type, which is an enum over the three types.
//! [AnyNode] is used in reth's `admin_addTrustedPeer` RPC method.
//!
//! The __final__ type is the [TrustedPeer] type, which is similar to a [NodeRecord] but may include
//! a domain name instead of a direct IP address. It includes a `resolve` method, which can be used
//! to resolve the domain name, producing a [NodeRecord]. This is useful for adding trusted peers at
//! startup, whose IP address may not be static each time the node starts. This is common in
//! orchestrated environments like Kubernetes, where there is reliable service discovery, but
//! services do not necessarily have static IPs.
//!
...
//! - [AnyNode]: An enum over [PeerId], [NodeRecord], and [Enr], useful in deserialization when the
//!   type of the node record is not known.
//! - [TrustedPeer]: A [NodeRecord] with an optional domain name, which can be resolved to a
//!   [NodeRecord]. Useful for adding trusted peers at startup, whose IP address may not be static.
