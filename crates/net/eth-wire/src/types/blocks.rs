...
use reth_primitives::{BlockBody, BlockHashOrNumber, Bytes, Header, HeadersDirection, H256};
...
/// The response to [`GetBlockBodies`], containing the already encoded block bodies that the peer
/// knows about if any were found.
#[derive(Clone, Debug, PartialEq, Eq, RlpEncodableWrapper, RlpDecodableWrapper, Default)]
#[cfg_attr(feature = "serde", derive(Serialize, Deserialize))]
pub struct RawBlockBodies(
    /// The requested block bodies, each of which should correspond to a hash in the request.
    pub Vec<Bytes>,
);
...
mod test {
    use crate::{
        types::{message::RequestPair, BlockBodies, BlockHeaders, GetBlockBodies, GetBlockHeaders},
        RawBlockBodies,
    };
    use hex_literal::hex;
    use reth_primitives::{
    ...
    #[test]
    fn decode_raw_block_bodies() {
        let data =
            hex!(""
    );

        let result = RequestPair::<BlockBodies>::decode(&mut &data[..]).unwrap();
        let bodies = result.message;
        let mut buf = Vec::new();
        bodies.encode(&mut buf);

        let decoded = BlockBodies::decode(&mut &buf[..]).unwrap();
        assert_eq!(bodies, decoded);

        let data = hex!("f851b84ff84df84ab84801f84588a2ba7e1f5201ba0288748e84a1ca197883904c6df2c2dcd79ef0fcc74b542a5fd74988465b8717f80a8b518090be6a78fdd85001ec75eb315b7d4f3eb1820102c0018080c0");
        let decoded = RawBlockBodies::decode(&mut &data[..]).unwrap();
        let decoded = BlockBodies::decode(&mut &data[..]).unwrap();
    }
}
