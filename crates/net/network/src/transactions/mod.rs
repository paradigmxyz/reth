...
impl<Pool: TransactionPool, N: NetworkPrimitives> TransactionsManager<Pool, N> {
    ...
    fn on_new_pooled_transaction_hashes(
        ...
        // Exclude pending imports, buffered eth/72 bodies, and known bad transactions before
        // counting types and applying policy. These cheap checks shrink the batch before
        ...
        announcement.retain(|tx| {
            if self.transactions_by_peers.contains_key(&tx.hash) ||
                self.buffered_blob_txs.contains_key(&tx.hash) ||
