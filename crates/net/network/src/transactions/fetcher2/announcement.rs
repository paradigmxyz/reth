...
impl PeerAnnouncements {
    ...
    fn remove(&mut self, hash: &TxHash) -> bool {
        ...
        // Keep removals O(1) until tombstones outnumber live metadata, then rebuild the FIFO in
        // one pass.
...
mod tests {
    ...
    #[test]
    fn stale_fifo_entries_do_not_change_reannouncement_order() {
        let peer_id = peer(1);
        let mut index = PeerAnnouncementIndex::default();
        ...
        assert_eq!(index.pack(peer_id, 2, 100, |_| true), vec![hash(2), hash(1)]);
        assert_eq!(index.peers[&peer_id].stale_entries, 0);
    }
