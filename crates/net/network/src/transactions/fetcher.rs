...
use super::{Peer, PooledTransactions, POOLED_TRANSACTIONS_RESPONSE_SOFT_LIMIT_BYTES};
...
`@self`: {:?}",
        ...
        if let Some(size) = self.eth68_meta.peek(&hash) {
            ...
            if next_acc_size <= POOLED_TRANSACTIONS_RESPONSE_SOFT_LIMIT_BYTES {
    ...
    pub(super) fn pack_hashes_eth68(
        ...
        if let Some(hash) = hashes.first() {
            if let Some(size) = self.eth68_meta.get(hash) {
                if *size >= POOLED_TRANSACTIONS_RESPONSE_SOFT_LIMIT_BYTES {
        ...
        hashes.retain(|&hash| match self.include_eth68_hash(&mut acc_size_response, hash) {
            ...
            false => {
                trace!(target: "net::tx",
                    ...
                    POOLED_TRANSACTIONS_RESPONSE_SOFT_LIMIT_BYTES=POOLED_TRANSACTIONS_RESPONSE_SOFT_LIMIT_BYTES,
        ...
        if acc_size_response < POOLED_TRANSACTIONS_RESPONSE_SOFT_LIMIT_BYTES {
...
mod test {
    ...
    fn pack_eth68_request_surplus_hashes() {
        ...
        let eth68_hashes_sizes = [
            POOLED_TRANSACTIONS_RESPONSE_SOFT_LIMIT_BYTES - 4,
            POOLED_TRANSACTIONS_RESPONSE_SOFT_LIMIT_BYTES, // this one will not fit
            2,                                             // this one will fit
