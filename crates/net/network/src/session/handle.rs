...
#[derive(Debug)]
pub struct ActiveSessionHandle<T: NetworkTypes> {
    /// The direction of the session
    pub(crate) direction: Direction,
    ...
    pub(crate) commands_to_session: mpsc::Sender<SessionCommand<T>>,
...
impl<T: NetworkTypes> ActiveSessionHandle<T> {
    ...
    ) -> Result<(), SendError<SessionCommand<T>>> {
...
#[derive(Debug)]
pub enum PendingSessionEvent<T: NetworkTypes> {
