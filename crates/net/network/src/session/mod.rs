...
use crate::session::active::{MAX_QUEUED_BROADCAST_ITEMS, RANGE_UPDATE_INTERVAL};
...
impl<N: NetworkPrimitives> SessionManager<N> {
    ...
    ///
    /// Applies size-based backpressure for broadcast messages: instead of relying solely on the
    /// command channel capacity, this checks the total number of buffered broadcast items in the
    /// session task. Many small messages carrying a single tx hash each are treated equivalently
    /// to one large message carrying many hashes.
    pub fn send_message(&self, peer_id: &PeerId, msg: PeerMessage<N>) {
        if let Some(session) = self.active_sessions.get(peer_id) {
            if msg.is_broadcast() && session.queued_broadcast_items() >= MAX_QUEUED_BROADCAST_ITEMS
            {
        ...
        match event {
            PendingSessionEvent::Established {
                ...
                let handle = ActiveSessionHandle {
                    ...
                    commands_to_session,
                    queued_broadcast_items: broadcast_items,
