...
/// The type of the underlying peer network connection when snap is shared.
...
mod tests {
    use super::*;
    use reth_eth_wire::{eth_snap_stream::EthSnapStreamError, EthMessage};
    ...
    const fn assert_eth_snap_stream<N, St, E>()
    where
        N: NetworkPrimitives,
        St: Stream<Item = Result<EthSnapMessage<N>, E>> + Sink<EthSnapMessage<N>>,
    {
    }

    #[test]
    const fn test_eth_stream_variants() {
        assert_eth_stream::<EthNetworkPrimitives, EthPeerConnection<EthNetworkPrimitives>>();
        assert_eth_stream::<EthNetworkPrimitives, EthSatelliteConnection<EthNetworkPrimitives>>();
        assert_eth_snap_stream::<
            EthNetworkPrimitives,
            EthSnapPeerConnection<EthNetworkPrimitives>,
            EthSnapStreamError,
        >();
        assert_eth_snap_stream::<
            EthNetworkPrimitives,
            EthSnapSatelliteConnection<EthNetworkPrimitives>,
            EthSnapStreamError,
        >();
        assert_eth_snap_stream::<
            EthNetworkPrimitives,
            EthRlpxConnection<EthNetworkPrimitives>,
            EthStreamError,
        >();
    }
}
