...
impl<N: NetworkPrimitives> NetworkManager<N> {
    ...
    fn on_swarm_event(&mut self, event: SwarmEvent<N>) {
        ...
        match event {
            ...
            SwarmEvent::SessionClosed { peer_id, remote_addr, error } => {
                ...
                self.update_backed_off_metrics();
                self.event_sender
    ...
    /// Updates the backed-off peers gauges (total + per-direction).
    #[inline]
    fn update_backed_off_metrics(&self) {
        let peers = self.swarm.state().peers();
        self.metrics.backed_off_peers.set(peers.num_backed_off_peers() as f64);
        self.metrics
            .backed_off_peers_direction
            .incoming
            .set(peers.num_backed_off_incoming_peers() as f64);
        self.metrics
            .backed_off_peers_direction
            .outgoing
            .set(peers.num_backed_off_outgoing_peers() as f64);
    }
