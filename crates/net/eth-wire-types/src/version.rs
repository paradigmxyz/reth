...
pub enum EthVersion {
    ...
    ///
    /// Introduced by [EIP-7975], which adds support for paginating block receipt lists by allowing
    /// partial `GetReceipts` requests and `Receipts` responses.
    ///
    /// [EIP-7975]: https://eips.ethereum.org/EIPS/eip-7975
    Eth70 = 70,
    /// The `eth` protocol version 71.
    ///
    /// Introduced by [EIP-8159], which adds `GetBlockAccessLists` and `BlockAccessLists` messages
    /// for requesting and serving block-level access lists.
    ///
    /// [EIP-8159]: https://eips.ethereum.org/EIPS/eip-8159
    Eth71 = 71,
    /// The `eth` protocol version 72.
    ///
    /// Introduced by [EIP-8070], which adds sparse blobpool support by extending
    /// `NewPooledTransactionHashes` with a cell custody mask and updating blob transaction
