...
impl Decodable for NewPooledTransactionHashes72 {
    fn decode(buf: &mut &[u8]) -> alloy_rlp::Result<Self> {
        ...
        let has_type3_tx = types.iter().any(|ty| *ty == 3);
        match (has_type3_tx, cell_mask) {
            (true, None) => {
                return Err(alloy_rlp::Error::Custom(
                    "cell_mask is required when type 3 transactions are present".into(),
                ))
            }
            (false, Some(_)) => {
                return Err(alloy_rlp::Error::Custom(
                    "cell_mask must be None when no type 3 transactions are present".into(),
                ))
            }
            _ => {}
        }
