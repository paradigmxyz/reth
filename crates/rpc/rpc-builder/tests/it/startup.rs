...
async fn test_http_addr_in_use() {
    ...
    let server =
        builder.build(TransportRpcModuleConfig::set_http(vec![RethRpcModule::Admin]), EthApiBuild);
    let result = server
...
async fn test_ws_addr_in_use() {
    ...
    let builder = test_rpc_builder();
    let server =
        builder.build(TransportRpcModuleConfig::set_ws(vec![RethRpcModule::Admin]), EthApiBuild);
    let result =
