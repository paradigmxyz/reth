...
/// Launches a new server for the auth module, using the given [ChainSpec] to configure the
/// [EngineApi]. This still uses the default [BeaconConsensusEngineHandle] and test payload
/// service.
pub async fn launch_auth_with_chainspec(
    secret: JwtSecret,
...
pub async fn launch_auth(secret: JwtSecret) -> AuthServerHandle {
    launch_auth_with_chainspec(secret, MAINNET.clone()).await
}
