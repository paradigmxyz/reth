...
pub fn try_convert_from_execution_payload_v2_to_block(
    ...
    let mut base_sealed_block =
        try_convert_from_execution_payload_v1_to_block(payload.payload_inner)?;
...
pub fn try_convert_from_execution_payload_v3_to_block(
    ...
    let mut base_block = try_convert_from_execution_payload_v2_to_block(payload.payload_inner)?;
...
pub fn try_convert_from_sealed_block_to_execution_payload(value: SealedBlock) -> ExecutionPayload {
    if value.header.parent_beacon_block_root.is_some() {
        ...
        ExecutionPayload::V3(try_convert_from_sealed_block_to_execution_payload_v3(value))
    } else if value.withdrawals.is_some() {
        // block with withdrawals: V2
        ExecutionPayload::V2(try_convert_from_sealed_block_to_execution_payload_v2(value))
    } else {
        // otherwise V1
        ExecutionPayload::V1(try_convert_from_sealed_block_to_execution_payload_v1(value))
    }
}
...
pub fn try_into_sealed_block(
    ...
    let mut base_payload = match value {
        ExecutionPayload::V1(payload) => try_convert_from_execution_payload_v1_to_block(payload)?,
        ExecutionPayload::V2(payload) => try_convert_from_execution_payload_v2_to_block(payload)?,
        ExecutionPayload::V3(payload) => try_convert_from_execution_payload_v3_to_block(payload)?,
    };
