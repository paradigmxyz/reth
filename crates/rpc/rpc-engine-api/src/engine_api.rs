    ...
    EngineT: EngineTypes + 'static,
{
    ...
    async fn new_payload_v1(&self, payload: ExecutionPayloadV1) -> RpcResult<PayloadStatus> {
        ...
        let res = EngineApi::new_payload_v1(self, payload.clone()).await;
        let elapsed = start.elapsed();
        self.inner.metrics.latency.new_payload_v1.record(elapsed);
        self.inner.metrics.new_payload_response.update_response_metrics(&res, payload.gas_used, elapsed);
        Ok(res?)
    }
    ...
    async fn new_payload_v2(&self, payload: ExecutionPayloadInputV2) -> RpcResult<PayloadStatus> {
        ...
        let start = Instant::now();
        let res = EngineApi::new_payload_v2(self, payload.clone()).await;
        let elapsed = start.elapsed();
        self.inner.metrics.latency.new_payload_v2.record(elapsed);
        self.inner.metrics.new_payload_response.update_response_metrics(&res, payload.execution_payload.gas_used, elapsed);
        Ok(res?)
    }
    ...
    async fn new_payload_v3(
        ...
        let start = Instant::now();
        let res =
            EngineApi::new_payload_v3(self, payload.clone(), versioned_hashes, parent_beacon_block_root)
                .await;
        let elapsed = start.elapsed();
        self.inner.metrics.latency.new_payload_v3.record(elapsed);
        self.inner.metrics.new_payload_response.update_response_metrics(&res, payload.payload_inner.payload_inner.gas_used, elapsed);
        Ok(res?)
    }
