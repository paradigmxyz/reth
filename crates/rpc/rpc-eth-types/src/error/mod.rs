...
use reth_rpc_types_compat::{CallFeesError, TransactionConversionError};
use reth_transaction_pool::error::{
    Eip4844PoolTransactionError, Eip7702PoolTransactionError, InvalidPoolTransactionError,
    PoolError, PoolErrorKind, PoolTransactionError,
};
use revm::context_interface::result::{
    EVMError, ExecutionResult, InvalidHeader, InvalidTransaction,
};
use revm_inspectors::tracing::MuxError;
use std::convert::Infallible;
use tracing::error;

pub use reth_rpc_types_compat::error::{RevertError, RpcInvalidTransactionError, ToRpcError};
use reth_rpc_types_compat::transaction::EthTxEnvError;
...
impl From<EthTxEnvError> for EthApiError {
    fn from(value: EthTxEnvError) -> Self {
        match value {
            EthTxEnvError::CallFees(CallFeesError::InvalidTransaction(err)) => {
