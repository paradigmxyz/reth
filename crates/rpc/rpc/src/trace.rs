    ...
    Eth: TraceExt + 'static,
{
    ...
    pub async fn trace_raw_transaction(
        ...
        let (evm_env, at) = self
            .eth_api()
            .evm_env_at(block_id.unwrap_or_default())
            .await?;
    ...
    pub async fn trace_get_index(
        ...
        Ok(self
            ...
            .await?
            .and_then(|traces| traces.into_iter().nth(index)))
    }
    ...
    pub async fn trace_block_opcode_gas(
        ...
        let Some(transactions) = res else {
            return Ok(None);
        };
    ...
    fn calculate_base_block_reward<H: BlockHeader>(
        ...
        Ok(Some(base_block_reward_pre_merge(
            &chain_spec,
            header.number(),
        )))
    }
    ...
    Eth: TraceExt + 'static,
{
    ...
    async fn trace_call_many(
        ...
        let _permit = self.acquire_trace_permit().await;
        Ok(Self::trace_call_many(self, calls, block_id)
            .await
            .map_err(Into::into)?)
    }
    ...
    async fn trace_block(
        ...
        let _permit = self.acquire_trace_permit().await;
        Ok(Self::trace_block(self, block_id)
            .await
            .map_err(Into::into)?)
    }
    ...
    async fn trace_transaction(
        ...
        let _permit = self.acquire_trace_permit().await;
        Ok(Self::trace_transaction(self, hash)
            .await
            .map_err(Into::into)?)
    }
    ...
    async fn trace_block_opcode_gas(&self, block_id: BlockId) -> RpcResult<Option<BlockOpcodeGas>> {
        let _permit = self.acquire_trace_permit().await;
        Ok(Self::trace_block_opcode_gas(self, block_id)
            .await
            .map_err(Into::into)?)
    }
}
