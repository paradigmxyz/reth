        ...
        BlockExecutorProvider<Primitives: NodePrimitives<Block = ProviderBlock<Eth::Provider>>>,
{
    ...
    async fn trace_block(
        ...
        self.eth_api()
            .spawn_with_state_at_block(block.parent_hash().into(), move |state| {
                ...
                this.eth_api()
                    .apply_pre_execution_changes(&block, &mut db, &evm_env)?;
    ...
    pub async fn debug_trace_raw_block(
        ...
        self.trace_block(
            Arc::new(block.into_recovered_with_signers(senders)),
            evm_env,
            opts,
        )
        .await
                    ...
                    GethDebugBuiltInTracerType::FourByteTracer => {
                        ...
                        let inspector = self
                            ...
                            .spawn_with_call_at(call, at, overrides, move |db, evm_env, tx_env| {
                                this.eth_api()
                                    .inspect(db, evm_env, tx_env, &mut inspector)?;
                                Ok(inspector)
                            })
                            .await?;
    ...
    fn trace_transaction(
        ...
        let tx_info = TransactionInfo {
            ...
            index: transaction_context
                ...
                .unwrap_or_default(),
            block_hash: transaction_context
                .as_ref()
                .map(|c| c.block_hash)
                .unwrap_or_default(),
            block_number: Some(evm_env.block_env.number),
        ...
        BlockExecutorProvider<Primitives: NodePrimitives<Block = ProviderBlock<Eth::Provider>>>,
{
    ...
    async fn debug_execution_witness(
        ...
        let _permit = self.acquire_trace_permit().await;
        Self::debug_execution_witness(self, block)
            .await
            .map_err(Into::into)
    }
    ...
    async fn debug_code_by_hash(
        ...
        Self::debug_code_by_hash(self, hash, block_id)
            .await
            .map_err(Into::into)
    }
