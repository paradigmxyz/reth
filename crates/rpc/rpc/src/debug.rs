...
use revm_inspectors::tracing::{DebugInspector, DebugInspectorError, TransactionContext};
use std::sync::Arc;
    ...
    async fn trace_block(
        ...
        self.eth_api()
            .spawn_with_state_at_block(block.parent_hash(), move |eth_api, mut db| {
                ...
                let mut inspector =
                    DebugInspector::new(opts).map_err(convert_debug_inspector_error)?;
                while let Some((index, tx)) = transactions.next() {
                    ...
                    let result = inspector
                        ...
                        .map_err(convert_debug_inspector_error_with_db)?;

                    results.push(TraceResult::Success { result, tx_hash: Some(tx_hash) });
                    if transactions.peek().is_some() {
                        inspector.fuse().map_err(convert_debug_inspector_error)?;
    ...
    pub async fn debug_trace_call(
        ...
        self.eth_api()
            .spawn_with_call_at(call, at, overrides, move |db, evm_env, tx_env| {
                let mut inspector =
                    DebugInspector::new(tracing_options).map_err(convert_debug_inspector_error)?;
        ...
        self.eth_api()
            .spawn_with_state_at_block(at, move |eth_api, mut db| {
                ...
                let mut inspector = DebugInspector::new(tracing_options.clone())
                    .map_err(convert_debug_inspector_error)?;
...
// Helper function to convert DebugInspectorError to EthApiError
fn convert_debug_inspector_error(err: DebugInspectorError) -> EthApiError {
    match err {
        DebugInspectorError::InvalidTracerConfig => EthApiError::InvalidTracerConfig,
        DebugInspectorError::UnsupportedTracer => EthApiError::Unsupported("unsupported tracer"),
        DebugInspectorError::JsTracerNotEnabled => {
            EthApiError::Unsupported("JS Tracer is not enabled")
        }
        DebugInspectorError::MuxInspector(e) => EthApiError::MuxTracerError(e),
        #[cfg(feature = "js-tracer")]
        DebugInspectorError::JsInspector(e) => {
            EthApiError::InternalJsTracerError(format!("{:?}", e))
        }
        DebugInspectorError::Database(_) => {
            EthApiError::Internal(RethError::msg("Database error in DebugInspector"))
        }
    }
}

// Helper function to convert DebugInspectorError<ProviderError> to EthApiError
fn convert_debug_inspector_error_with_db<DBError>(err: DebugInspectorError<DBError>) -> EthApiError
where
    DBError: Into<RethError>,
{
    match err {
        DebugInspectorError::InvalidTracerConfig => EthApiError::InvalidTracerConfig,
        DebugInspectorError::UnsupportedTracer => EthApiError::Unsupported("unsupported tracer"),
        ...
        DebugInspectorError::Database(e) => EthApiError::Internal(e.into()),
    }
}
