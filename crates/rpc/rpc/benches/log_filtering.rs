...
use reth_primitives::{bloom::logs_bloom, Address, Receipt, H256, MAINNET, U256};
use reth_provider::{BlockWriter, BundleStateWithReceipts, OriginalValuesKnown, ProviderFactory};
...
fn generate_test_data<R: Rng>(
    ...
    for block_idx in block_range.clone() {
        ...
        let mut block = random_block(
            rng,
            block_idx,
            Some(parent_hash.unwrap_or(H256::random())),
            Some(tx_count),
            None,
        )
        .unseal();
    ...
    for (block, senders, receipts) in data {
        ...
        BundleStateWithReceipts::new(
            BundleState::default(),
            Vec::from([receipts.into_iter().map(Some).collect()]),
            block_number,
        )
    ...
    let (log_address_indices, _log_topic_indices, _) =
        provider_rw.get_log_addresses_and_topics(block_range).unwrap();
...
mod implementations {
    ...
    impl<DB: Database> BenchLogFiltering<DB> for IndexLogFiltering<DB> {
        ...
        fn get_logs_in_block_range(
            ...
            let mut log_index_filter = LogIndexFilter::new(from_block..=to_block, u64::MAX);
            ...
            for block_numbers in log_index_filter.iter() {
                let blocks =
                    provider.blocks(block_numbers.into_iter().map(Into::into).collect())?;
                ...
                for block in blocks {
                    ...
                    let receipts =
                        provider.receipts_by_block(block.number.into())?.unwrap_or_default();
