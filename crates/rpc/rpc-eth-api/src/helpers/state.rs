...
pub trait EthState: LoadState + SpawnBlocking {
        ...
        Self: EthApiSpec,
    {
        Ok(async move {
            ...
            self.spawn_blocking_io_with_state(Some(block_id), move |_, state| {
                let _permit = permit;
    ...
    > + RpcNodeCoreExt
{
    ...
    /// Executes `f` with the state at the given [`BlockId`], or the latest state if `None`, on a
    /// blocking IO task.
    ///
    /// The `pending` state is resolved on the calling task, before the blocking task starts.
    /// Building the pending block needs a blocking task of its own, so a blocking task that waited
    /// for it could hold the thread the build needs. Every other state is a synchronous provider
    /// read and is resolved inside the blocking task.
    ...
    where
        Self: SpawnBlocking,
    {
        self.spawn_blocking_io_with_state(block_id, move |this, state| {
            // first fetch the on chain nonce of the account
