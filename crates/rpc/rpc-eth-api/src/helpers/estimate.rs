...
pub trait EstimateCall: Call {
        ...
        S: StateProvider,
    {
        ...
        let mut res = match evm.transact(tx_env.clone()).map_err(Self::Error::from_evm_err) {
            ...
            {
                return Err(Self::map_out_of_gas_err(&mut evm, tx_env, max_gas_limit))
            }
        ...
        let gas_refund = match res.result {
            ...
            ExecutionResult::Revert { output, .. } => {
                ...
                return if tx_request_gas_limit.is_some() || tx_request_gas_price.is_some() {
                    Err(Self::map_out_of_gas_err(&mut evm, tx_env, max_gas_limit))
                } else {
        ...
        EthApiError: From<DB::Error>,
    {
        ...
        let res = match evm.transact(tx_env) {
            Ok(res) => res,
            Err(err) => return Self::Error::from_evm_err(err),
        };
        match res.result {
