...
pub trait EstimateCall: Call {
    ...
    fn estimate_gas_with<S>(
        ...
        #[cfg(not(any()))]
        {
            let _ = (&mut evm_env, &mut request, state, overrides);
        ...
        #[cfg(any())]
        {
                ...
                match db.database.basic_account(&to) {
                    Ok(Some(account)) => {
                        account.bytecode_hash.is_none() ||
                            account.bytecode_hash == Some(KECCAK_EMPTY)
                    }
                    _ => true,
                }
            } else {
            ...
            // Check funds of the sender (only useful to check if transaction gas price is more than
            // 0).
            //
            ...
            let gas_refund = match res.result {
                ...
                ExecutionResult::Halt { reason, .. } => {
                    // here we don't check for invalid opcode because already executed with highest
                    // gas limit
            ...
            // transaction succeeds with. We find this by doing a binary search over the possible
            // range.
            ...
            // the gas limit to the execution gas used plus the gas refund, so we check
            // this first <https://github.com/ethereum/go-ethereum/blob/a5a4fa7032bb248f5a7c40f4e8df2b131c4186a4/eth/gasestimator/gasestimator.go#L135
            //
            ...
            if optimistic_gas_limit < highest_gas_limit {
                ...
                // Update the gas limit estimates (highest and lowest) based on the execution
                // result.
            ...
            while lowest_gas_limit + 1 < highest_gas_limit {
                ...
                // binary search is small enough (less than 1.5% of the highest gas
                // limit) <https://github.com/ethereum/go-ethereum/blob/a5a4fa7032bb248f5a7c40f4e8df2b131c4186a4/eth/gasestimator/gasestimator.go#L152
                let ratio =
                    (highest_gas_limit - lowest_gas_limit) as f64 / (highest_gas_limit as f64);
                ...
                // Execute transaction and handle potential gas errors, adjusting limits
                // accordingly.
