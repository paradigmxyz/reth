...
pub trait EthTransactions: LoadTransaction<Provider: BlockReaderIdExt> {
        ...
        Self: EthApiSpec + LoadBlock + EstimateCall + LoadFee,
    {
        async move {
            ...
            let gas_price_override = gas_price.filter(|gas_price| !gas_price.is_zero());
            if let Some(gas_price) = gas_price_override {
                request.as_mut().set_gas_price(gas_price.to());
            }

            let gas_limit_override = gas_limit.filter(|gas_limit| *gas_limit != U64::ZERO);
            ...
            if gas_price_override.is_none() && request.as_ref().gas_price().is_none() {
