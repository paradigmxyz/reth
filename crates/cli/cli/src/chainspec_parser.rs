...
///
/// This trait extends `TypedValueParser` to provide a parser for chain specifications.
/// Implementors of this trait must provide a list of supported chains and a function to parse
/// a given string into a `ChainSpec`.
pub trait ChainSpecParser: TypedValueParser<Value = Arc<ChainSpec>> + Default {
    ...
    /// `ChainSpec`.
    fn parse(s: &str) -> Result<Arc<ChainSpec>, eyre::Error>;
}
