...
impl Command {
    ...
    pub async fn execute(self) -> eyre::Result<()> {
        ...
        let (_discv4, mut discv4_service, _discv5, mut discv5_forwarder) = if self.v5 {
            ...
            let (ipv4, ipv6) = if self.addr.is_ipv4() {
                (Some(shared_socket.clone()), secondary_socket.clone())
            } else {
                (secondary_socket.clone(), Some(shared_socket.clone()))
            };
    ...
    fn discv4_config(&self, nat: &BootnodeNat, seeds: &BootnodeSeeds) -> Discv4Config {
        ...
        builder.external_ip_resolver(Some(nat.resolver.clone())).add_boot_nodes(
            seeds.discv4_records().filter(|record| {
                let served = self.serves_family(record.address, nat);
                if !served {
                    warn!(
        ...
        for node in &self.bootnodes {
            let record = match node {
                AnyNode::NodeRecord(record) => {
                    // v4-mapped hosts parse as V6; normalize them so they route as IPv4
    ...
    ///
    /// Intentionally not `NodeRecord::try_from(&Enr)`: this needs to prefer the family this
    /// bootnode serves instead of that conversion's IPv4-first order.
    fn enr_endpoint(&self, enr: &Enr<SecretKey>, nat: &BootnodeNat) -> eyre::Result<NodeRecord> {
        ...
        // per EIP-778, generic `udp`/`tcp` keys also apply to IPv6 when the v6-specific
        // counterparts are absent
        let v6 = enr.ip6().zip(enr.udp6().or_else(|| enr.udp4())).map(|(ip, udp_port)| {
            NodeRecord {
                address: ip.into(),
                udp_port,
                tcp_port: enr.tcp6().or_else(|| enr.tcp4()).unwrap_or(0),
                id,
            }
            .into_ipv4_mapped()
        });

        // an unservable endpoint is kept as a last resort (the startup warning covers it), and a
        // zero-udp endpoint only after that, so `seed_nodes` reports the missing port
        let nonzero = |record: &NodeRecord| record.udp_port != 0;
        v4.into_iter()
            .chain(v6)
            .filter(nonzero)
            .find(|record| self.serves_family(record.address, nat))
            .or_else(|| v4.filter(nonzero))
            .or_else(|| v6.filter(nonzero))
            .or(v4)
            .or(v6)
            .ok_or_else(|| {
...
/// Whether discv5 can dial `enr` with the sockets it binds, delegating to
/// [`discv5::IpMode::get_contactable_addr`]. A zero-port endpoint passes the ip mode check but is
/// not dialable, so it is rejected too.
fn discv5_contactable(enr: &discv5::Enr, bind_ipv4: bool, bind_ipv6: bool) -> bool {
    match (bind_ipv4, bind_ipv6) {
        (true, true) => IpMode::DualStack,
        (false, true) => IpMode::Ip6,
        _ => IpMode::Ip4,
    }
    .get_contactable_addr(enr)
...
mod tests {
    ...
    fn bootnodes_normalize_and_validate_enodes() {
        ...
        // a v4-mapped host normalizes to IPv4
        let command = Command::parse_from([
            "reth",
            "--bootnodes",
    ...
    #[test]
    fn enr_seed_uses_generic_ports_for_ip6() {
        let sk = rng_secret_key();
        let enr = Enr::builder()
            .ip6("2001:db8::9".parse().unwrap())
            .udp4(30305)
        ...
        let command =
            Command::parse_from(["reth", "--addr", "[::]:30303", "--bootnodes", &enr.to_base64()]);
        let seeds = command.seed_nodes(&command.resolved_nat().unwrap()).unwrap();
        assert_eq!(seeds.enr_records[0].address, "2001:db8::9".parse::<IpAddr>().unwrap());
        assert_eq!(seeds.enr_records[0].udp_port, 30305);
        assert_eq!(seeds.enr_records[0].tcp_port, 30306);
    }

    #[test]
    fn enr_seed_skips_zero_udp_family() {
        let sk = rng_secret_key();
        let enr = Enr::builder()
            .ip4("9.9.9.9".parse().unwrap())
            .udp4(0)
        ...
        // v4-primary: the zero-udp v4 endpoint must not win over the usable v6 endpoint
        let command = Command::parse_from([
            "reth",
            "--addr",
            "0.0.0.0:30303",
        ...
        // discv5 must not seed the zero-port v4 endpoint either
        assert!(!discv5_contactable(&seeds.enrs[0], true, false));
        assert!(discv5_contactable(&seeds.enrs[0], false, true));

        // zero udp on every family is still an unusable entry
        let enr = Enr::builder().ip4("9.9.9.9".parse().unwrap()).udp4(0).build(&sk).unwrap();
        let command = Command::parse_from([
            "reth",
            "--addr",
            "0.0.0.0:30303",
        ...
        let err = command.seed_nodes(&command.resolved_nat().unwrap()).unwrap_err();
        assert!(err.to_string().contains("no discovery port"), "{err}");
    }
