...
use reth_execution_types::{BlockReverts, EvmState, RevertToSlot};
            ...
            tasks.spawn_blocking(move || {
                ...
                loop {
                    ...
                    if verify_state_roots {
                        ...
                        let execution_state = state.into_execution_state();
                        let state_root = verify_state_root(&parent_state, &execution_state, chunk_start, block.state_root())?;
...
fn verify_state_root<P: HashedPostStateProvider + StateRootProvider>(
    ...
    let actual = parent.state_root(parent.hashed_post_state(state))?;
    eyre::ensure!(
        actual == expected,
...
mod tests {
    ...
    fn root_verification_detects_corrupt_execution_state() {
        ...
        let execution_state = |balance| {
            reth_execution_types::execution_state_from_init(
                [(
                    address,
                    (
                        None,
                        Some(Account { nonce: 1, balance, bytecode_hash: None }),
                        Default::default(),
                    ),
                )],
                [],
        ...
        let state = execution_state(U256::from(42));
        let expected = state_root_unhashed([(
            address,
            TrieAccount { nonce: 1, balance: U256::from(42), ..Default::default() },
        )]);
        assert_eq!(verify_state_root(&parent, &state, 128, expected).unwrap(), expected);

        let corrupt = execution_state(U256::from(43));
