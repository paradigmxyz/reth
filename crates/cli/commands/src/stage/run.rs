...
use reth_stages::{
    stages::{
        ...
        IndexStorageHistoryStage, MerkleChangeSets, MerkleStage, SenderRecoveryStage,
...
impl<C: ChainSpecParser<ChainSpec: EthChainSpec + Hardforks + EthereumHardforks>> Command<C> {
        ...
        F: FnOnce(Arc<C::ChainSpec>) -> Comp,
    {
        ...
        let mut stages_to_run: Vec<(Box<dyn Stage<_>>, Option<Box<dyn Stage<_>>>)> = match self.stage
        {
            StageEnum::Headers => {
            ...
            StageEnum::Bodies => {
                ...
                vec![(Box::new(stage), None)]
            }
            StageEnum::Senders => vec![(
            ...
            StageEnum::MerkleChangeSets => vec![(Box::new(MerkleChangeSets::new()), None)],
            StageEnum::AccountHistory => vec![(
        ...
        for (mut exec_stage, mut unwind_stage) in stages_to_run.drain(..) {
            ...
            info!(target: "reth::cli", stage = %exec_stage.id(), "Executing stage");
            loop {
