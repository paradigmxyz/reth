...
macro_rules! handle_stage {
    ...
    ($stage_fn:ident, $tool:expr, $command:expr, $runtime:expr, $executor:expr, $consensus:expr) => {{
        let StageCommand { output_datadir, from, to, dry_run, .. } = $command;
        let output_datadir =
            output_datadir.with_chain($tool.chain().chain(), DatadirArgs::default());
...
impl<C: ChainSpecParser<ChainSpec: EthChainSpec + EthereumHardforks>> Command<C> {
        ...
        F: FnOnce(Arc<C::ChainSpec>) -> Comp,
    {
        ...
        match &self.command {
            ...
            Stages::Merkle(cmd) => {
                handle_stage!(dump_merkle_stage, &tool, cmd, runtime.clone(), evm_config, consensus)
            }
        }
