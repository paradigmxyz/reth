...
/// Package chunk archives from a source datadir into a directory and generate a manifest.
///
/// This is the filesystem-backed convenience wrapper around
/// [`generate_manifest_with_sink`].
pub fn generate_manifest(
    ...
    generate_manifest_with_sink(
        source_datadir,
        &DirectoryArchiveSink::new(output_dir),
        base_url,
        block,
        chain_id,
        blocks_per_file,
...
/// Package chunk archives from a source datadir into `archive_sink` and generate a manifest.
///
/// Each archive is written as a complete `.tar.zst` stream. The archive name passed to
/// [`SnapshotArchiveSink::create_archive`] is the relative name recorded in the manifest. The
/// sink's writer is only finalized after the tar archive and Zstandard stream have both been
/// finalized successfully.
///
/// This lets callers publish archive bytes without staging them on the local filesystem while
/// retaining the manifest format and archive layout generated by [`generate_manifest`].
pub fn generate_manifest_with_sink(
    ...
    for ty in &[
        ...
        for i in 0..num_chunks {
            ...
            planned_chunks.push(PlannedChunk {
                chunk_idx: i,
                archive_file_name: chunk_filename(key, start, end),
                source_files,
            });
        }
        ...
        if found_any {
            let mut packaged_chunks = planned_chunks
                ...
                .map(|planned| -> Result<PackagedChunk> {
                    let packaged = write_chunk_archive(
                        archive_sink,
                        &planned.archive_file_name,
    ...
    if !rocksdb_files.is_empty() {
        let (rocksdb_size, rocksdb_output_files) =
            package_single_component(archive_sink, "rocksdb_indices.tar.zst", &rocksdb_files)?;
        components.insert(
...
/// Destination for snapshot archive streams.
///
/// Snapshot generation may package chunked components in parallel, so implementations must be
/// safe to call from multiple threads. The archive name is relative and is the name recorded in
/// the generated manifest.
pub trait SnapshotArchiveSink: Send + Sync {
    /// Creates a writer for a snapshot archive.
    fn create_archive(&self, archive_file_name: &str) -> Result<Box<dyn SnapshotArchiveWriter>>;
}

/// Writer for one snapshot archive stream.
///
/// [`Self::finish`] is called only after the tar archive and its enclosing Zstandard frame were
/// successfully finalized. Implementations can use it as the signal to publish a streamed
/// object. If generation fails before then, the writer is dropped without `finish` being called.
pub trait SnapshotArchiveWriter: Write + Send {
    /// Finalizes the archive destination after a valid archive stream has been written.
    fn finish(self: Box<Self>) -> Result<()>;
}

/// Filesystem-backed archive sink used by [`generate_manifest`].
#[derive(Debug)]
...
impl DirectoryArchiveSink {
    /// Creates a sink that writes archive files below `output_dir`.
...
impl SnapshotArchiveSink for DirectoryArchiveSink {
    fn create_archive(&self, archive_file_name: &str) -> Result<Box<dyn SnapshotArchiveWriter>> {
        let path = self.output_dir.join(archive_file_name);
        Ok(Box::new(FileArchiveWriter(std::fs::File::create(path)?)))
    }
}
...
fn package_single_component(
    ...
    let packaged = write_archive_from_planned_files(archive_sink, archive_file_name, files)?;
...
fn write_chunk_archive(
    ...
    write_archive_from_planned_files(archive_sink, archive_file_name, &planned_files)
}
...
fn write_archive_from_planned_files(
    ...
    let writer = CountingWriter::new(archive_sink.create_archive(archive_file_name)?);
...
mod tests {
    ...
    struct InMemoryArchiveSink {
        archives: Arc<Mutex<BTreeMap<String, Vec<u8>>>>,
    }

    impl InMemoryArchiveSink {
        fn archive(&self, name: &str) -> Vec<u8> {
            self.archives.lock().unwrap().get(name).cloned().unwrap()
        }
    }

    impl SnapshotArchiveSink for InMemoryArchiveSink {
        fn create_archive(
            &self,
            archive_file_name: &str,
        ) -> Result<Box<dyn SnapshotArchiveWriter>> {
            Ok(Box::new(InMemoryArchiveWriter {
    ...
    struct InMemoryArchiveWriter {
        archive_file_name: String,
        archives: Arc<Mutex<BTreeMap<String, Vec<u8>>>>,
        bytes: Vec<u8>,
    }

    impl Write for InMemoryArchiveWriter {
        fn write(&mut self, buf: &[u8]) -> std::io::Result<usize> {
    ...
    impl SnapshotArchiveWriter for InMemoryArchiveWriter {
        fn finish(mut self: Box<Self>) -> Result<()> {
            self.archives
                .lock()
                .unwrap()
                .insert(self.archive_file_name.clone(), std::mem::take(&mut self.bytes));
            Ok(())
        }
    }
    ...
    #[test]
    fn generate_manifest_writes_valid_zstd_archives_to_custom_sink() {
        let source = tempdir().unwrap();
        let db_dir = source.path().join("db");
        std::fs::create_dir_all(&db_dir).unwrap();
        std::fs::write(db_dir.join("mdbx.dat"), b"state-data").unwrap();
        ...
        let manifest =
            generate_manifest_with_sink(source.path(), &sink, None, 0, 1, 500_000).unwrap();

        let state = manifest.component(SnapshotComponentType::State).unwrap();
        let ComponentManifest::Single(state) = state else {
            panic!("state should be a single archive")
        };
        let archive_bytes = sink.archive(&state.file);
        assert_eq!(state.size, archive_bytes.len() as u64);

        let decoder = zstd::Decoder::new(Cursor::new(archive_bytes)).unwrap();
        let mut archive = tar::Archive::new(decoder);
        let mut entries = archive.entries().unwrap();
        let mut entry = entries.next().unwrap().unwrap();
        assert_eq!(entry.path().unwrap(), Path::new("db/mdbx.dat"));
        let mut contents = Vec::new();
        entry.read_to_end(&mut contents).unwrap();
        assert_eq!(contents, b"state-data");
