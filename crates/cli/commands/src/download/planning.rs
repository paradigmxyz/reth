...
pub(crate) fn collect_planned_archives(
    ...
    for (ty, selection) in selections {
        ...
        if let Some(ComponentManifest::Chunked(chunked)) = manifest.component(*ty) &&
            let Some((field, len)) = chunked.chunk_lengths_mismatch()
        {
            eyre::bail!(
                "Invalid modular manifest: {} {field} length ({len}) does not match chunk count ({})",
                ty.key(),
                chunked.num_chunks()
            );
        }
...
mod tests {
    ...
    #[test]
    fn collect_planned_archives_rejects_mismatched_chunk_sizes_length() {
        let mut components = BTreeMap::new();
        components.insert(
            "transactions".to_string(),
            ComponentManifest::Chunked(ChunkedArchive {
        ...
        let err = collect_planned_archives(&manifest, &selections).unwrap_err();
        assert!(
            err.to_string().contains("chunk_sizes length (1) does not match chunk count (2)"),
            "expected chunk_sizes length error, got: {err}"
        );
    }
}
