...
///
/// `Address | Topic | 200` -> data is from transition 0 to 200.
///
/// `Address | Topic | 300` -> data is from transition 201 to 300.
#[derive(
    Debug, Default, Clone, Eq, Ord, PartialOrd, PartialEq, AsRef, Serialize, Deserialize, Hash,
)]
pub struct LogShardedKey {
    /// Address that emitted the log.
    pub address: Address,
    /// Log topic with highest transition id.
    #[as_ref]
    pub sharded_key: ShardedKey<B256>,
}

impl LogShardedKey {
    /// Creates a new `LogShardedKey`.
    pub fn new(address: Address, topic: B256, highest_block_number: BlockNumber) -> Self {
        Self { address, sharded_key: ShardedKey { key: topic, highest_block_number } }
    }
    ...
    pub fn last(address: Address, topic: B256) -> Self {
        Self { address, sharded_key: ShardedKey { key: topic, highest_block_number: u64::MAX } }
    }
}

impl Encode for LogShardedKey {
    type Encoded = Vec<u8>;
