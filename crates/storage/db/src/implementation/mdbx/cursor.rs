...
pub struct Cursor<K: TransactionKind, T: Table> {
    ...
    triedb: Option<crate::triedb::Cursor<T>>,
}
...
impl<K: TransactionKind, T: Table> Cursor<K, T> {
    ...
    pub(crate) fn with_triedb(
        mut self,
        tx: Option<std::sync::Arc<crate::triedb::Transaction>>,
    ) -> Self {
        if let Some(tx) = tx &&
            let Some(table) = crate::triedb::TableSpec::named(T::NAME)
        {
            self.triedb = Some(crate::triedb::Cursor::new(tx, table));
        }
        self
    }
}
...
impl<K: TransactionKind, T: Table> DbCursorRO<T> for Cursor<K, T> {
    fn first(&mut self) -> PairResult<T> {
        #[cfg(feature = "monad-triedb")]
        if let Some(cursor) = &mut self.triedb {
            return cursor.first();
        }
...
impl<K: TransactionKind, T: DupSort> DbDupCursorRO<T> for Cursor<K, T> {
    ...
    fn seek_by_key_subkey(
        ...
        #[cfg(feature = "monad-triedb")]
        if let Some(cursor) = &mut self.triedb {
            return cursor.seek_by_key_subkey(key, subkey);
        }
        self.inner
    ...
    fn walk_dup(
        ...
        if self.triedb.is_some() {
            ...
            return Ok(DupWalker { cursor: self, start });
        }
...
impl<T: Table> DbCursorRW<T> for Cursor<RW, T> {
    ...
    fn upsert(&mut self, key: T::Key, value: &T::Value) -> Result<(), DatabaseError> {
        #[cfg(feature = "monad-triedb")]
        if let Some(cursor) = &mut self.triedb {
            return cursor.upsert(key, value);
        }
        let key = key.encode();
    ...
    fn insert(&mut self, key: T::Key, value: &T::Value) -> Result<(), DatabaseError> {
        #[cfg(feature = "monad-triedb")]
        if let Some(cursor) = &mut self.triedb {
            return cursor.insert(key, value);
        }
        let key = key.encode();
        let value = compress_to_buf_or_ref!(self, value);
        self.execute_with_operation_metric(
    ...
    fn append(&mut self, key: T::Key, value: &T::Value) -> Result<(), DatabaseError> {
        #[cfg(feature = "monad-triedb")]
        if let Some(cursor) = &mut self.triedb {
            return cursor.append(key, value);
        }
        let key = key.encode();
        let value = compress_to_buf_or_ref!(self, value);
        self.execute_with_operation_metric(
...
impl<T: DupSort> DbDupCursorRW<T> for Cursor<RW, T> {
    fn delete_current_duplicates(&mut self) -> Result<(), DatabaseError> {
        #[cfg(feature = "monad-triedb")]
        if let Some(cursor) = &mut self.triedb {
            return cursor.delete_current_duplicates();
        }
    ...
    fn append_dup(&mut self, key: T::Key, value: T::Value) -> Result<(), DatabaseError> {
        #[cfg(feature = "monad-triedb")]
        if let Some(cursor) = &mut self.triedb {
            return cursor.append_dup(key, value);
        }
        let key = key.encode();
        let value = compress_to_buf_or_ref!(self, value);
        self.execute_with_operation_metric(
