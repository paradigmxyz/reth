...
mod tests {
    ...
    fn db_dup_sort() {
        ...
        // PUT (2,2)
        let value22 = StorageEntry { key: H256::with_last_byte(2), value: U256::from(2) };
        env.update(|tx| tx.put::<PlainStorageState>(key, value22).expect(ERROR_PUT)).unwrap();

        // PUT (1,1)
        let value11 = StorageEntry { key: H256::with_last_byte(1), value: U256::from(1) };
        ...
        // Seek value with exact subkey
        {
            let tx = env.tx().expect(ERROR_INIT_TX);
            let mut cursor = tx.cursor_dup_read::<PlainStorageState>().unwrap();
            let mut walker = cursor.walk_dup(Some(key), Some(H256::with_last_byte(1))).unwrap();
            assert_eq!(
                (key, value11),
                walker
                    .next()
    ...
    fn dup_value_with_same_subkey() {
        ...
        // PUT key1 (0,1)
        let value01 = StorageEntry { key: H256::with_last_byte(0), value: U256::from(1) };
