...
impl Database for DatabaseEnv {
    ...
    fn tx_unsync(&self) -> Result<Self::TXUnsync, DatabaseError> {
        tx::TxUnsync::new(
            self.inner.begin_ro_unsync().map_err(|e| DatabaseError::InitTx(e.into()))?,
            self.dbis.clone(),
            self.metrics.clone(),
        )
        .map_err(|e| DatabaseError::InitTx(e.into()))
    ...
    fn tx_mut_unsync(&self) -> Result<Self::TXMutUnsync, DatabaseError> {
        tx::TxUnsync::new(
            self.inner.begin_rw_unsync().map_err(|e| DatabaseError::InitTx(e.into()))?,
            self.dbis.clone(),
            self.metrics.clone(),
        )
        .map_err(|e| DatabaseError::InitTx(e.into()))
    ...
    pub fn drop_orphan_table(&self, name: &str) -> Result<bool, DatabaseError> {
        ...
        match tx.open_db(Some(name)) {
            Ok(db) => {
                ...
                unsafe {
                    tx.drop_db(db).map_err(|e| DatabaseError::Delete(e.into()))?;
                }
                tx.commit().map_err(|e| DatabaseError::Commit(e.into()))?;
                Ok(true)
            }
