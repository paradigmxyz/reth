...
impl Zstd {
    ...
    pub fn compress_with_dictionaries(
        ...
        if let Some(compressor) = compressor {
            // Compressor requires the destination buffer to be big enough to write, otherwise it fails.
            // However, we don't know how big it will be. If data is small enough, the compressed buffer will actually be larger.
            // We keep retrying. If we eventually fail, it probably means it's another kind of error.
...
impl Compression for Zstd {
    ...
    fn prepare_compression(
        ...
        // There's a per 2GB hard limit on each column data training, iterator should take care of it. Each element should ideally be bigger than 1 (otherwise sigev)
        // huffman coding + sueprstring ?
        ...
        for column in columns {
            ...
            dictionaries.push(zstd::dict::from_continuous(
                &data,
                &sizes,
                self.max_dict_size,
            )?);
