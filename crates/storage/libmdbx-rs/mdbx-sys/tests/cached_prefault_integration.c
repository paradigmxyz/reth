/* Public-API regression for Linux cache-only prefaults.
 * Compile with cc -D_GNU_SOURCE -DMDBX_DEBUG=1 -DMDBX_USE_FALLOCATE=0
 * -std=c11 -O1 -g -pthread cached_prefault_integration.c -o /tmp/test -lm.
 * Add -DMDBX_MINCORE_CACHE_ONLY=1 to test the Linux policy on another OS.
 * For UBSan, add -DMDBX_UNALIGNED_OK=0 to use portable unaligned accesses.
 * Non-Linux builds retain residency probing, with a deterministic cold shim. */
#define _GNU_SOURCE 1
#ifndef MDBX_BUILD_FLAGS
#define MDBX_BUILD_FLAGS "cached-prefault integration regression"
#endif
#include <sys/mman.h>
#include <sys/types.h>
#include <sys/uio.h>
...
static size_t pattern_writes, vectored_pattern_writes, pattern_pages,
    mincore_calls;

static ssize_t test_pwrite(int fd, const void *buf, size_t count,
                           off_t offset) {
  const bool pattern =
      watched_env && fd == watched_env->lazy_fd &&
      buf == ptr_disp(watched_env->page_auxbuf, watched_env->ps * 2);
  const ssize_t written = pwrite(fd, buf, count, offset);
  if (pattern && written == (ssize_t)count) {
    ++pattern_writes;
    pattern_pages += count / watched_env->ps;
  }
  return written;
}

/* Vectored counts include both speculative batches and overflow allocations. */
static ssize_t test_pwritev(int fd, const struct iovec *iov, int count,
                            off_t offset) {
  bool pattern = watched_env && fd == watched_env->lazy_fd;
  size_t bytes = 0;
  for (int i = 0; i < count; ++i) {
    pattern = pattern && iov[i].iov_base == ptr_disp(watched_env->page_auxbuf,
                                                     watched_env->ps * 2);
    bytes += iov[i].iov_len;
  }
  const ssize_t written = pwritev(fd, iov, count, offset);
  if (pattern && written == (ssize_t)bytes) {
    ++vectored_pattern_writes;
    pattern_pages += bytes / watched_env->ps;
  }
  return written;
}
...
static size_t value_size(unsigned key, unsigned pagesize) {
  return key % 37 == 0 ? pagesize + 19 : 80 + key % 3 * 17;
}

static void fill_value(unsigned char *value, size_t size, unsigned generation,
                       unsigned key) {
  for (size_t i = 0; i < size; ++i)
    value[i] = (unsigned char)(generation * 13 ^ key ^ i);
}

static bool deleted(unsigned generation, unsigned key) {
  return generation > 0 && key % 11 == generation % 11;
}

static void update(MDBX_env *env, unsigned generation, unsigned pagesize,
                   bool abort_txn) {
  MDBX_txn *txn;
...
static void run(unsigned pagesize) {
  char path[] = "/tmp/mdbx-cached-prefault-XXXXXX";
  CHECK(mkdtemp(path));
  MDBX_env *env;
  OK(mdbx_env_create(&env));
  OK(mdbx_env_set_geometry(env, 0, (intptr_t)pagesize * 2048,
                           (intptr_t)pagesize * 4096, 0, 0, pagesize));
  OK(mdbx_env_set_option(env, MDBX_opt_prefault_write_enable, 1));
  OK(mdbx_env_open(
      env, path, MDBX_WRITEMAP | MDBX_NOSTICKYTHREADS | MDBX_NORDAHEAD, 0600));
  watched_env = env;
  pattern_writes = vectored_pattern_writes = pattern_pages = mincore_calls = 0;
  ...
  CHECK(pattern_writes + vectored_pattern_writes > 0);
  CHECK(MDBX_USE_MINCORE == 1);
#if MDBX_MINCORE_CACHE_ONLY
  CHECK(mincore_calls == 0 && info.mi_pgop_stat.mincore == 0);
#else
  CHECK(mincore_calls > 0);
#endif
  printf("public API %u-byte pages: %zu single + %zu vectored pattern writes, "
         "%zu pages, %zu probes, snapshot/abort verified\n",
         pagesize, pattern_writes, vectored_pattern_writes, pattern_pages,
         mincore_calls);
  watched_env = NULL;
...
int main(void) {
  ...
  run(16384);
  run(65536);
  puts("cached prefault public API: update/delete, snapshot, abort and reopen "
       "passed");
  return 0;
}
