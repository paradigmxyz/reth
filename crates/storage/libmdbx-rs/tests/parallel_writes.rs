    ...
    for round in 0..5 {
        ...
        // Thread 2: Update storage (simulated - same thread for simplicity)
        {
            ...
            for i in 0..100u32 {
                ...
                // UPSERT alone appends a different duplicate each round, growing live data.
                if cursor.set::<Cow<'_, [u8]>>(&key).unwrap().is_some() {
                    cursor.del(WriteFlags::NO_DUP_DATA).unwrap();
                }
                cursor.put(&key, &val, WriteFlags::UPSERT).unwrap();
            }
        }
