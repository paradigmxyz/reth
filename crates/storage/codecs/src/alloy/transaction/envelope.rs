...
macro_rules! impl_compact_tx_envelope {
    ($($name:ident$(<$($generic:ident),*>)?),+) => {
        $(
            use crate::Vec;
            use crate::alloy::transaction::envelope::{ToTxCompact, FromTxCompact};
            ...
            impl$(<$($generic:
                crate::Compact
                    ...
                    + Transaction
                    + Send
                    + Sync),*>)? crate::Compact for $name$(<$($generic),*>)? {
                fn to_compact<B>(&self, buf: &mut B) -> usize
                where
                    B: crate::BufMut + AsMut<[u8]>,
                {
                ...
                fn from_compact(mut buf: &[u8], _len: usize) -> (Self, &[u8]) {
                    ...
                    let (signature, buf) = alloy_primitives::PrimitiveSignature::from_compact(buf, sig_bit);
                    let (tx_type, buf) = <$name$(<$($generic),*>)? as FromTxCompact>::TxType::from_compact(buf, tx_bits);
...
pub(crate) use impl_compact_tx_envelope;

mod ethereum {
    use super::{impl_compact_tx_envelope, EthereumTxEnvelope};

    impl_compact_tx_envelope!(EthereumTxEnvelope<Eip4844>);
}
