...
///
/// Example usage:
/// * `#[reth_db_codec(rlp)]`: will implement `derive_arbitrary(rlp)` or `derive_arbitrary(compact, rlp)`, if `compact` is the `reth_db_codec`.
/// * `#[reth_db_codec(no_arbitrary)]`: will skip `derive_arbitrary` (both trait implementations and tests)
#[proc_macro_attribute]
#[rustfmt::skip]
#[allow(unreachable_code)]
pub fn reth_db_codec(args: TokenStream, input: TokenStream) -> TokenStream {
