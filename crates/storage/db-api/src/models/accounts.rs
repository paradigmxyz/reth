...
impl Decode for BlockNumberAddress {
    fn decode(value: &[u8]) -> Result<Self, DatabaseError> {
        ...
        let address = Address::from_slice(&value[8..]);
        Ok(Self((num, address)))
    }
}
...
mod tests {
    ...
    fn test_block_number_address() {
        let num = 1u64;
        let addr = address!("0xba5e000000000000000000000000000000000000");
        let key = BlockNumberAddress((num, addr));

        let mut bytes = [0u8; 28];
        bytes[..8].copy_from_slice(&num.to_be_bytes());
        bytes[8..].copy_from_slice(addr.as_slice());
