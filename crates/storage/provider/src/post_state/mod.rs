...
impl PostState {
    ...
    pub fn receipts_root(&self, block: BlockNumber) -> H256 {
        #[cfg(feature = "optimism")]
        {
            ...
            // the receipts root calculation does not include the deposit nonce in the receipt
            // encoding. This will be fixed in the next hardfork, however, for now, we must strip
            // the deposit nonce from the receipts before calculating the receipts root.
            let receipts = self
                .receipts(block)
                .iter()
                .map(|r| {
