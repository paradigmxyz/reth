...
macro_rules! delegate_impls_to_as_ref {
    (for $target:ty => $($trait:ident $(where [$($generics:tt)*])? {  $(fn $func:ident$(<$($generic_arg:ident: $generic_arg_ty:path),*>)?(&self, $($arg:ident: $argty:path),*) -> $ret:path;)* })* ) => {
    ...
    ($target:ty $(where [$($generics:tt)*])?) => {
        $crate::providers::state::macros::delegate_impls_to_as_ref!(
            ...
            BlockHashProvider $(where [$($generics)*])? {
                ...
                fn canonical_hashes_range<R: std::ops::RangeBounds<reth_primitives::BlockNumber>>(&self, range: R) -> reth_interfaces::Result<Vec<reth_primitives::H256>>;
            }
