    ...
    ($target:ty $(where [$($generics:tt)*])?) => {
        $crate::providers::state::macros::delegate_impls_to_as_ref!(
            ...
            StateProvider $(where [$($generics)*])?{
                ...
                fn proof(&self, address: reth_primitives::Address, keys: Vec<reth_primitives::H256>) -> reth_interfaces::Result<(Vec<reth_primitives::Bytes>, reth_primitives::H256, Vec<Vec<reth_primitives::Bytes>>)>;
            }
        );
    }
}
