...
impl<TX: DbTxMut + DbTx + 'static, N: NodeTypes> StorageTrieWriter for DatabaseProvider<TX, N> {
    ...
    fn write_storage_trie_updates_sorted<'a>(
        ...
        let mut num_entries = 0;
        let storage_trie_iter = storage_tries;
        let (lower, upper) = storage_trie_iter.size_hint();
        let mut storage_tries = Vec::with_capacity(upper.unwrap_or(lower));
        for (hashed_address, updates) in storage_trie_iter {
            if updates.is_empty() && !updates.is_deleted() {
                continue
            }
            storage_tries.push((hashed_address, updates));
        }
