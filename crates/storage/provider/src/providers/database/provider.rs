...
impl<TX: DbTxMut + DbTx> DatabaseProvider<TX> {
    ...
    /// the transaction ids.
    /// 2. Iterate over the [StorageChangeSets][tables::StorageChangeSet] table
    /// and the [AccountChangeSets][tables::AccountChangeSet] tables in reverse order to reconstruct
    /// the changesets.
...
impl<TX: DbTx> TransactionsProvider for DatabaseProvider<TX> {
    ...
    fn senders_by_tx_range(
        ...
        self.cursor_read_collect::<tables::TransactionSenders, _>(range, Ok).map_err(Into::into)
    }
