...
impl<TX: DbTx + 'static, N: NodeTypesForProvider> DatabaseProvider<TX, N> {
    ...
    /// [`tables::HashedAccounts`] and [`tables::HashedStorages`] tables, based on the given storage
    /// and account changesets.
    ...
    {
        // iterate previous value and get hashed state value to create changeset
    ...
    for DatabaseProvider<TX, N>
{
            ...
            for (block_index, mut storage_changes) in reverts.storage.into_iter().enumerate() {
                ...
                for PlainStorageRevert { address, wiped, storage_revert } in storage_changes {
                    ...
                    let mut storage = storage_revert
                        .into_iter()
                        .map(|(k, v)| (keccak256(B256::new(k.to_be_bytes())), v))
                        .collect::<Vec<_>>();
                    // sort storage slots by key.
                    storage.par_sort_unstable_by_key(|a| a.0);

                    // If we are writing the primary storage wipe transition, the pre-existing
                    // hashed storage state has to be taken from the database and written to
                    // storage history. See [StorageWipe::Primary] for more details.
                    //
    ...
    fn write_state_changes(&self, mut changes: StateChangeset) -> ProviderResult<()> {
        ...
        for PlainStorageChangeset { address, wipe_storage, storage } in changes.storage {
            ...
            // cast storages to hashed B256.
            let mut storage = storage
                .into_iter()
                .map(|(k, value)| StorageEntry { key: keccak256(B256::from(k)), value })
                .collect::<Vec<_>>();
            // sort storage slots by key.
    ...
    fn remove_state_above(&self, block: BlockNumber) -> ProviderResult<()> {
        ...
        // iterate over local hashed state remove all account and all storages.
