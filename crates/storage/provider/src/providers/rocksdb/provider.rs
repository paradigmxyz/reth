...
impl RocksDBProvider {
    ...
    fn write_account_history<N: reth_node_types::NodePrimitives>(
        ...
        for (block_idx, block) in blocks.iter().enumerate() {
            ...
            // Iterate raw account reverts directly. This matches the account entries emitted by
            // to_plain_state_reverts without cloning AccountInfo values.
    ...
    fn write_storage_history<N: reth_node_types::NodePrimitives>(
        ...
        for (block_idx, block) in blocks.iter().enumerate() {
            ...
            // Iterate raw storage reverts directly. This avoids building PlainStorageRevert
            // vectors when the history writer only needs touched slot keys.
            for storage_block_reverts in block.execution_outcome().state.reverts.iter() {
                for (address, revert) in storage_block_reverts {
                    if revert.wipe_storage || !revert.storage.is_empty() {
