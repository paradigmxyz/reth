...
impl RocksDBProvider {
    ...
    fn write_account_history<N: reth_node_types::NodePrimitives>(
        ...
        for (block_idx, block) in blocks.iter().enumerate() {
            ...
            // Only index accounts with actual account-info changes (RevertTo or DeleteIt),
            // matching the changeset entries written by the account changeset writer.
    ...
    fn write_storage_history<N: reth_node_types::NodePrimitives>(
        ...
        for (block_idx, block) in blocks.iter().enumerate() {
            ...
            // Index all storage slots with reverts, matching the changeset entries written
            // by the storage changeset writer.
            for tx_reverts in reverts.iter() {
                for (address, account_revert) in tx_reverts {
