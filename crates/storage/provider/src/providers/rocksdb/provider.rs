    ...
    fn write_account_history<N: reth_node_types::NodePrimitives>(
        ...
        for (block_idx, block) in blocks.iter().enumerate() {
            ...
            for account_block_reverts in block.execution_outcome().state.reverts.iter() {
                for (address, revert_account) in account_block_reverts {
                    if !matches!(revert_account.account, AccountInfoRevert::DoNothing) {
    ...
    fn write_storage_history<N: reth_node_types::NodePrimitives>(
        ...
        for (block_idx, block) in blocks.iter().enumerate() {
            ...
            for storage_block_reverts in block.execution_outcome().state.reverts.iter() {
                for (address, revert_account) in storage_block_reverts {
                    for slot in revert_account.storage.keys() {
