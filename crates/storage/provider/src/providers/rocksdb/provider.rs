...
impl RocksDBProvider {
    ...
    fn write_account_history<N: reth_node_types::NodePrimitives>(
        ...
        for (address, indices) in account_history {
            let last = *indices.last().expect("indices not empty");
            let key = ShardedKey::new(address, last);
            let list = BlockNumberList::new_pre_sorted(indices);
            batch.put::<tables::AccountsHistory>(key, &list)?;
        }
        ctx.pending_batches.lock().push(batch.into_inner());
        Ok(())
    }
    ...
    fn write_storage_history<N: reth_node_types::NodePrimitives>(
        ...
        for ((address, storage_key), indices) in storage_history {
            let last = *indices.last().expect("indices not empty");
            let key = StorageShardedKey::new(address, storage_key, last);
            let list = BlockNumberList::new_pre_sorted(indices);
            batch.put::<tables::StoragesHistory>(key, &list)?;
        }
        ctx.pending_batches.lock().push(batch.into_inner());
        Ok(())
    }
