use crate::{
    test_utils::blocks::BlockChainTestData,
    ...
    fn default() -> MockEthProvider {
        MockEthProvider {
            blocks: Arc::new(Mutex::new(
                BlockChainTestData::default()
                    .blocks
                    .iter()
                    .map(|sealed_block_with_senders| {
                        let block = sealed_block_with_senders.0.block.clone().unseal();
                        (block.header.parent_hash, block)
                    })
                    .collect::<HashMap<_, _>>(),
            )),
            headers: Arc::new(Mutex::new(
                BlockChainTestData::default()
                    .blocks
                    .iter()
                    .map(|sealed_block_with_senders| {
...
//     // println!("mock_provider: {:?}", mock_provider.transaction_by_hash(H256::zero()));

//     let mocked_blocks = BlockChainTestData::default().blocks;

//     // Créez un HashMap à partir de vos blocs mockés
//     let blocks_map: HashMap<H256, Block> = mocked_blocks
//         .iter()
//         .map(|sealed_block_with_senders| {
//             let block = sealed_block_with_senders.0.block.clone().unseal();
//             (block.header.parent_hash, block)
...
//     // let a = MockEthProvider {
//     //     blocks: Arc::new(Mutex::new(blocks_map)),
//     //     headers: Default::default(), // Initialisez les autres champs selon vos besoins
//     //     accounts: Default::default(),
//     //     chain_spec: Arc::new(reth_primitives::ChainSpecBuilder::mainnet().build()),
//     // };
...
//     println!("mock_provider: {:?}", a.blocks);
// }
