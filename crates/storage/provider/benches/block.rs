...
fn block_provider_insert(c: &mut Criterion) {
    ...
    let block_strategy = proptest::arbitrary::any::<SealedBlock>().prop_flat_map(|sealed_block| {
        proptest::collection::vec(proptest::arbitrary::any::<Address>(), sealed_block.body.len())
            .prop_map(move |senders| {
    ...
    for &size in &block_amounts {
        ...
        group.bench_with_input(BenchmarkId::new("Insert", size), &size, |b, &size| {
            let blocks: Vec<SealedBlockWithSenders> =
                prop::collection::vec(block_strategy.clone(), size)
                    .new_tree(&mut runner)
                    .unwrap()
                    .current();
