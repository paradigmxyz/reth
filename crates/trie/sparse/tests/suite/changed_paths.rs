...
pub(super) fn test_changed_paths_skip_inserted_branch_on_branch_short_key_split<T: SparseTrie>(
    ...
    let mut updates = B256Map::from_iter([(split_key, LeafUpdate::Changed(vec![0x04; 64]))]);
    ...
    assert!(contains_changed_path(&changed_paths, Nibbles::from_nibbles([0x01, 0x02, 0x03])));
    assert!(contains_changed_path(&changed_paths, Nibbles::from_nibbles([0x01, 0x02, 0x04])));
    assert!(!contains_changed_path(&changed_paths, Nibbles::default()));
}
