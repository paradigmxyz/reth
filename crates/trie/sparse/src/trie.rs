...
mod tests {
        ...
        while let Some(node) = node_iter.try_next().unwrap() {
            match node {
                ...
                TrieElement::Leaf(key, account) => {
                    let account = from_account_to_trie_account(account, EMPTY_ROOT_HASH);
    ...
    fn sparse_trie_empty_update_one() {
        ...
        let value_encoded = || {
            let mut account_rlp = Vec::new();
            from_account_to_trie_account(value(), EMPTY_ROOT_HASH).encode(&mut account_rlp);
            account_rlp
        };
    ...
    fn sparse_trie_empty_update_repeated() {
        ...
        let old_value_encoded = {
            let mut account_rlp = Vec::new();
            from_account_to_trie_account(old_value, EMPTY_ROOT_HASH).encode(&mut account_rlp);
            account_rlp
        };
        let new_value = Account { nonce: 2, ..Default::default() };
        let new_value_encoded = {
            let mut account_rlp = Vec::new();
            from_account_to_trie_account(new_value, EMPTY_ROOT_HASH).encode(&mut account_rlp);
            account_rlp
        };
