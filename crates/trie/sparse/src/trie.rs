use crate::{
    ...
    LeafLookup, LeafLookupError, LeafUpdate, PruneTrieStats, SparseTrie as SparseTrieTrait,
...
impl SparseTrieExt for SerialSparseTrie {
    ...
    fn prune_preserving(
        ...
        let mut stack: Vec<(Nibbles, usize)> = Vec::new();
        stack.push((Nibbles::default(), 0));

        while let Some((path, depth)) = stack.pop() {
            let Some(node) = self.nodes.get(&path) else { continue };

            // Get children to visit from current node
            let children: Vec<Nibbles> = match node {
                SparseNode::Empty | SparseNode::Hash(_) | SparseNode::Leaf { .. } => Vec::new(),
                SparseNode::Extension { key, .. } => {
                    let mut child = path;
                    child.extend(key);
                    vec![child]
            ...
            for child in children {
                if depth == max_depth {
                    ...
                    // Check if child has a computed hash and can be replaced with Hash stub
                    let hash =
        ...
        // Remove all nodes that are strict descendants of pruned roots
        self.nodes.retain(|path, _| {
            !effective_pruned_roots.iter().any(|(root, _)| path.starts_with(root) && path != root)
        });

        // Remove values that are descendants of pruned roots
        self.values.retain(|path, _| {
    ...
    fn update_leaves(
        ...
        for key in keys {
            ...
            match update {
                LeafUpdate::Changed(value) => {
                    if value.is_empty() {
                        ...
                        match self.remove_leaf(&full_path, NoRevealProvider) {
                            Ok(()) => {}
                            Err(e) => {
                                if let reth_execution_errors::SparseTrieErrorKind::BlindedNode {
                                    path,
                                    ..
                                } = e.kind()
                                {
                                    let min_len = path.len() as u8;
                ...
                LeafUpdate::Touched => {
                    // Touched is read-only: check if path is accessible
                    if let Err(LeafLookupError::BlindedNode { path, .. }) =
