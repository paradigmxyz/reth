...
impl ActivityGuard {
    ...
    fn start(
        ...
        let span = tracing::trace_span!(target: "engine::tree::activity", "trie_activity",
            ...
            storage_in_flight = tracing::field::Empty, proof_batches = tracing::field::Empty,
            pending_accounts = tracing::field::Empty, draining = tracing::field::Empty, wake = tracing::field::Empty,
    ...
    /// Records the outstanding dependencies when entering the coordinator receive.
    pub fn dependencies(&self, storage: usize, proofs: usize, accounts: usize, draining: bool) {
        if let Some(active) = &self.active {
            active.span.record("storage_in_flight", storage);
            active.span.record("proof_batches", proofs);
            active.span.record("pending_accounts", accounts);
            active.span.record("draining", draining);
        }
    }

    /// Records which incoming stream ended a receive interval.
    pub fn wake(&self, kind: &'static str) {
        if let Some(active) = &self.active {
            active.span.record("wake", kind);
        }
    }
