...
use reth_trie_common::{
    ...
    ProofTrieNode, RlpNode, StorageMultiProof, TrieAccount, TrieNode, EMPTY_ROOT_HASH,
    ...
    S: SparseTrieInterface + Default + Clone,
{
    ...
    fn revealed_trie_mut(
        ...
        match self.state {
            SparseTrie::Blind(_) => {
                let (root_node, masks) = provider_factory
                    .account_node_provider()
                    .trie_node(&Nibbles::default())?
                    .map(|node| {
                        TrieNode::decode(&mut &node.node[..]).map(|decoded| (decoded, node.masks))
                    })
                    .transpose()?
                    .unwrap_or((TrieNode::EmptyRoot, None));
