    ...
    S: SparseTrieTrait + SparseTrieExt + Default + Clone,
{
    ...
    pub fn prune_preserving(&mut self, config: &crate::hot_accounts::SmartPruneConfig<'_>) {
        ...
        #[cfg(feature = "metrics")]
        {
            self.metrics.prune.skipped_modified.record(stats.skipped_modified as f64);
...
mod tests {
    ...
    mod prune_preserving_tests {
        use super::*;
        use crate::{hot_accounts::SmartPruneConfig, HotAccounts};
        use alloy_primitives::keccak256;

        fn create_storage_multiproof(slot: B256, value: U256) -> StorageMultiProof {
            let slot_path = Nibbles::unpack(slot);
            let leaf_value: Vec<u8> = alloy_rlp::encode_fixed_size(&value).to_vec();
            let leaf_node = TrieNode::Leaf(LeafNode::new(slot_path, leaf_value));
            let leaf_encoded = alloy_rlp::encode(&leaf_node);
            let root = keccak256(&leaf_encoded);

            StorageMultiProof {
                root,
                subtree: ProofNodes::from_iter([(Nibbles::default(), leaf_encoded.into())]),
                branch_node_masks: BranchNodeMasksMap::default(),
            }
        }

        fn setup_sparse_with_storage_tries(
            addresses: &[B256],
        ) -> SparseStateTrie<SerialSparseTrie, SerialSparseTrie> {
            let mut sparse = SparseStateTrie::<SerialSparseTrie, SerialSparseTrie>::default();

            for (i, address) in addresses.iter().enumerate() {
                let slot = B256::from_slice(&[i as u8 + 1; 32]);
                let value = U256::from(i as u64 + 1);
                let storage_proof = create_storage_multiproof(slot, value);

                // Reveal storage trie for this account
                sparse
                    .reveal_decoded_storage_multiproof(*address, storage_proof.try_into().unwrap())
                    .unwrap();
            }
        ...
        #[test]
        fn prune_preserving_cold_accounts_evicted() {
            // Setup: 3 cold accounts (not in any hot tier)
            let cold_addr_1 = B256::repeat_byte(0xAA);
            let cold_addr_2 = B256::repeat_byte(0xBB);
            let cold_addr_3 = B256::repeat_byte(0xCC);

            let mut sparse =
                setup_sparse_with_storage_tries(&[cold_addr_1, cold_addr_2, cold_addr_3]);

            // Verify all storage tries are present
            assert!(sparse.storage_trie_ref(&cold_addr_1).is_some());
            assert!(sparse.storage_trie_ref(&cold_addr_2).is_some());
            assert!(sparse.storage_trie_ref(&cold_addr_3).is_some());

            // Create config with budget of 1 (should evict 2 of the 3 cold tries)
            let hot_accounts = HotAccounts::new();
            let config = SmartPruneConfig::new(2, 1, &hot_accounts);

            // Prune
            sparse.prune_preserving(&config);

            // Count remaining tries - only 1 should remain (budget is 1, no hot accounts)
            let remaining = [cold_addr_1, cold_addr_2, cold_addr_3]
                .iter()
                .filter(|addr| sparse.storage_trie_ref(addr).is_some())
                .count();

            assert_eq!(remaining, 1, "Should have 1 storage trie remaining (budget is 1)");
        }

        #[test]
        fn prune_preserving_always_hot_accounts_preserved() {
            // Setup: 1 hot account (system contract) + 2 cold accounts
            let system_contract_addr =
                keccak256(alloy_primitives::address!("0x000F3df6D732807Ef1319fB7B8bB8522d0Beac02"));
            let cold_addr_1 = B256::repeat_byte(0xAA);
            let cold_addr_2 = B256::repeat_byte(0xBB);

            let mut sparse =
                setup_sparse_with_storage_tries(&[system_contract_addr, cold_addr_1, cold_addr_2]);

            // Use mainnet hot accounts which includes the beacon roots contract
            let hot_accounts = HotAccounts::for_mainnet();
            let config = SmartPruneConfig::new(2, 1, &hot_accounts);

            // Prune with budget of 1 - hot account should stay, cold accounts fight for remaining
            sparse.prune_preserving(&config);

            // System contract (always hot) must be preserved
            assert!(
                sparse.storage_trie_ref(&system_contract_addr).is_some(),
                "System contract storage trie should be preserved (Tier A - always hot)"
            );
        }

        #[test]
        fn prune_preserving_primary_tier_preserved() {
            // Setup: 1 known builder (Tier B/primary) + 2 cold accounts
            let builder_addr =
                keccak256(alloy_primitives::address!("0x95222290DD7278Aa3Ddd389Cc1E1d165CC4BAfe5")); // Beaverbuild
            let cold_addr_1 = B256::repeat_byte(0xAA);
            let cold_addr_2 = B256::repeat_byte(0xBB);

            let mut sparse =
                setup_sparse_with_storage_tries(&[builder_addr, cold_addr_1, cold_addr_2]);

            // Use mainnet hot accounts which includes known builders
            let hot_accounts = HotAccounts::for_mainnet();
            let config = SmartPruneConfig::new(2, 1, &hot_accounts);
            ...
            // Builder (Tier B/primary) must be preserved
            assert!(
                sparse.storage_trie_ref(&builder_addr).is_some(),
                "Known builder storage trie should be preserved (Tier B - primary)"
            );
        }

        #[test]
        fn prune_preserving_secondary_tier_preserved() {
            // Setup: 1 fee recipient (Tier B/secondary) + 2 cold accounts
            let fee_recipient = B256::repeat_byte(0xFE);
            let cold_addr_1 = B256::repeat_byte(0xAA);
            let cold_addr_2 = B256::repeat_byte(0xBB);

            let mut sparse =
                setup_sparse_with_storage_tries(&[fee_recipient, cold_addr_1, cold_addr_2]);

            // Create tracker and record fee recipient
            let mut hot_accounts = HotAccounts::new();
            hot_accounts.record_fee_recipient(fee_recipient);
            ...
            // Fee recipient (Tier B/secondary) must be preserved
            assert!(
                sparse.storage_trie_ref(&fee_recipient).is_some(),
                "Fee recipient storage trie should be preserved (Tier B - secondary)"
            );
        }

        #[test]
        fn prune_preserving_non_hot_accounts_pruned() {
            // Setup: 2 hot accounts + 3 cold accounts with budget of 3
            let system_contract =
                keccak256(alloy_primitives::address!("0x000F3df6D732807Ef1319fB7B8bB8522d0Beac02"));
            let builder =
                keccak256(alloy_primitives::address!("0x95222290DD7278Aa3Ddd389Cc1E1d165CC4BAfe5"));
            let cold_addr_1 = B256::repeat_byte(0xAA);
            let cold_addr_2 = B256::repeat_byte(0xBB);
            let cold_addr_3 = B256::repeat_byte(0xCC);

            let mut sparse = setup_sparse_with_storage_tries(&[
                system_contract,
                builder,
                cold_addr_1,
                cold_addr_2,
            ...
            let hot_accounts = HotAccounts::for_mainnet();
            // Budget of 3: hot accounts (2) fill up first, only 1 slot for cold
            let config = SmartPruneConfig::new(2, 3, &hot_accounts);

            // Prune
            sparse.prune_preserving(&config);

            // Hot accounts must be preserved
            assert!(
                sparse.storage_trie_ref(&system_contract).is_some(),
                "System contract should be preserved"
            );
            assert!(sparse.storage_trie_ref(&builder).is_some(), "Builder should be preserved");

            // Count remaining cold accounts - should be at most 1 (3 budget - 2 hot = 1 slot)
            let cold_remaining = [cold_addr_1, cold_addr_2, cold_addr_3]
                .iter()
                .filter(|addr| sparse.storage_trie_ref(addr).is_some())
                .count();
            ...
            assert!(
                cold_remaining <= 1,
                "At most 1 cold account should remain (budget 3 - 2 hot = 1 slot)"
            );
        }

        #[test]
        fn prune_preserving_hot_accounts_exceed_budget() {
            // Setup: 3 hot accounts with budget of 2 - all hot should still be preserved
            let system1 =
                keccak256(alloy_primitives::address!("0x000F3df6D732807Ef1319fB7B8bB8522d0Beac02")); // Beacon roots
            let system2 =
                keccak256(alloy_primitives::address!("0x0000F90827F1C53a10cb7A02335B175320002935")); // Block hash history
            let builder =
                keccak256(alloy_primitives::address!("0x95222290DD7278Aa3Ddd389Cc1E1d165CC4BAfe5")); // Beaverbuild
            let cold_addr = B256::repeat_byte(0xAA);

            let mut sparse =
                setup_sparse_with_storage_tries(&[system1, system2, builder, cold_addr]);

            let hot_accounts = HotAccounts::for_mainnet();
            // Budget of 2, but 3 hot accounts - all hot should still be preserved
            let config = SmartPruneConfig::new(2, 2, &hot_accounts);

            // Prune
            sparse.prune_preserving(&config);

            // All hot accounts must be preserved even though they exceed budget
            assert!(
                sparse.storage_trie_ref(&system1).is_some(),
                "System contract 1 should be preserved"
            );
            assert!(
                sparse.storage_trie_ref(&system2).is_some(),
                "System contract 2 should be preserved"
            );
            assert!(sparse.storage_trie_ref(&builder).is_some(), "Builder should be preserved");

            // Cold account should be evicted (budget exceeded by hot accounts)
            assert!(
                sparse.storage_trie_ref(&cold_addr).is_none(),
                "Cold account should be evicted when hot accounts exceed budget"
            );
        }
    }
}
