...
proptest! {
    ...
    fn fuzz_in_memory_account_nodes(mut init_state: BTreeMap<B256, U256>, state_updates: [BTreeMap<B256, Option<U256>>; 10]) {
        ...
        let (_, mut trie_nodes) = <StateRoot<_, _> as DatabaseStateRoot<_>>::from_provider(&provider)
            .root_with_updates()
            .unwrap();
        ...
        for state_update in state_updates {
            ...
            let (state_root, trie_updates) = <StateRoot<_, _> as DatabaseStateRoot<_>>::from_provider(&provider)
    ...
    fn fuzz_in_memory_storage_nodes(mut init_storage: BTreeMap<B256, U256>, storage_updates: [(bool, BTreeMap<B256, U256>); 10]) {
        ...
        let (_, _, mut storage_trie_nodes) =
            <StorageRoot<_, _> as DatabaseStorageRoot<_>>::from_provider_hashed(&provider, hashed_address).root_with_updates().unwrap();
        ...
        for (is_deleted, mut storage_update) in storage_updates {
            ...
            let (storage_root, _, trie_updates) =
                <StorageRoot<_, _> as DatabaseStorageRoot<_>>::from_provider_hashed(&provider, hashed_address)
