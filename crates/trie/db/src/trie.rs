//! Database integration with merkle-patricia tree.

use crate::{prefix_set::PrefixSetLoader, TxRefWrapper};
use reth_db_api::transaction::DbTx;
use reth_execution_errors::StateRootError;
use reth_primitives::{Address, BlockNumber, B256};
#[cfg(feature = "metrics")]
use reth_trie::metrics::{TrieRootMetrics, TrieType};
use reth_trie::{updates::TrieUpdates, StateRoot, StateRootProgress, StorageRoot};
use std::ops::RangeInclusive;
use tracing::debug;

type StateRootWithTx<'a, TX> = StateRoot<TxRefWrapper<'a, TX>, TxRefWrapper<'a, TX>>;

/// A trait used for creating [`StateRoot`] from a [transaction].
///
/// [transaction]: DbTx
pub trait StateRootDb<'a, TX: DbTx> {
    /// Create a new [`StateRoot`] instance from `tx`.
    ...
    ) -> Result<StateRootWithTx<'a, TX>, StateRootError> {
        let loaded_prefix_sets = PrefixSetLoader::new(tx).load(range)?;
        Ok(StateRootWithTx::from_tx(tx).with_prefix_sets(loaded_prefix_sets))
    }
...
impl<'a, TX: DbTx> StateRootDb<'a, TX> for StateRoot<TxRefWrapper<'a, TX>, TxRefWrapper<'a, TX>> {
    /// Create a new [`StateRoot`] instance.
    fn from_tx(tx: &'a TX) -> Self {
        Self::new(TxRefWrapper::from(tx), TxRefWrapper::from(tx))
    }
}

/// Creates [`StorageRoot`] from a database [transaction].
///
/// [transaction]: DbTx
pub trait StorageRootDb<'a, TX: DbTx> {
...
impl<'a, TX: DbTx> StorageRootDb<'a, TX>
    for StorageRoot<TxRefWrapper<'a, TX>, TxRefWrapper<'a, TX>>
{
    /// Create a new storage root calculator from database transaction and raw address.
    fn from_tx(tx: &'a TX, address: Address) -> Self {
        Self::new(
...
mod tests {
    use super::*;
    use crate::{BranchNodeCompact, HashBuilder, Nibbles, TrieMask};
    ...
    use reth_primitives::{
        constants::EMPTY_ROOT_HASH, hex_literal::hex, keccak256, Account, StorageEntry, U256,
    };
    ...
    fn incremental_vs_full_root(inputs: &[&str], modified: &str) {
        ...
        trie_updates.write_to_database(&TxRefWrapper(tx.tx_ref()), hashed_address).unwrap();
    ...
    fn account_and_storage_trie() {
...

        {
            ...
            assert_eq!(root, computed_expected_root);
            assert_eq!(
                trie_updates.account_nodes_ref().len() + trie_updates.removed_nodes().len(),
                1
            );
    ...
    fn account_trie_around_extension_node_with_dbtrie() {
        ...
        let (got, updates) = StateRoot::from_tx(tx.tx_ref()).root_with_updates().unwrap();
        assert_eq!(expected, got);
        updates.write_to_database(&TxRefWrapper::from(tx.tx_ref())).unwrap();
    ...
    proptest! {
        ...
        fn fuzz_state_root_incremental(account_changes: [BTreeMap<B256, U256>; 5]) {
            ...
            for accounts in account_changes {
                ...
                trie_updates.write_to_database(&TxRefWrapper(tx.tx_ref())).unwrap();
            }
        }
    }
