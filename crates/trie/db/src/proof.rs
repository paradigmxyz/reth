...
use crate::TxRefWrapper;
use reth_db_api::transaction::DbTx;
use reth_trie::proof::Proof;

/// A trait for creating [`Proof`] from a [transaction].
///
/// [transaction]: DbTx
pub trait ProofDb<'a, TX: DbTx> {
    /// Create a new instance from `tx`.
    fn from_tx(tx: &'a TX) -> Self;
}

impl<'a, TX: DbTx> ProofDb<'a, TX> for Proof<TxRefWrapper<'a, TX>, TxRefWrapper<'a, TX>> {
    fn from_tx(tx: &'a TX) -> Self {
...
mod tests {
    use super::*;
    use crate::{proof::ProofDb, trie::StateRootDb};
    ...
    use reth_trie::{Nibbles, StateRoot};
