...
impl PrefixSetLocal {
    ...
    /// if the given prefix is a prefix of any key in the set.
    #[inline]
    pub fn contains(&self, prefix: &[u8], index: &mut usize) -> bool {
        if self.all {
            return true
        }

        while *index > 0 && &self.keys[*index] > prefix {
            *index -= 1;
        }

        for (idx, key) in self.keys[*index..].iter().enumerate() {
            if key.has_prefix(prefix) {
                *index += idx;
                return true
            }
...
impl From<PrefixSet> for PrefixSetLocal {
    fn from(prefix_set: PrefixSet) -> Self {
        Self { keys: prefix_set.keys, all: prefix_set.all }
    }
}

impl<'a> IntoIterator for &'a PrefixSetLocal {
    type IntoIter = std::slice::Iter<'a, reth_trie_common::Nibbles>;
    type Item = &'a reth_trie_common::Nibbles;
    fn into_iter(self) -> Self::IntoIter {
        self.iter()
