...
pub struct StateRoot<T, H> {
    ...
    /// Whether every child under a changed branch path should be walked.
...
impl<T, H> StateRoot<T, H> {
    ...
    /// Configures the state root walker to visit every child of changed branch paths.
...
impl<T, H> StorageRoot<T, H> {
    ...
    /// Configures the storage root walker to visit every child of changed branch paths.
    pub const fn with_walk_all_changed_branch_children(mut self, enabled: bool) -> Self {
        self.walk_all_changed_branch_children = enabled;
        self
    }
