            ...
            match node {
                ...
                TrieElement::Leaf(hashed_address, account) => {
                    ...
                    if retain_updates && total_updates_len as u64 >= self.threshold {
                        ...
                        trie_updates.changed_nodes.extend(walker_deleted_keys.into_iter().map(|k| (k, None)));
                        let (hash_builder, hash_builder_updates) = hash_builder.split();
                        trie_updates.changed_nodes.extend(hash_builder_updates.into_iter().map(|(k, v)| (k, Some(v))));
