...
impl MultiProofTargetsV2 {
    /// Creates an empty target set with capacity for account and storage target groups.
...
impl Iterator for ChunkedMultiProofTargetsV2 {
    ...
    fn next(&mut self) -> Option<Self::Item> {
        let account_capacity = self.account_targets.len().min(self.size);
        let storage_capacity = (self.storage_targets.len() +
            usize::from(self.current_account_storage.is_some()))
        .min(self.size);
        let mut chunk = MultiProofTargetsV2::with_capacity(account_capacity, storage_capacity);
        let mut count = 0;
