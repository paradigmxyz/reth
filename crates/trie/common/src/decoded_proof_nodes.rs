use alloy_trie::{HashMap, Nibbles};
...
pub struct DecodedProofNodes(HashMap<Nibbles, Bytes>);
...
impl FromIterator<(Nibbles, Bytes)> for DecodedProofNodes {
    fn from_iter<T: IntoIterator<Item = (Nibbles, Bytes)>>(iter: T) -> Self {
        Self(HashMap::from_iter(iter))
    }
}

impl Extend<(Nibbles, Bytes)> for DecodedProofNodes {
