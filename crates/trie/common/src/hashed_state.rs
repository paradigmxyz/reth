...
impl HashedPostState {
    ...
    pub fn from_bundle_state<'a, KH: KeyHasher>(
        ...
        state
            ...
            .map(|(address, account)| {
                ...
                let hashed_storage = (!storage.is_empty()).then_some(storage);
    ...
    pub fn construct_prefix_sets(&self) -> TriePrefixSetsMut {
        ...
        TriePrefixSetsMut { account_prefix_set, storage_prefix_sets }
    }
        ...
        self.storages.retain(|&address, storage| {
            ...
            let (retain, storage_not_in_targets) = match targets.get(&address) {
                Some(storage_in_targets) => {
                    ...
                    (retain, (!storage_not_in_targets.is_empty()).then_some(storage_not_in_targets))
                }
    ...
    pub fn extend_from_sorted(&mut self, sorted: &HashedPostStateSorted) {
        ...
        for (hashed_address, sorted_storage) in &sorted.storages {
            match self.storages.entry(*hashed_address) {
                hash_map::Entry::Vacant(entry) => {
                    let mut new_storage = HashedStorage::new();
...
impl HashedStorage {
    ...
    pub fn from_plain_storage<'a>(storage: impl IntoIterator<Item = (&'a U256, &'a U256)>) -> Self {
        storage.into_iter().map(|(key, value)| (keccak256(B256::from(*key)), *value)).collect()
    }
...
impl FromIterator<(B256, U256)> for HashedStorage {
    fn from_iter<T: IntoIterator<Item = (B256, U256)>>(iter: T) -> Self {
...
impl HashedPostStateSorted {
    ...
    pub fn merge_slice<T: AsRef<Self>>(items: &[T]) -> Self {
        ...
        struct StorageAcc<'a>(Vec<&'a [(B256, U256)]>);
        ...
        for item in items {
            for (addr, storage) in &item.as_ref().storages {
                acc.entry(*addr)
                    .or_insert_with(|| StorageAcc(Vec::new()))
                    .0
                    .push(storage.storage_slots.as_slice());
            }
        }
        ...
        let storages = acc
            .into_iter()
            .map(|(addr, entry)| {
                let storage_slots = kway_merge_sorted(entry.0);
...
impl HashedStorageSorted {
    ...
    ///
    /// Nodes are merged with `other`'s values taking precedence for duplicates.
    pub fn extend_ref(&mut self, other: &Self) {
    ...
    pub fn merge_batch<'a>(updates: impl IntoIterator<Item = &'a Self>) -> Self {
        ...
        let storage_slots = kway_merge_sorted(updates.iter().map(|u| u.storage_slots.as_slice()));
...
/// targets.
///
/// For each account, chunks with storage updates come first, followed by account updates.
#[derive(Debug)]
...
mod tests {
    ...
    fn test_hashed_post_state_with_storages() {
        ...
        let storage_1 = (keccak256(address_1), HashedStorage::new());
        let storage_2 =
            (keccak256(address_2), HashedStorage::from_iter([(B256::ZERO, U256::ZERO)]));
    ...
    fn test_chunks_ordering_guarantee() {
        // Test that chunks preserve the ordering: storage updates -> account
        ...
        // Should have 3 chunks: 2 storage updates + 1 account
        assert_eq!(chunks.len(), 3);

        // First two chunks must be storage updates.
        assert!(chunks[0].accounts.is_empty());
        assert_eq!(chunks[0].storages.get(&addr).unwrap().storage.len(), 1);
        ...
        assert_eq!(chunks[2].accounts.len(), 1);
        assert!(chunks[2].accounts.contains_key(&addr));
        assert!(chunks[2].storages.is_empty());
    }
    ...
    fn test_hashed_storage_sorted_extend_ref() {
        ...
        assert_eq!(storage3.storage_slots.len(), 4);
        assert_eq!(storage3.storage_slots[2].0, B256::from([3; 32]));
        assert_eq!(storage3.storage_slots[2].1, U256::from(30));
        assert_eq!(storage3.storage_slots[3].0, B256::from([4; 32]));
        assert_eq!(storage3.storage_slots[3].1, U256::ZERO);
    }
    ...
    /// Test sorted storage values override existing values.
    #[test]
    fn test_hashed_storage_extend_from_sorted_override() {
        let slot1 = B256::random();
        let slot2 = B256::random();
        ...
        let sorted = HashedStorageSorted {
            storage_slots: vec![(slot1, U256::from(200)), (slot2, U256::from(300))],
        };

        storage.extend_from_sorted(&sorted);

        assert_eq!(storage.storage.len(), 2);
        assert_eq!(storage.storage.get(&slot1), Some(&U256::from(200)));
        assert_eq!(storage.storage.get(&slot2), Some(&U256::from(300)));
    }
