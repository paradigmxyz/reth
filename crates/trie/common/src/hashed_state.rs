...
impl ChunkedHashedPostState {
    fn new(mut hashed_post_state: HashedPostState, size: usize) -> Self {
        ...
        for (address, storages) in hashed_post_state.storages {
            // if the storage is empty then we want to include the account proof only, so we
            // delay adding to the flattened vec
            if !storages.storage.is_empty() || storages.wiped {
                let account = hashed_post_state.accounts.remove(&address);

                // we should be pushing a storage wipe first
                if storages.wiped {
                    flattened.push((address, account, FlattenedHashedPostStateItem::StorageWipe));
                }
...
mod tests {
    ...
    fn test_wiped_account_across_chunk_boundary() {
        ...
        let state = HashedPostState {
            ...
            storages: B256Map::from_iter([(
                addr,
                HashedStorage { wiped: true, storage: B256Map::default() },
            )]),
        };
        ...
        assert_eq!(
            ...
            Some(HashedPostState {
                ...
                storages: B256Map::from_iter([(
                    addr,
                    HashedStorage { wiped: false, storage: B256Map::default() }
                )])
            })
        );
