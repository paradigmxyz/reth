...
struct AvailabilitySheet {
    ...
    /// Approximate number of idle workers.
    ///
    /// This mirrors the flags so dispatch can answer `has_multiple_idle` with one atomic load
    /// instead of scanning every worker flag on every chunking decision.
