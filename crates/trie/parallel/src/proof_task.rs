    ...
    Provider: TrieCursorFactory + HashedCursorFactory,
{
    ...
    fn compute_legacy_storage_proof(
        ...
        let multi_added_removed_keys = multi_added_removed_keys
            .unwrap_or_else(|| Arc::new(RwLock::new(MultiAddedRemovedKeys::new())));
        let multi_added_removed_keys = multi_added_removed_keys.read();
...
{
    let multi_added_removed_keys = ctx.multi_added_removed_keys.as_ref().map(|keys| keys.read());
    let accounts_added_removed_keys = multi_added_removed_keys.as_ref().map(|keys| keys.get_accounts());
...
pub struct AccountMultiproofInput {
    ...
    /// Provided by the user to give the necessary context to retain extra proofs.
    pub multi_added_removed_keys: Option<Arc<RwLock<MultiAddedRemovedKeys>>>,
    /// Context for sending the proof result.
...
struct AccountMultiproofParams<'a> {
    ...
    /// Provided by the user to give the necessary context to retain extra proofs.
    multi_added_removed_keys: Option<&'a Arc<RwLock<MultiAddedRemovedKeys>>>,
