...
//!
//! When the `debug-jitter` feature is enabled, proof workers can optionally add
//! random delays after completing jobs. This helps trigger out-of-order timing bugs
//! that may only manifest in real-world conditions.
//!
//! Control via environment variable:
//! - `RETH_PROOF_WORKER_JITTER_MS`: Maximum jitter in milliseconds (0-N random delay)
//!
//! Example: `RETH_PROOF_WORKER_JITTER_MS=5` adds 0-5ms random delays.
...
fn get_jitter_config() -> Option<u64> {
    *JITTER_CONFIG.get_or_init(|| {
        std::env::var("RETH_PROOF_WORKER_JITTER_MS")
            .ok()
...
///
/// When `RETH_PROOF_WORKER_JITTER_MS` is set to a positive value N,
...
pub(crate) fn maybe_apply_jitter(worker_id: usize, job_type: &str) {
