...
mod tests {
    ...
    fn test_remove_leaf_remaining_child_needs_reveal() {
        ...
        provider.add_revealed_node(
            Nibbles::from_nibbles([0x1]),
            RevealedNode { node: encoded.into(), masks: None },
        );
    ...
    fn test_remove_leaf_remaining_extension_node_child_is_revealed() {
        ...
        provider.add_revealed_node(
            ...
            RevealedNode {
                node: encoded.into(),
                // Give it a fake mask so that it appears like it will be stored in the db
                masks: Some(BranchNodeMasks {
                    tree_mask: TrieMask::default(),
                    hash_mask: TrieMask::new(0b1111),
                }),
            },
        );
        ...
        let revealed_nodes: Vec<ProofTrieNode> = hash_builder_proof_nodes
            ...
            .map(|(path, node)| {
                ...
                ProofTrieNode {
                    ...
                    masks: masks_from_options(hash_mask, tree_mask),
                }
            })
            .collect();
