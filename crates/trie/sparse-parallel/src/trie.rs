            ...
            for child in children {
                if depth == max_depth {
                    ...
                    if let Some(hash) = hash {
                        // Use untracked version to avoid marking subtrie as modified during prune
...
mod tests {
    ...
    fn test_prune_extension_node_depth_semantics() {
        ...
        let root_before = trie.root();
        // With `depth >= max_depth` heat check, nodes at depth 1 in hot Lower subtries
        // are protected. Heat accumulates from multiple operations during setup.
        // Need enough cycles for heat to fully decay before pruning can occur.
        for _ in 0..10 {
            trie.prune(1);
        }
