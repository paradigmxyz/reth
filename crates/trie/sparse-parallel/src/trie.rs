...
mod tests {
    ...
    fn sparse_trie_fuzz_regression_test() {
        let test_data: Vec<BTreeMap<Nibbles, Account>> = vec![
            BTreeMap::from_iter([(
                Nibbles::from_nibbles_unchecked([vec![0xd, 0x4, 0x7], vec![0x0; 61]].concat()),
                Account { nonce: 0, balance: U256::ZERO, bytecode_hash: None },
            )]),
            BTreeMap::from_iter([(
                Nibbles::from_nibbles_unchecked([vec![0xd, 0xd, 0xc], vec![0x0; 61]].concat()),
                Account { nonce: 0, balance: U256::ZERO, bytecode_hash: None },
            )]),
            BTreeMap::from_iter([(
                Nibbles::from_nibbles_unchecked([vec![0xd, 0xd, 0x5], vec![0x0; 61]].concat()),
                Account { nonce: 0, balance: U256::ZERO, bytecode_hash: None },
            )]),
            BTreeMap::from_iter([(
                Nibbles::from_nibbles_unchecked([vec![0xd, 0xd, 0x5], vec![0x0; 61]].concat()),
                Account { nonce: 0, balance: U256::ONE, bytecode_hash: None },
            )]),
        ];
