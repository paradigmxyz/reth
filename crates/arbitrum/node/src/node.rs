    ...
    N: FullNodeTypes<Types: NodeTypes<ChainSpec = ChainSpec, Primitives = reth_arbitrum_primitives::ArbPrimitives>>,
{
    ...
    async fn build_consensus(self, ctx: &BuilderContext<N>) -> eyre::Result<Self::Consensus> {
        Ok(std::sync::Arc::new(
            crate::consensus::ArbBeaconConsensus::new(ctx.chain_spec()),
        ))
    }
}
        ...
        Box::pin(async move {
            ...
            let (exec_data, block_hash, send_root) = {
                ...
                fn read_address20(cur: &mut &[u8]) -> eyre::Result<alloy_primitives::Address> {
                    if cur.len() < 20 { return Err(eyre::eyre!("insufficient bytes for Address20")); }
                    let mut out = [0u8; 20];
                ...
                fn read_u64_be(cur: &mut &[u8]) -> eyre::Result<u64> {
                    if cur.len() < 8 { return Err(eyre::eyre!("insufficient bytes for u64")); }
                    let mut buf = [0u8; 8];
                ...
                fn parse_l2_message_to_txs(mut bytes: &[u8], chain_id: alloy_primitives::U256, poster: alloy_primitives::Address, request_id: Option<alloy_primitives::B256>) -> eyre::Result<Vec<reth_arbitrum_primitives::ArbTransactionSigned>> {
                    ...
                    match l2_kind {
                        0x00 | 0x01 => {
                            ...
                            let env = if l2_kind == 0x00 {
                                ...
                                let req = request_id.ok_or_else(|| eyre::eyre!("cannot issue contract tx without L1 request id"))?;
                        ...
                        0x03 => {
                            ...
                            while !inner.is_empty() {
                                match <Vec<u8> as Decodable>::decode(&mut inner) {
                                    Ok(seg) => {
                                        let mut seg_txs = parse_l2_message_to_txs(&seg, chain_id, poster, request_id)?;
                ...
                fn abi_encode_u256(v: &alloy_primitives::U256) -> [u8;32] { v.to_be_bytes::<32>() }
                ...
                fn encode_batch_posting_report_data(batch_timestamp: alloy_primitives::U256, batch_poster: alloy_primitives::Address, batch_num: u64, batch_data_gas: u64, l1_base_fee: alloy_primitives::U256) -> alloy_primitives::Bytes {
                ...
                let mut txs: Vec<reth_arbitrum_primitives::ArbTransactionSigned> = match kind {
                    ...
                    6 => Vec::new(),
                    7 => {
                        let req = request_id.ok_or_else(|| eyre::eyre!("cannot issue L2FundedByL1 tx without L1 request id"))?;
                        if l2_owned.is_empty() { return Err(eyre::eyre!("L2FundedByL1 message has no data")); }
                        let inner_kind = l2_owned[0];
                        let mut cur = &l2_owned[1..];
                        let zero = alloy_primitives::U256::from(0);
                        let one = alloy_primitives::U256::from(1);
                        let deposit_request_id = alloy_primitives::keccak256([req.0.as_slice(), &zero.to_be_bytes::<32>()].concat());
                        let unsigned_request_id = alloy_primitives::keccak256([req.0.as_slice(), &one.to_be_bytes::<32>()].concat());
                        let mut l2_buf = Vec::with_capacity(1 + cur.len());
                        l2_buf.push(inner_kind);
                        l2_buf.extend_from_slice(cur);
                        let mut derived = parse_l2_message_to_txs(&l2_buf, chain_id_u256, poster, Some(unsigned_request_id))
                            .map_err(|e| eyre::eyre!("parse L2FundedByL1 inner tx error: {e}"))?;
                        if derived.is_empty() { return Err(eyre::eyre!("L2FundedByL1 produced no inner tx")); }
                        let value = {
                        ...
                        let dep = arb_alloy_consensus::tx::ArbTxEnvelope::Deposit(arb_alloy_consensus::tx::ArbDepositTx {
                        ...
                        out.push(reth_arbitrum_primitives::ArbTransactionSigned::decode_2718(&mut s1)
                    ...
                    9 => {
                        ...
                        let retry_to_opt = if retry_to == alloy_primitives::Address::ZERO { None } else { Some(retry_to) };
                        ...
                        if data_len_u64 as usize > cur.len() { return Err(eyre::eyre!("retryable data too large")); }
                        let retry_data = cur[..(data_len_u64 as usize)].to_vec();
                        let req = request_id.ok_or_else(|| eyre::eyre!("cannot issue submit retryable without L1 request id"))?;
                        let env = arb_alloy_consensus::tx::ArbTxEnvelope::SubmitRetryable(arb_alloy_consensus::tx::ArbSubmitRetryableTx {
                            chain_id: chain_id_u256,
                            request_id: req,
                            from: poster,
                            l1_base_fee,
                    ...
                    12 => {
                        ...
                        let value = read_u256_be32(&mut cur)?;
                        let req = request_id.ok_or_else(|| eyre::eyre!("cannot issue deposit without L1 request id"))?;
                        let env = arb_alloy_consensus::tx::ArbTxEnvelope::Deposit(arb_alloy_consensus::tx::ArbDepositTx {
                            chain_id: chain_id_u256,
                            l1_request_id: req,
                            from: poster,
                            to,
                    ...
                    13 => {
                        ...
                        let base_data_gas = batch_gas_cost.ok_or_else(|| eyre::eyre!("cannot compute batch gas cost"))?;
                        let batch_data_gas = base_data_gas.saturating_add(extra_gas);
                        let data = encode_batch_posting_report_data(batch_timestamp, batch_poster_addr, batch_num, batch_data_gas, l1bf);
                        let env = arb_alloy_consensus::tx::ArbTxEnvelope::Internal(arb_alloy_consensus::tx::ArbInternalTx {
                            chain_id: chain_id_u256,
                            data,
                        });
                        let mut enc = env.encode_typed();
