        ...
        >,
{
    fn execute_message_to_block_sync(
        provider: &<N as reth_node_api::ProviderFactoryExt>::Provider,
        db_factory: &reth_node_api::ProviderFactory<N>,
        ...
        let sender: alloy_primitives::Address = reth_arbitrum_primitives::ARB_SYS_ADDRESS;
        let bal = reth_evm::system_calls::balance::ensure_has_enough_funds::<
            reth_arbitrum_primitives::ArbPrimitives,
            _,
        >(&mut db, sender, 1_000_000_000_000_000_000u128.saturating_mul(1000)).map_err(|e| eyre::eyre!("ensure sender balance error: {e}"))?;
        let outcome = builder.transact(sender, txs).map_err(|e| eyre::eyre!("block builder transact error: {e}"))?;
        let outcome = builder
            .set_result_and_post_accounting(outcome)
            .map_err(|e| eyre::eyre!("set_result_and_post_accounting error: {e}"))?;
        let outcome = builder
            .apply_evm_transitions(outcome)
            .map_err(|e| eyre::eyre!("apply_evm_transitions error: {e}"))?
            .flush_state()
            .map_err(|e| eyre::eyre!("flush_state error: {e}"))?;
        let outcome = builder
            .fill_block_and_states(outcome)
            .map_err(|e| eyre::eyre!("fill_block_and_states error: {e}"))?
        ...
        >,
{
    ...
    > {
        ...
        Box::pin(async move {
            ...
            return Ok((new_block_hash, new_send_root));
            ...
            let mut new_block_hash: alloy_primitives::B256 = alloy_primitives::B256::ZERO;
            let mut new_send_root: alloy_primitives::B256 = alloy_primitives::B256::ZERO;
            reth_tracing::tracing::info!(target: "arb-reth::follower", parent=%parent_hash, parent_gas_limit = sealed_parent.gas_limit(), "follower: loaded parent header");

            {
                ...
                new_block_hash = sealed_block.hash();
                new_send_root = reth_arbitrum_evm::header::extract_send_root_from_header_extra(
                    header.extra_data.as_ref(),
                );
