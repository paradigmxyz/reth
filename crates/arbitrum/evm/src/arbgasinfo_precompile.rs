...
fn handle_get_l1_basefee_estimate(input: &mut PrecompileInput<'_>) -> PrecompileResult {
    ...
    // Gas cost: 1 SLOAD + COPY, capped to available gas
...
fn handle_get_prices_in_wei(input: &mut PrecompileInput<'_>) -> PrecompileResult {
    ...
    // Gas cost: 2 SLOADs + COPY, capped to available gas
    let gas_cost = (2 * SLOAD_GAS_EIP2200 + COPY_GAS).min(gas_limit);
...
fn handle_get_current_tx_l1_fees(input: &mut PrecompileInput<'_>) -> PrecompileResult {
    let gas_limit = input.gas;
