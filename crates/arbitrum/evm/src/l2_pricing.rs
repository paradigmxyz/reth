use crate::storage::{Storage, StorageBackedBigUint, StorageBackedUint64};
...
impl<D: Database> L2PricingState<D> {
    ...
    pub fn initialize(storage: &Storage<D>, initial_base_fee: U256) {
        ...
        StorageBackedBigUint::new(state, base_key, BASE_FEE_WEI_OFFSET).set(initial_base_fee).ok();

        StorageBackedUint64::new(state, base_key, GAS_BACKLOG_OFFSET).set(0).ok();
    ...
    pub fn update_pricing_model(&self, _l2_base_fee: U256, time_passed: u64) -> Result<(), ()> {
        ...
        let gas_to_add_i64 =
            if gas_to_add > i64::MAX as u128 { i64::MAX } else { gas_to_add as i64 };
