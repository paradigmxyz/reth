...
#[instrument(level = "trace", skip(provider), target = "pruner")]
pub(crate) fn prune_receipts_by_logs<DB: Database>(
    ...
    let to_block = PruneMode::Distance(MINIMUM_PRUNING_DISTANCE)
        .prune_target_block(tip_block_number, MINIMUM_PRUNING_DISTANCE, PruneSegment::ContractLogs)?
        .map(|(bn, _)| bn)
        .unwrap_or_default();
    ...
    let mut from_tx_number = match initial_last_pruned_block {
        Some(block) => {
            provider.block_body_indices(block)?.map(|block| block.last_tx_num() + 1).unwrap_or(0)
        }
        None => 0,
    };
    ...
    // `address_filter`
    let address_filter = receipts_log_filter.group_by_block(tip_block_number, last_pruned_block)?;
...
mod tests {
    ...
    fn prune_receipts_by_logs() {
        ...
        let run_prune = || {
            ...
            let result = super::prune_receipts_by_logs(&provider, &receipts_log_filter, tip, 10);
