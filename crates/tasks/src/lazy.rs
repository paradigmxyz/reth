...
///
/// Create via [`Runtime::spawn_lazy_background`](crate::Runtime::spawn_lazy_background).
#[derive(Clone)]
pub struct LazyBackground<T> {
    inner: Arc<LazyBackgroundInner<T>>,
}

struct LazyBackgroundInner<T> {
    ...
    pub fn get(&self) -> &T {
        self.inner.value.get_or_init(|| {
            let rx = self
                ...
                .take()
                .expect("LazyBackground receiver already taken without value being set");
            rx.blocking_recv().expect("LazyBackground task dropped without producing a value")
        })
    }
...
impl<T: std::fmt::Debug + Send + Sync + 'static> std::fmt::Debug for LazyBackground<T> {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        let mut s = f.debug_struct("LazyBackground");
...
mod tests {
    ...
    #[test]
    fn test_lazy_background_resolves() {
        let (tx, rx) = oneshot::channel();
        let handle = LazyBackground::new(rx);
    ...
    #[test]
    fn test_lazy_background_clone_shares_value() {
        let (tx, rx) = oneshot::channel();
        let handle = LazyBackground::new(rx);
    ...
    #[test]
    fn test_lazy_background_into_inner() {
        let (tx, rx) = oneshot::channel();
        let handle = LazyBackground::new(rx);
    ...
    #[test]
    fn test_lazy_background_into_inner_with_clone() {
        let (tx, rx) = oneshot::channel();
        let handle = LazyBackground::new(rx);
    ...
    #[test]
    #[should_panic(expected = "LazyBackground task dropped without producing a value")]
    fn test_lazy_background_panics_on_dropped_sender() {
