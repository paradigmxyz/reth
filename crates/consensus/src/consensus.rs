...
use reth_primitives::{
    ...
    BlockNumber, SealedBlock, SealedHeader, H256,
};
...
impl BeaconConsensus {
    ...
    pub fn new(chain_spec: ChainSpecUnified) -> Self {
        Self {
...
impl Consensus for BeaconConsensus {
    ...
    fn validate_header(&self, header: &SealedHeader, parent: &SealedHeader) -> Result<(), Error> {
        ...
        if header.number < self.chain_spec.paris_block() {
    ...
    fn has_block_reward(&self, block_num: BlockNumber) -> bool {
        block_num < self.chain_spec.paris_block()
    }
}
