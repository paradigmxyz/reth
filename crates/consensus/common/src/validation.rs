...
pub fn validate_4844_header_standalone<H: BlockHeader>(
    ...
    let _excess_blob_gas = header.excess_blob_gas().ok_or(ConsensusError::ExcessBlobGasMissing)?;
    ...
    // TODO: Remove
    // // `excess_blob_gas` must also be a multiple of `DATA_GAS_PER_BLOB`. This will be checked
    // later // (via `calc_excess_blob_gas`), but it doesn't hurt to catch the problem sooner.
