...
use reth_rpc_types::{engine::ForkchoiceState, BlockNumberOrTag};
...
pub struct DevMiningTask<Client, Pool: TransactionPool, Executor, ExecutedBlock> {
    ...
    /// Executed blocks ready to be emitted
    executed_blocks: VecDeque<ExecutedBlock>,
...
impl<Client, Pool, Executor, ExecutedBlock> DevMiningTask<Client, Pool, Executor, ExecutedBlock>
where
    Client: StateProviderFactory + CanonChainTracker,
    Pool: TransactionPool,
    Executor: BlockExecutorProvider,
    ExecutedBlock: From<SealedBlockWithSenders>,
{
    pub fn new(
        chain_spec: Arc<ChainSpec>,
        client: Client,
    ...
    async fn mine_block(&mut self) -> Result<SealedBlockWithSenders, ()> {
        todo!();
    }
}
...
impl<Client, Pool, Executor, ExecutedBlock> DevMiningTask<Client, Pool, Executor, ExecutedBlock>
where
    Client: StateProviderFactory + CanonChainTracker + Unpin,
    Pool: TransactionPool + Unpin,
    Executor: BlockExecutorProvider,
    ExecutedBlock: From<SealedBlockWithSenders> + Unpin,
{
    fn poll_next(mut self: Pin<&mut Self>, cx: &mut Context<'_>) -> Poll<Option<Self>> {
        if let Some(block) = self.executed_blocks.pop_front() {
            todo!()
        }

        if let Some(mut task) = self.insert_task.take() {
