use crate::{consensus::MyConsensusEngine, engine_api::http::HttpJsonRpc, timing, ClStorage};
        ...
        'first_layer: loop {
            ...
            if this.insert_task.is_none() {
                ...
                this.insert_task = Some(Box::pin(async move {
                    ...
                    let forkchoice_updated_result = match MyConsensusEngine::forkchoice_updated(
                        &api,
                    ...
                    let forkchoice_updated_result =
                        match MyConsensusEngine::forkchoice_updated_with_attributes(
                            &api,
                            last_block_hash.clone(),
                        )
                        .await
                    ...
                    let payload_status = match MyConsensusEngine::new_payload(
                        &api,
                        execution_payload,
                    )
                    .await
                    ...
                    if let Some(latest_valid_hash) = &payload_status.latest_valid_hash {
                        ...
                        let forkchoice_updated_result: ForkchoiceUpdated =
                            match MyConsensusEngine::forkchoice_updated(
                                &api,
