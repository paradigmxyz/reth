...
impl BlockExecutionStrategyFactory for CustomExecutorStrategyFactory {
        ...
        DB: Database,
    {
        ...
        CustomExecutorStrategy { evm: EvmWithHook::new(evm), factory: self, block }
    }
}
    ...
    E: Evm<DB = &'db mut State<DB>>,
{
    ...
    fn with_state_hook(&mut self, hook: Option<Box<dyn reth_evm::system_calls::OnStateHook>>) {
        self.evm.hook = hook;
    }
}
