...
fn main() -> eyre::Result<()> {
    reth::cli::Cli::parse_args().run(|builder, args| async move {
        ...
        // let custom_rlpx_handler = CustomRlpxProtoHandler{ state: ProtocolState { events:
        // node.network.events.clone()}}; node.network.
        // add_rlpx_sub_protocol(custom_rlpx_handler);
