//! Command that initializes the node by importing OP chain below Bedrock, from a file.
...
use super::import::{build_import_pipeline, load_config};
...
mod tests {
    ...
    fn parse_common_import_command_chain_args() {
        for chain in SUPPORTED_CHAINS {
            let args: ImportOpCommand = ImportOpCommand::parse_from(["op-reth", "--chain", chain, "."]);
            assert_eq!(
                Ok(args.chain.chain),
