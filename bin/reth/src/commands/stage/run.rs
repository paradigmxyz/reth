    ...
    pub async fn execute(self) -> eyre::Result<()> {
        ...
        let (mut exec_stage, mut unwind_stage): (Box<dyn Stage<_>>, Option<Box<dyn Stage<_>>>) =
            match self.stage {
                ...
                StageEnum::AccountHistory => (Box::new(IndexAccountHistoryStage::default().with_etl_config(etl_config)), None),
                StageEnum::StorageHistory => (Box::new(IndexStorageHistoryStage::default().with_etl_config(etl_config)), None),
                _ => return Ok(()),
            };
